...
{$ENDIF}
  ...
  TSQLMemFieldTypeDesc = packed record
    Name:               AnsiString;
    AdvancedFieldType:  TSQLMemAdvancedFieldType;
  end;

// 0 - unknown
var SQLMemFieldTypeNamesCRC: array[1..35] of Cardinal;
var SQLFieldTypeNamesCRC: array[1..59] of Cardinal;

const SQLMemFieldTypes: array[1..35] of TSQLMemFieldTypeDesc = (
...
{$ENDIF}
  ...
  function AdvancedFieldTypeToBaseFieldType(AdvancedFieldType: TSQLMemAdvancedFieldType): TSQLMemBaseFieldType;
  // Convert BaseFieldType to AdvancedFieldType
  function BaseFieldTypeToAdvancedFieldType(BaseFieldType: TSQLMemBaseFieldType): TSQLMemAdvancedFieldType;

  // Converts TFieldType to TSQLMemAdvancedFieldType
  function FieldTypeToSQLMemAdvFieldType(FieldType: TFieldType): TSQLMemAdvancedFieldType;
  // Converts TSQLMemAdvancedFieldType to TFieldType
  function SQLMemAdvFieldTypeToFieldType(AdvancedFieldType: TSQLMemAdvancedFieldType): TFieldType;

  // AdvancedFieldType for print
  function AftToStr(AdvancedFieldType: TSQLMemAdvancedFieldType): AnsiString;
  // BaseFieldType for print
  function BftToStr(BaseFieldType: TSQLMemBaseFieldType): AnsiString;

  // SQLMem field type name to AdvancedFieldType
  function SQLMemStrToAft(SQLMemTypeName: AnsiString): TSQLMemAdvancedFieldType;
  // SQLMem field type name to BaseFieldType
  function SQLMemStrToBft(SQLMemTypeName: AnsiString): TSQLMemBaseFieldType;
  // SQL field type name to AdvancedFieldType
  function SQLStrToAft(SQLTypeName: AnsiString): TSQLMemAdvancedFieldType;
  // SQL field type name to BaseFieldType
  function SQLStrToBft(SQLTypeName: AnsiString): TSQLMemBaseFieldType;


  // Convert SQLMemDate to Date
  function SQLMemDateToDate(SQLMemDate: TSQLMemDate): TDate; overload;
  // Convert Date to SQLMemDate
  function DateToSQLMemDate(Date: TDate): TSQLMemDate; overload;

  // Convert SQLMemTime to Time
  function SQLMemTimeToTime(SQLMemTime: TSQLMemTime): TTime; overload;
  // Convert Time to SQLMemTime
  function TimeToSQLMemTime(Time: TTime): TSQLMemTime; overload;

  // Convert SQLMemDateTime to DateTime
  function SQLMemDateTimeToDateTime(SQLMemDateTime: TSQLMemDateTime): TDateTime; overload;
  // Convert DateTime to SQLMemDateTime
  function DateTimeToSQLMemDateTime(DateTime: TDateTime): TSQLMemDateTime; overload;

  // return true if field type is a BLOB field type
  function IsBLOBFieldType(FieldType: TSQLMemBaseFieldType): Boolean; overload;
  function IsBLOBFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean; overload;

  // return true if field type is a varchar field type
  function IsVarcharFieldType(FieldType: TSQLMemBaseFieldType): Boolean; overload;
  function IsVarcharFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean; overload;

  // return true if field type is a AnsiString field type, but not wide AnsiString
  function IsStringFieldType(FieldType: TSQLMemBaseFieldType): Boolean; overload;
  function IsStringFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean; overload;

  // return true if field type is a wide AnsiString field type
  function IsWideStringFieldType(FieldType: TSQLMemBaseFieldType): Boolean; overload;
  function IsWideStringFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean; overload;

  // return true if field type is bytes field type
  function IsBytesFieldType(FieldType: TSQLMemBaseFieldType): Boolean; overload;
  function IsBytesFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean; overload;

  // return true if field type is Autoinc field type
  function IsAutoincFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean; overload;

  // return true if DataType is numeric
  function IsNumericFieldType(FieldType: TSQLMemBaseFieldType): Boolean;

  // return true if DataType is integer
  function IsIntegerFieldType(FieldType: TSQLMemBaseFieldType): Boolean;

  // return true if DataType is TateTime, Time, Date, TimeStamp
  function IsDateTimeFieldType(FieldType: TSQLMemBaseFieldType): Boolean;

  // Result = Can cast this type
  function IsConvertableFieldType(FieldType: TSQLMemAdvancedFieldType): boolean;
  ...
  function GetStrLength(Buffer: PAnsiChar; FieldType: TSQLMemAdvancedFieldType): Integer;

  function GetReservedWord(Word: TReservedWord): WideString;

  // return HEX AnsiString converted from the source buffer
  function SQLMemBinaryToHEX(Data: PAnsiChar; Size: Integer): WideString;
  // return MIME64 AnsiString converted from the source buffer
  function SQLMemBinaryToMIME64(Data: PAnsiChar; Size: Integer): WideString;
  // allocate buffer and return binary data converted from HEX
  function SQLMemHEXToBinary(const HEXValue: WideString): PAnsiChar;
  // allocate buffer and return binary data converted from MIME64
  function SQLMemMIME64ToBinary(const MIME64Value: WideString): PAnsiChar;
{$IFDEF DEBUG_LOG}
  function LockTypeToStr(LockType: TSQLMemLockType): AnsiString;
  function GetRecordModeToStr(GetRecordMode: TSQLMemGetRecordMode): AnsiString;
  function GetRecordResultToStr(GetRecordResult: TSQLMemGetRecordResult): AnsiString;
{$ENDIF}
...
begin
  ...
  if (Result = aftUnknown) then
    raise ESQLMemException.Create(30289, ErrorGUnknownDataType,
                               [IntToStr(Integer(FieldType))]); 
end;//FieldTypeToSQLMemAdvFieldType
...
end;//SQLMemAdvFieldTypeToFieldType
...
begin
  Result := aftUnknown;
  crc := GetTableNameCRCAnsi(SQLMemTypeName,True);
  for i := 1 to High(SQLMemFieldTypeNamesCRC) do
   if (SQLMemFieldTypeNamesCRC[i] = crc) then
    begin
     Result := SQLMemFieldTypes[i].AdvancedFieldType;
     break;
    end;
end; // SQLMemStrToAft
...
begin
  ...
  for i := 1 to High(SQLMemFieldTypeNamesCRC) do
   if (SQLMemFieldTypeNamesCRC[i] = crc) then
    begin
     Result := AdvancedFieldTypeToBaseFieldType(SQLMemFieldTypes[i].AdvancedFieldType);
     break;
    end;
end; // SQLMemStrToBft
...
// Convert SQLMemDate to Date
//------------------------------------------------------------------------------
function SQLMemDateToDate(SQLMemDate: TSQLMemDate): TDate;
begin
  if (SQLMemDate = DATE_IS_NULL) then
   Result := 0
  else
   Result := SQLMemDate - DateDelta;
end;//SQLMemDateToDate


//------------------------------------------------------------------------------
// Convert Date to SQLMemDate
//------------------------------------------------------------------------------
function DateToSQLMemDate(Date: TDate): TSQLMemDate;
begin
  Result := Trunc(Date) + DateDelta;
end;//DateToSQLMemDate


//------------------------------------------------------------------------------
// Convert SQLMemTime to Time
//------------------------------------------------------------------------------
function SQLMemTimeToTime(SQLMemTime: TSQLMemTime): TTime;
begin
  if (SQLMemTime = TIME_IS_NULL) then
   Result := 0
  else
   Result := Frac(SQLMemTime / (24*60*60*1000));
end;//SQLMemTimeToTime


//------------------------------------------------------------------------------
// Convert Time to SQLMemTime
//------------------------------------------------------------------------------
function TimeToSQLMemTime(Time: TTime): TSQLMemTime;
begin
  //Result := Trunc(Frac(Time) * (24*60*60*1000));
  Result := Round(Frac(Time) * (24*60*60*1000));
end;//TimeToSQLMemTime


//------------------------------------------------------------------------------
// Convert SQLMemDateTime to DateTime
//------------------------------------------------------------------------------
function SQLMemDateTimeToDateTime(SQLMemDateTime: TSQLMemDateTime): TDateTime;
begin
  if (SQLMemDateTime.Time = TIME_IS_NULL) then
   Result := SQLMemDateToDate(SQLMemDateTime.Date)
  else
   Result := SQLMemDateToDate(SQLMemDateTime.Date) + SQLMemTimeToTime(SQLMemDateTime.Time);
end;//SQLMemDateTimeToDateTime


//------------------------------------------------------------------------------
// Convert DateTime to SQLMemDateTime
//------------------------------------------------------------------------------
function DateTimeToSQLMemDateTime(DateTime: TDateTime): TSQLMemDateTime;
begin
  Result.Date := DateToSQLMemDate(DateTime);
  Result.Time := TimeToSQLMemTime(DateTime);
end;//SQLMemDateTimeToDateTime
...
// return true if field type is a BLOB field type
//------------------------------------------------------------------------------
function IsBLOBFieldType(FieldType: TSQLMemBaseFieldType): Boolean;
begin
...
// return true if field type is a BLOB field type
//------------------------------------------------------------------------------
function IsBLOBFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean;
begin
...
// return true if field type is a varchar field type
//------------------------------------------------------------------------------
function IsVarcharFieldType(FieldType: TSQLMemBaseFieldType): Boolean;
begin
...
// return true if field type is a varchar field type
//------------------------------------------------------------------------------
function IsVarcharFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean;
begin
...
// return true if field type is a AnsiString field type, but not wide AnsiString
//------------------------------------------------------------------------------
function IsStringFieldType(FieldType: TSQLMemBaseFieldType): Boolean;
begin
...
// return true if field type is a AnsiString field type, but not wide AnsiString
//------------------------------------------------------------------------------
function IsStringFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean;
begin
...
// return true if field type is a wide AnsiString field type
//------------------------------------------------------------------------------
function IsWideStringFieldType(FieldType: TSQLMemBaseFieldType): Boolean;
begin
...
// return true if field type is a wide AnsiString field type
//------------------------------------------------------------------------------
function IsWideStringFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean;
begin
...
//------------------------------------------------------------------------------
function IsBytesFieldType(FieldType: TSQLMemBaseFieldType): Boolean;
begin
...
// return true if field type is a Bytes field type, but not wide Bytes
//------------------------------------------------------------------------------
function IsBytesFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean;
begin
...
//------------------------------------------------------------------------------
function IsAutoincFieldType(FieldType: TSQLMemAdvancedFieldType): Boolean;
begin
...
function GetReservedWord(Word: TReservedWord): WideString;
begin
  Result := SQLMemSQLReservedWords[Integer(Word)];
end; // GetReservedWord
...
procedure SQLMemFillTypeNamesCRC;
var i:    Integer;
    crc:  Cardinal;
begin
  for i := 1 to High(SQLMemFieldTypeNamesCRC) do
   begin
    crc := GetTableNameCRCAnsi(SQLMemFieldTypes[i].Name,True);
    SQLMemFieldTypeNamesCRC[i] := crc;
   end;
  for i := 1 to High(SQLFieldTypeNamesCRC) do
   begin
...
end; // SQLMemFillTypeNamesCRC


initialization

{$IFDEF DEBUG_LOG_INIT}
aaWriteToLog('SQLMemConverts> starting initialization...');
{$ENDIF}
  SQLMemMemoryIncUseCount;
  SQLMemFillTypeNamesCRC;
{$IFDEF DEBUG_LOG_INIT}
aaWriteToLog('SQLMemConverts> initialized');
{$ENDIF}
