...
{$IFNDEF D5H}
   ,SQLMemD4Routines
{$ENDIF}
  , SQLMemMain, SQLMemConst, SQLMemTypes, SQLMemConverts, SQLMemComMain
  {$IFDEF DEBUG_LOG}
  ,SQLMemDebug
  {$ENDIF}
  ,dbcgrids, Mask;
...
type
  TSQLMemFormDatabaseDesigner = class(TForm)
    gbTables: TGroupBox;
    Splitter1: TSplitter;
    dbManager: TGroupBox;
    lbTables: TListBox;
  ...
  public
    ...
    procedure FillFieldsTable(SourceTable: TSQLMemDataset);
  end;

var
  fmSQLMemDatabaseDesigner: TSQLMemFormDatabaseDesigner;
...
procedure TSQLMemFormDatabaseDesigner.bnCloseClick(Sender: TObject);
begin
 Close;
end;

procedure TSQLMemFormDatabaseDesigner.bnLoadClick(Sender: TObject);
...
procedure TSQLMemFormDatabaseDesigner.bnSaveClick(Sender: TObject);
begin
 if (Database <> nil) then
...
procedure TSQLMemFormDatabaseDesigner.bnRefreshClick(Sender: TObject);
var dataActive: Boolean;
begin
  dataActive := DataTable.Active;
  if (dataActive) then DataTable.Close;
...
procedure TSQLMemFormDatabaseDesigner.GetTables;
begin
  if (Database <> nil) then
   begin
...
procedure TSQLMemFormDatabaseDesigner.GetSourceDatasets;
var dsList: TList;
    i:      Integer;
...
begin
 tName := '';
 cName := ds.Name;
 if (ds is TSQLMemTable) then
  tName := TSQLMemTable(ds).TableName;
 if (tName <> '') then
  newName := tName
 else
  newName := cName;
...
procedure TSQLMemFormDatabaseDesigner.FormCreate(Sender: TObject);
begin
  ...
  if (Sender is TSQLMemDatabase) then
   Database := TSQLMemDatabase(Sender)
  else
   Database := nil;
  DataTable := TSQLMemTable.Create(nil);
  FieldsTable := TSQLMemTable.Create(nil);
  FieldsTable.InMemory := True;
  FieldsTable.TableName := '@FieldsTable';
  ForeignKeysTable := TSQLMemTable.Create(nil);
  ForeignKeysTable.InMemory := True;
  ForeignKeysTable.TableName := '@ForeignKeysTable';
  IndexesTable := TSQLMemTable.Create(nil);
  IndexesTable.InMemory := True;
  IndexesTable.TableName := '@IndexesTable';

  ImportTable := TSQLMemTable.Create(nil);
  ImportTable.InMemory := True;
  ImportTable.TableName := '@ImportTable';
  dsImport.DataSet := ImportTable;
  Query := TSQLMemQuery.Create(nil);
  pgMain.ActivePage := tsTableManager;
  pgTableManager.ActivePage := tsData;
end;

procedure TSQLMemFormDatabaseDesigner.FormClose(Sender: TObject;
...
procedure TSQLMemFormDatabaseDesigner.FormShow(Sender: TObject);
begin
 GetTables;
 GetSourceDatasets;
end;

procedure TSQLMemFormDatabaseDesigner.bnImportClick(Sender: TObject);
var s,s1:       AnsiString;
    t:          Cardinal;
    n,er:       Integer;
    f:          Double;
     ...
     begin
      ...
      try
        ...
        if (rbAll.Checked) then
         ...
         begin
          if (ds is TSQLMemDataSet) then
           begin
            tbl.AssignDefinitions(TSQLMemDataset(ds));
           end
          else
           begin
...
procedure TSQLMemFormDatabaseDesigner.lbTablesClick(Sender: TObject);
var n: Integer;
begin
...
procedure TSQLMemFormDatabaseDesigner.bnAllClick(Sender: TObject);
var
    idx:  String;
    bm:   TBookmark;
begin
...
procedure TSQLMemFormDatabaseDesigner.bnLoadSQLScriptClick(Sender: TObject);
begin
 if (Database <> nil) then
  if (odLoadSQL.Execute) then
   try
     reSQLConsole.Lines.LoadFromFile(odLoadSQL.FileName);
   except
     on e: Exception do
...
procedure TSQLMemFormDatabaseDesigner.bnSaveSQLClick(Sender: TObject);
begin
 if (Database <> nil) then
  if (sdSaveSQL.Execute) then
   try
     reSQLConsole.Lines.SaveToFile(sdSaveSQL.FileName);
   except
     on e: Exception do
...
procedure TSQLMemFormDatabaseDesigner.bnOpenQueryClick(Sender: TObject);
var
    t:            Cardinal;
    f:            Double;
    s,s1:         AnsiString;
...
procedure TSQLMemFormDatabaseDesigner.bnCloseSQLClick(Sender: TObject);
begin
 Query.Close;
end;

procedure TSQLMemFormDatabaseDesigner.bnExportToSQLClick(Sender: TObject);
var s:        String;
    t:        Cardinal;
    f:        Double;
begin
...
procedure TSQLMemFormDatabaseDesigner.CreateFieldsTable;
var i: Integer;
begin
  with FieldsTable do
   begin
...
procedure TSQLMemFormDatabaseDesigner.CreateIndexesTable;
begin
...
procedure TSQLMemFormDatabaseDesigner.CreateForeignKeysTable(skipNameCheck: Boolean = false);
var i:  Integer;
    sl: TStringList;
begin
{
...
procedure TSQLMemFormDatabaseDesigner.FillFieldsTable(SourceTable: TSQLMemDataset);
var i,j : integer;
    s : string;
begin
{
 if ((SourceTable = DataTable) or (SourceTable = Query)) then
  begin
   eRecordSize.Text := IntToStr(TSQLMemDataset(SourceTable).GetDiskRecordSize);
   eOptimalPageSize.Text := IntToStr(TSQLMemDataset(SourceTable).GetOptimalPageSize);
  end;
}
  ...
  begin
     for i := 0 to SourceTable.AdvFieldDefs.Count-1 do
      ...
      // ftype := SourceTable.FieldDefs.Items[i].DataType;
       s := '';
       for j := Low(SQLMemFieldTypes) to High(SQLMemFieldTypes) do
        if (SQLMemFieldTypes[j].AdvancedFieldType = SourceTable.AdvFieldDefs.Items[i].DataType) then
         begin
          s := SQLMemFieldTypes[j].Name;
          break;
         end;
