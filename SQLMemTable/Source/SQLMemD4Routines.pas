...
(*
  ...
  reSafeCallError     = 24;
*)
function SQLMem_VarArrayGet(var A: Variant; IndexCount: Integer;
...
begin
  if TVarData(A).VType and varArray = 0 then
   raise ESQLMemException.Create(10434,ErrorLVarNotArray);
...
procedure Error(errorCode: Byte);
begin
 raise ESQLMemException.Create(10437,ErrorLSystemError,[errorCode]);
end;
...
begin
  if TVarData(A).VType and varArray = 0 then
   raise ESQLMemException.Create(10433,ErrorLVarNotArray);
  VarArrayPtr := GetVarArray(A);
  if VarArrayPtr^.DimCount <> IndexCount then
   raise ESQLMemException.Create(10435,ErrorLVarArrayBounds,
  ...
  begin
    if SafeArrayPtrOfIndex(VarArrayPtr, @Indices, P) <> 0 then
     raise ESQLMemException.Create(10436,ErrorLVarArrayBounds,
      [-1,-1]);
  ...
  if SafeArrayGetElement(VarArrayPtr, @Indices,
      @TVarData(Result).VPointer) <> 0 then
       raise ESQLMemException.Create(10438,ErrorLVarArrayBounds,
        [-1,-1]);
    TVarData(Result).VType := VarType;
  end;
end; //SQLMem_VarArrayGet
...
begin
  if not TextToFloat(PChar(S), Result, fvExtended) then
    raise ESQLMemException.Create(10441,ErrorLInvalidFloat,[S]);
end;
...
begin
  if not TextToFloat(PChar(S), Result, fvExtended, FormatSettings) then
    raise ESQLMemException.Create(10442,ErrorLInvalidFloat,[S]);
end;
...
begin
  if not TryFloatToCurr(Value, Result) then
    raise ESQLMemException.Create(10439,ErrorLInvalidCurrency,[FloatToStr(Value)]);
end;
...
begin
  if not TryStrToBool(S, Result) then
    raise ESQLMemException.Create(10440,ErrorLInvalidBoolean,[s]);
end;
...
// ShowMessage('SQLMemD4Routines initialization started');
aaWriteToLog('SQLMemD4Routines> initialization started');
{$ENDIF}

  ClearAnyProc := @VarInvalidOp;

{$IFDEF DEBUG_LOG_INIT}
aaWriteToLog('SQLMemD4Routines> initialization finished');
{$ENDIF}
