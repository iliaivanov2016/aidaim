...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
  ...
  if (!ACRTable1->Exists)
  {
   ...
   ACRTable1->FieldDefs->Add("Name",ftString,30,False);
   ACRTable1->FieldDefs->Add("Photo",ftGraphic,0,False);
   ACRTable1->FieldDefs->Add("Journal",ftFmtMemo,0,False);
   ACRTable1->FieldDefs->Add("Comments",ftMemo,0,False);
   ACRTable1->FieldDefs->Add("File",ftBlob,0,False);

   ACRTable1->IndexDefs->Clear();
   ACRTable1->IndexDefs->Add("PrimaryKey","ID",TIndexOptions()<<ixPrimary);
   ACRTable1->IndexDefs->Add("ByName","Name",TIndexOptions()<<ixCaseInsensitive);
