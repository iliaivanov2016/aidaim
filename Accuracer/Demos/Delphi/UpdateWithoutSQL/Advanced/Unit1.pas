...
uses
  ...
  Dialogs, DB,
  ACRMain, ACRVariant, ACRTypes, ACRConst,
...
begin
 ...
 try
   ...
   value.AsString := tMain.FieldByName('name').AsString;
   sl.Add('vessel_details');
   // internal method used for optimization of UPDATE SQL statement
   tInout.UpdateVisibleRecords(sl,[value]);
 finally
  try
   tInout.Filtered := false;
   tInout.Refresh;
