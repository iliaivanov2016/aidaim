...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, Grids, DBGrids, Db, FR_PTabl,
  ACRMain;

type
  TMainForm = class(TForm)
    AccuracerTable1: TACRTable;
    ...
    btFindFirst: TButton;
    btFindNext: TButton;
    AccuracerDatabase1: TACRDatabase;
    bnReport: TBitBtn;
...
begin
 if MessageDlg('Do you really want to delete '+
               QuotedStr(AccuracerTable1.FieldByName('Company').AsString)+
               ' company?',mtConfirmation,[mbYes,mbNo],0) = mrYes then
  AccuracerTable1.Delete;
 // update buttons
 UpdateButtons;
end;
...
begin
 ...
 if (AccuracerTable1.RecordCount > 0) then
  begin
 ...
 if AccuracerTable1.Filtered then
  begin
...
begin
 AccuracerTable1.Filtered := false;
 UpdateButtons;
end;
...
begin
 with AccuracerTable1 do
  begin
   ...
   if cbNoPartialCompare.Checked then
    FilterOptions := FilterOptions + [foNoPartialCompare];
   if not AccuracerTable1.FindFirst then
...
begin
 if not AccuracerTable1.FindNext then
  MessageDlg('Record not found.',mtInformation,[mbOk],0);
end;
