...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
  ...
  if (!ACRTable1->Exists)
  {
   ...
   ACRTable1->IndexDefs->Add("ByCompanyAsc","Company",TIndexOptions()<<ixCaseInsensitive);
   ACRTable1->IndexDefs->Add("ByCompanyDesc","Company",TIndexOptions()<<ixDescending);
   ACRTable1->CreateTable();
   // open table
   ACRTable1->Active = true;
   // add complex index
   ACRTable1->AddIndex("ByAddrAscCompanyDesc","Address;Company",TIndexOptions()<<ixCaseInsensitive,"Company","");
  }
  // open table
  ACRTable1->Active = true;
  // update buttons
