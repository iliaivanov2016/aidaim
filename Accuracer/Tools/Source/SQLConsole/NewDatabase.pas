...
uses
  ...
  Dialogs, Buttons, StdCtrls, ExtCtrls, Grids, Spin, ComCtrls, ACRConst, ACRDECFmt;
...
//   keyHEX := StrToFormat(key,fmMain.CurrentDB.CryptoParams.KeySize,fmtHEX);
   try
    keyHEX := fm.Encode(key^,fmMain.CurrentDB.CryptoParams.KeySize)
   finally
    fm.Free;
   end;
