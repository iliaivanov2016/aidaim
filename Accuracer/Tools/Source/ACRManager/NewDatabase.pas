...
uses
  Windows, ACRMain, ACRDecFmt, Messages, SysUtils, {Variants,} Classes, Graphics, Controls, Forms,
...
type
  TFormNewDatabase = class(TForm)
    ...
    procedure seMaxConnectionsChange(Sender: TObject);
...
uses MainUnit, ACRDecUtil, WorkGrids;
...
begin
 ...
 if not (Button1.Visible) then
  begin
   MainForm.ConvertDataSettingsToDataParams;
   bClose := false;
  end
 else
  begin
   ...
   EncryptedClick(FormNewDatabase);
...
var
 inv : PAnsiChar; //Pál Laurinyecz 2008. 10. 28.
 invHEX :string;
 fm: TFormat_HEX;
begin
   IsChange := True;
   EmptyGrid(InitVectorGrid);
   MainForm.CurrentDB.CryptoParams.MakeRandomInitVector;
   inv := MainForm.CurrentDB.CryptoParams.GetInitVector;
   fm := TFormat_HEX.Create;
   try
     invHEX := fm.Encode(inv^,MainForm.CurrentDB.CryptoParams.MaxInitVectorSize);
   finally
    fm.Free;
   end;
...
var
 key : PAnsiChar; //Pál Laurinyecz 2008. 10. 28.
 keyHEX :string;
 fm:   TFormat_HEX;
begin
   IsChange := True;
   EmptyGrid(KeyGrid);
   MainForm.CurrentDB.CryptoParams.MakeRandomKey(seKeySize.Value);
   key := MainForm.CurrentDB.CryptoParams.GetKey;
   fm := TFormat_HEX.Create;
   try
    keyHEX := fm.Encode(key^,MainForm.CurrentDB.CryptoParams.KeySize);
   finally
    fm.Free;
   end;
...
procedure TFormNewDatabase.cbAlgorithmSelect(Sender: TObject);
begin
 IsChange := True;
end;
 
procedure TFormNewDatabase.cbModeSelect(Sender: TObject);
begin
 IsChange := True;
end;

procedure TFormNewDatabase.sePageSizeChange(Sender: TObject);
begin
 if sePageSize.Modified then IsChange := True;
end;

procedure TFormNewDatabase.seMaxConnectionsChange(Sender: TObject);
begin
 if seMaxConnections.Modified then IsChange := True;
end;

procedure TFormNewDatabase.tPasswordChange(Sender: TObject);
begin
 if tPassword.Modified then IsChange := True;
end;

procedure TFormNewDatabase.seKeySizeChange(Sender: TObject);
begin
