...
//------------------------------------------------------------------------------
  ...
  TACRCryptoKey = packed record
   Key:             array [0..ACR_MAX_KEY] of Byte;
   KeySize:         Word; // 0 by default
  end;

  TACRCryptoInfo = packed record
   KeyInfo:         TACRCryptoKey;
   InitVector:      array [0..ACR_MAX_VECTOR] of Byte;
   InitVectorSize:  Word;
   Password:        AnsiString; // ACRDefaultPassword by default
  ...
  end;
{
  ...
  TACRCryptoControlBlock = packed record
   Data:            array [0..ACR_MAX_CONTROL_BLOCK] of Byte;
  end; // 256

  TACRCryptoHeader = packed record
   ControlBlock:      TACRCryptoControlBlock;
 ...
 TACRCryptoParams = TACRCryptoInfo;

//------------------------------------------------------------------------------
// general types
//------------------------------------------------------------------------------

 TACRIntegerArray = class;

 TACRErrorCode = Integer;
 TACRPageNo = Integer;
 PACRPageNo = ^TACRPageNo;
 TACRPageBuffer = PAnsiChar;
 TACRGetRecordMode = (grmCurrent, grmNext, grmPrior);
 TACRGetRecordResult = (grrOK, grrBOF, grrEOF, grrError, grrReloadCache); // grrReloadCache - cleint cache must be reloaded
 TACRBLOBOpenMode = (bomWrite, bomRead, bomReadWrite);
 TACRRecordBuffer = PAnsiChar;
 TACRState = Cardinal;
 TACRObjectID = Integer;
 TACRTableID = TACRObjectID; 
 // changed in v.5
// TACRObjectName = ShortString;
 TACRObjectName = WideString;
 TACRSessionID = Integer;
 TACRRecordNo = Int64;
 TACRPageItemID = Word;
 TACRRecordID = packed record
  PageNo:        TACRPageNo; // page number or record number (disk engine or memory, temporary engine)
  PageItemNo:    TACRPageItemID;
 end;
 PACRRecordID = ^TACRRecordID;
 TACRLockParams = packed record
  RetryCount:   Integer;
  Delay:        Integer;
 end;
 { TODO -oLeo : change to cardinal }
 TACROptions = packed record
...
//------------------------------------------------------------------------------
 ...
 TACRIsPageSynchronizationRequired =
  function (PageState: Cardinal): Boolean of object;

 // 65535 - page is used by other table or 100% filled 
 TACRPageRecordCount = Word;
 PACRPageRecordCount = ^TACRPageRecordCount;
 TACRDBFileType = (dbftUnknown,dbftTablesList,dbftActiveSessionsList,dbftStoredFunctionManager);
 TACRLockType = (ltIS,ltS,ltIRW,ltRW,ltU,ltX);
 TACRLockObjectType = (lotDatabase,lotTable,lotRecord);
 ...
 TACRTablePageType = (tptRowStart,tptRowContinue,tptVarchar,tptBLOB);
 TACRDBStateType = (
//                     0       1               2             3
...
// +----------+-----------+----------+--------------+-----------------+
 TACRDBHeader = packed record
  Signature:        TACRSignature;
  Version:          Double;
  PageSize:         Word;
  HeaderSize:       Word;
  ExtentPageCount:  Word;
  ...
  CryptoHeader:     TACRCryptoHeader; // for database encryption
  FSMHeaderSize:    Byte;
  TLHeaderSize:     Byte;
...
*)
 ...
 TACRFSMHeader = packed record
  TotalPageCount:   TACRPageNo;
  FreePageCount:    TACRPageNo;
  State:            Cardinal; // state - increment by 1 on each modification
 end; // FreeSpaceManager header - can be modified only by TACRFreeSpaceManager

 TACRTLHeader = packed record
  TableCount:       Cardinal;
  State:            Cardinal; // state - increment by 1 on each modification
  StatePageCount:   Cardinal; // number of table state pages
 end; // TACRTableList header - can be modified only by TACRTableList

 // added in v.5.10
 TACRSFMHeader = packed record
  Count:        Integer;
  State:        TACRState;
 end; // TACRSFMHeader


 TACRLockedBytes = packed record
  LockedByteCount:  Word;
 end;

 TACRPageTypeID = Word;

 TACRDiskPageHeader = packed record
  Signature:        Array [0..3] of AnsiChar;
  NextPageNo:       TACRPageNo;
  ObjectID:         TACRObjectID;
  CRC32:            Cardinal;
  PageType:         TACRPageTypeID; // 2 bytes???
  CRCType:          Byte;
  CipherType:       Byte;
  Reserved:         Byte;
 end; // 21 bytes
 PACRDiskPageHeader = ^TACRDiskPageHeader;

 TACRBackupDescHeader = packed record
  Date:                 TDateTime;
  FileSize:             Int64;
  TableCount:           Integer;
  DescLength:           Integer;
  ...
  // description
  // tables names
 end; // TACRBackupDescHeader

 TACRBackupHeader = packed record
  Signature:            Array [0..3] of AnsiChar;
  BlockSize:            Integer;
  CompressionAlgorithm: Byte;
  CompressionMode:      Byte;
  CryptoHeader:         TACRCryptoHeader; // for database encryption
  DescHeader:           TACRBackupDescHeader;
 end; // TACRBackupHeader


 TACRBlockHeader = packed record
  CompressedSize:   Integer;
  UncompressedSize: Integer;
 end; // TACRBlockHeader


  // ACRDatabaseFile Mode Types
  TACRShareMode = (smExclusive, smShared);
  TACRAccessMode = (amReadOnly, amReadWrite);
 
 // Any list header
 PACRListHeader = ^TACRListHeader;
 TACRListHeader = packed record
   Count:       Integer;
   ItemSize:    Integer;
   NextPageNo:  TACRPageNo; // or INVALID_PAGE_NO
 end;// 12 byte

 // Header for internal file
 PACRInternalFileHeader = ^TACRInternalFileHeader;
 TACRInternalFileHeader = packed record
 ...
 TACRSystemDirectoryListItem = packed record
   FirstPageNo: TACRPageNo;
   FileID:      TACRDBFileType;
 end;// 5 bytes


 // Table List Item
 PACRTableListItem = ^TACRTableListItem;
 TACRTableListItem = packed record
// updated to Unicode
...
// added in v.5 - PageNo and offset from beginning of PageBuffer to 32-bit state
   TableStateAddress:                   TACRRecordID;
// added in v.5
   CreationDate:                        TDateTime;
   MetaDataFilePageNo:                  TACRPageNo;
   MostUpdatedDataFilePageNo:           TACRPageNo;
// commented in v.5
//   PFSPageMapFilePageNo:                TACRPageNo;
   LockFilePageNo:                      TACRPageNo;
//   CryptoHeader:                        TACRCryptoHeader; // for table encryption
 end;//34 bytes

 TACRTablePFSPageMapItem = packed record
   PageNo:          TACRPageNo;
   PageRecordCount: Integer;
 end;

 TACRTablePFSItem = packed record
 ...
 // blob or VarChar
 TACRDataItem = packed record
    ItemID:       Word;
    DataSize:     Word;
 end;
 TACRDataItemsMap = array of TACRDataItem;

 // added in v.5
 TACRLastTableOperation = (
                            ltoCreateTable,
                            ltoInsert,
                            ltoUpdate,
                            ltoDelete,
 ...
 TACRTableFlags = (tffWriteFailed);
 TACRTableState = packed record
   TableState:            TACRState; // Most Updated Data state plus code of last operation
   ...
   LastTableOperation:    TACRLastTableOperation; // last table operation that changed MostUpdatedData
 ...
 TACRTableInfoArray = array of TACRTableInfo;
...
// +----------+---------+------------+-------------------+-----------------+
 ...
 // stats for all table
 TACRTableLockInfo = packed record
   LockXSessionID:      TACRSessionID; // INVALID_SESSION_ID if not locked
   LockIRWSessionID:    TACRSessionID; // INVALID_SESSION_ID if not locked
   LockRWSessionID:     TACRSessionID; // INVALID_SESSION_ID if not locked
 ...
 TACRSessionLockInfo = packed record
   SessionID:      TACRSessionID;
   NumLocksIS:     Cardinal;
   NumLocksS:      Cardinal;
   NumLocksIRW:    Cardinal;
   LockedRecordID: TACRRecordID;
   WaitTime:       Cardinal; // start waiting time
   WaitLockType:   TACRLockType;
   IsWaiting:      ByteBool;
   LockX:          ByteBool;
   LockRW:         ByteBool;
   LockU:          ByteBool;
 end;
 PACRSessionLockInfo = ^TACRSessionLockInfo;

 TACRTransactionLockInfo = packed record
   SessionID:      TACRSessionID;
 ...
 PACRTransactionLockInfo = ^TACRTransactionLockInfo;
...
//------------------------------------------------------------------------------
 ...
 // join type
 TACRJoinType = (ajtCross, ajtInner, ajtLeftOuter,
                 ajtRightOuter, ajtFullOuter);

 // union type
 TACRUnionType = (autUnion, autIntersect, autExcept);

 // table | joined table | subquery
 TACRTableType = (attTable, attJoinedTable, attSubQuery);

 // table | joined table | subquery
 TACRQueryExprType = (qetSelect, qetUnion, qetExcept, qetIntersect);

 // hash value of the record or field
 TACRRecordHashValue = Cardinal;
 PACRRecordHashValue = pCardinal;

 TACRDistinctAlgorithm = (adaNone, adaBySourceIndex, adaByDestIndex, adaByRecordHash);
...
//    aftIDispatch,
 ...
 //TACRDefaultValueType = (dvtNull, dvtConst, dvtSequence, dvtFunction, dvtQuery);
 TACRCompareResult = (cmprEqual, cmprLower, cmprGreater,
                      cmprLeftNull, cmprRightNull, cmprBothNull);


 TACRHexStringFormat = (ahexfDefault,ahexfDelphi,ahexfCPP);                     
//------------------------------------------------------------------------------
// record types
//------------------------------------------------------------------------------

 // used in ACRMemEngine for saving / loading table to stream
 TACRMemoryTableFileHeader = packed record
  Signature:            array [0..3] of AnsiChar ;
  Version:              Double;
...
//NameLength:           Byte;
 ...
 TACRIndexPosition = TObject;
 TACRSearchInfo = Pointer;

  // bookmark information type
  TACRBookmarkFlag = (abfCurrent, abfBOF, abfEOF, abfInserted);
  TACRBookmarkInfo = packed record
   BookmarkData: TACRRecordID;      // record ID
   BookmarkFlag: TACRBookmarkFlag;  // bookmark flag
  end;
  PACRBookmarkInfo = ^TACRBookmarkInfo;

 TACRSequenceValue = Int64;

 TACRTimeoutParams = record
  RetryCount:   Word;
  Timeout:      Integer;
 end;

 //------------------------- Search types --------------------------------------
  TACRKeyIndex = (kiLookup, kiRangeStart, kiRangeEnd, kiCurRangeStart,
    kiCurRangeEnd, kiSave);
  TACRSearchCondition = (scNone,scEqual,scLower,scGreater,scLowerEqual,scGreaterEqual);
  TACRKeySearchType = (kstFirst,kstLast,kstAny);

  PACRKeyBuffer = ^TACRKeyBuffer;
  TACRKeyBuffer = packed record
    Modified: Boolean;
    Exclusive: Boolean;
    FieldCount: Integer;
  end;

  TACRNavigationInfo = packed record
    // in
    SessionID:        TACRSessionID; // for disk engine only; INVALID_SESSION_ID for memory and temp engines
    GetRecordMode:    TACRGetRecordMode;
    IndexID:          TACRObjectID; // current index id (INVALID_OBJECT_ID if no active index)
    // in and out
...
////////////////////////////////////////////////////////////////////////////////
//
// TACRScanSearchCondition
//
////////////////////////////////////////////////////////////////////////////////

  TACRScanSearchCondition = class(TObject)
   public
    Condition:                 TACRSearchCondition;
    KeyRecordBuffer:           TACRRecordBuffer; // if nil then Expression will be used
...
{$IFDEF RECORD_SEARCH_CACHE_IN_CURSOR}
    ParamIndexes:              TACRIntegerArray;
    OwnKeyBuffer:              Boolean;
    OwnExpression:             Boolean;
{$ENDIF}
    constructor Create;
    destructor Destroy; override;
    procedure Assign(ScanSearchCondition: TACRScanSearchCondition);
  end;

  TACRExtractedConditionInfo = packed record
    KeyRecordBuffer:           TACRRecordBuffer;
    ExtractedExpressionNode:   Pointer;
    Expression:                Pointer; // if nil then KeyBuffer will be used
  end;
  PACRExtractedConditionInfo = ^TACRExtractedConditionInfo;


////////////////////////////////////////////////////////////////////////////////
//
// TACRScanSearchConditionArray
//
////////////////////////////////////////////////////////////////////////////////

{$IFNDEF RECORD_SEARCH_CACHE_IN_CURSOR}
  TACRScanSearchConditionArray = class(TObject)
   public
    Items:    array of TACRScanSearchCondition;
    Count:    Integer;
    ...
    procedure AddCondition(ScanSearchCondition: TACRScanSearchCondition);
...
////////////////////////////////////////////////////////////////////////////////
//
// TACRScanSearchConditionCache
//
////////////////////////////////////////////////////////////////////////////////


  TACRLocateCondition = packed record
    FieldsNamesCRC:       Cardinal;
    FieldCount:           Integer;
    CaseInsensitive:      Boolean;
    PartialKey:           Boolean;
    Conditions:           TACRList; // list o index conditions used in this locate
    ScanConditionNo:      Integer;
    ScanEndConditionNo:   Integer;
    FieldDefsNumbers:     TACRIntegerArray;// array of numbers in FFieldDefs used in this locate
    Params:               TObject; // TACRSQLParams
  end;
  PACRLocateCondition = ^TACRLocateCondition;


  TACRLastSearchOperation = (lsoNone,lsoLocate,lsoFindKey,lsoFilterRecord);

  TACRScanSearchConditionCache = class(TObject)
   private
    FConditions:              TACRList;
    FLocateConditions:        TACRList;
    FLastLocateCondition:     PACRLocateCondition;
    FFindKeySearchCondition:  TACRScanSearchCondition;
    FLastOperation:           TACRLastSearchOperation;
   ...
   public
    ...
    destructor Destroy; override;
    procedure Clear;
    function FindLocateCondition(crc: Cardinal; CaseInsensitive,PartialKey: Boolean): PACRLocateCondition;
    procedure AddLocateCondition(pLocateCondition: PACRLocateCondition);
    function CreateLocateCondition: PACRLocateCondition;
    procedure FreeLocateCondition(pLocateCondition: PACRLocateCondition);
    function GetFindKeySearchCondition: TACRScanSearchCondition;
    procedure AddFindKeySearchCondition(KeyCondition: TACRScanSearchCondition);
    procedure PrepareForFilter;
    procedure AddFilterCondition(Condition: TACRScanSearchCondition);
    procedure AddFilterExpression(Expression,IndexDefs: TObject);
   public
    property Conditions: TACRList read FConditions;
    property LocateConditions: TACRList read FLocateConditions;
    property FilterConditionCount: Integer read FFilterConditionCount;
    property LastOperation: TACRLastSearchOperation read FLastOperation;
    ...
    property LastLocateCondition: PACRLocateCondition read FLastLocateCondition;
  end; // TACRScanSearchConditionCache
{$ENDIF}


 // ORDER BY clause element
 TACRSortSpecification = record
...
//----------------------- Field Types ------------------------------------------
 ...
 PACRDate = ^TACRDate;
 TACRDate = Integer;  // Number of days from Christmas ( -5 883 516 .. 5 883 516 )

 PACRTime = ^TACRTime;
 TACRTime = Integer; // Number of milliseconds from day begin ( 0 .. 24*60*60*1000 )

 PACRDateTime = ^TACRDateTime;
 TACRDateTime = packed record  // Total 8 byte
   Date: TACRDate;  // Number of days from Christmas ( -5 883 516 .. 5 883 516 )
   Time: TACRTime; // Number of milliseconds from day begin ( 0 .. 24*60*60*1000 )
 end;//TACRDateTime                                                = 86 400 000
 TACRDatePart = (dpYEAR,dpQUARTER,dpMONTH,dpDAY,dpWEEK,dpHOUR,dpMINUTE,dpSECOND,dpMILLISECOND,dpUNDEFINED);

 PACRLogical = ^TACRLogical;
 //TACRLogical = ByteBool;
 TACRLogical = WordBool; //TDataset using WordBool

 PACRCurrency = ^TACRCurrency;
 //TACRCurrency = Currency;
 TACRCurrency = Double;  // Overflow fix


TACRPagesArray = TACRIntegerArray;

//------------------------------------------------------------------------------
// PageManager types
//------------------------------------------------------------------------------
  TACRSessionPageInfo = record
    SessionID:          TACRSessionID;
    DirtyPages:         TList;
    AddedPageNumbers:   TACRPagesArray;
    RemovedPageNumbers: TACRPagesArray;
  end;
...
////////////////////////////////////////////////////////////////////////////////
 ...
 TACRIntegerArray = class(TObject)
   public
     ...
     procedure Assign(v: TACRIntegerArray);
...
////////////////////////////////////////////////////////////////////////////////
 ...
 TACRInt64Array = class(TObject)
   public
     ...
     procedure Assign(v: TACRInt64Array);
...
////////////////////////////////////////////////////////////////////////////////
 ...
 TACRRecordIDArray = class(TObject)
   private
     FSorted:        Boolean;
     FInMemory:      Boolean;
     FTemporary:     Boolean;
     FMaxRecordID:   TACRRecordID;
   public
     Items:          array of TACRRecordID;
     ItemCount:      Integer;
     AllocBy:        Integer;
     deAllocBy:      Integer;
     MaxAllocBy:     Integer;
     ...
     procedure SetSize(newSize: Integer);
     procedure Realloc(NewItemCount, NewAllocItemCount: Integer);
     procedure Assign(v: TACRRecordIDArray);
     procedure Append(var value: TACRRecordID);
     procedure Insert(ItemNo: Integer; const value: TACRRecordID);
     procedure Delete(ItemNo: Integer);
    protected
     // return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
     function CompareRecordIDMemory(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
     // return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
     function CompareRecordIDTemporary(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
     // return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
     function CompareRecordIDDisk(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
    public
     // return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
     function CompareRecordID(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
    protected
     // return -1 if not found, otherwise return position in Items array [0..ItemCount-1]
     function FindRecordByIDSorted(const RecordID: TACRRecordID): Integer;
    public
     procedure Sort;
     // return -1 if not found, otherwise return position in Items array [0..ItemCount-1]
     function FindRecordByID(const RecordID: TACRRecordID): Integer;
    public
     property Sorted: Boolean read FSorted write FSorted;
     property InMemory: Boolean read FInMemory write FInMemory;
     property Temporary: Boolean read FTemporary write FTemporary;
     property MaxRecordID: TACRRecordID read FMaxRecordID write FMaxRecordID;
 end; // TACRRecordIDArray
...
////////////////////////////////////////////////////////////////////////////////
 ...
 PACRGetHashKeyFromHashFunction =
   function (
    const     HashValue: TACRRecordHashValue;     // Hash value
    const     Count:     TACRRecordHashValue = 0  // SetCount
            ): TACRRecordHashValue of object;
 // return count from key elemenent
 PACRGetCountFromHashKeyFunction =
   function (
    const     HashValue: TACRRecordHashValue      // Hash value
            ): TACRRecordHashValue of object;
 // return updated key element
 PACRSetCountToHashKeyFunction =
   function (
    const     HashValue: TACRRecordHashValue;     // Hash value
    const     Count:     TACRRecordHashValue = 0  // SetCount
            ): TACRRecordHashValue of object;
 PACRIsHashKeysEqual =
   function (
    const     HashValue: TACRRecordHashValue;     // Hash value
    const     HashKey:   TACRRecordHashValue     // Hash key value
            ): Boolean of object;
 TACRHashArray = class(TObject)
   private
...
// Data > 0 - Count+1 elements sotred with such key
    ...
    FKeyElementSize:   Integer; // size in bytes of key element:
                                // ACR_HASH_VALUE_SIZE + SizeOf(PChar); // 8
    FValueElementSize: Integer; // size in bytes of value element:
                                // FValueElementSize := ACR_HASH_VALUE_SIZE + FDataSize;
    ...
    FKeyMask:           TACRRecordHashValue;
    FReallocMemCount:   Integer;

    FGetHashKeyPtr:             PACRGetHashKeyFromHashFunction;
    FGetCountFromHashKeyPtr:    PACRGetCountFromHashKeyFunction;
    FSetCountToHashKeyPtr:      PACRSetCountToHashKeyFunction;
    FIsHashKeysEqualPtr:        PACRIsHashKeysEqual;
   protected
    ...
    function FindHashValue(
              ...
              const HashValue: TACRRecordHashValue
               ): Boolean;
    // debug
   ...
   public
    ...
    function Append(HashValue: TACRRecordHashValue; Data: PAnsiChar = nil): Boolean;
    // return true if hash value already exists
//    function IsValueExists(HashValue: TACRRecordHashValue): Boolean; virtual; abstract;
    // return pointer to Data associated with hash if hash value already exists
//    function GetData(HashValue: TACRRecordHashValue): PAnsiChar; virtual; abstract;
    property GetHashKeyFromHashFunction: PACRGetHashKeyFromHashFunction
              read FGetHashKeyPtr write FGetHashKeyPtr;
    property GetCountFromHashKeyPtr: PACRGetCountFromHashKeyFunction
      read FGetCountFromHashKeyPtr write FGetCountFromHashKeyPtr;
    property SetCountToHashKeyPtr: PACRSetCountToHashKeyFunction
              read FSetCountToHashKeyPtr write FSetCountToHashKeyPtr;
    property IsHashKeysEqualPtr: PACRIsHashKeysEqual read FIsHashKeysEqualPtr write FIsHashKeysEqualPtr;
...
////////////////////////////////////////////////////////////////////////////////
 ...
 TACRTableNameLength = Word;
 TACRSortedStringPtrArray = class(TList);
 TACRObjectNameArray = class (TObject)
  private
   FItemOffsets:   TACRIntegerArray; // offsets of each item from FBuffer
   FBuffer:        PAnsiChar;
  ...
  public
   ...
   destructor Destroy; override;
   procedure Clear;
   procedure Assign(Source: TACRObjectNameArray); 
...
////////////////////////////////////////////////////////////////////////////////
 ...
 TACRWideStringList = class (TPersistent)
  public
...
////////////////////////////////////////////////////////////////////////////////
 ...
 TACRBLOBDescriptor = packed record
 ...
 end; // TACRBLOBDescriptor - 32 bytes
 PACRBLOBDescriptor = ^TACRBLOBDescriptor;

 // blob descriptor that stores for each blob value
 TACRPartialBLOBDescriptor = packed record
  NumBlocks:             Integer; // number of blocks
  UncompressedSize:      Int64; // size of uncompressed data
 end; // TACRPartialBLOBDescriptor - 8 bytes
 PACRPartialBLOBDescriptor = ^TACRPartialBLOBDescriptor;

 // blob descriptor that stores for each blob value
 TACRPartialTemporaryBLOBDescriptor = packed record
  NumBlocks:             Integer; // number of blocks
  UncompressedSize:      Int64; // size of uncompressed data
  CompressedSize:        Int64; // size of compressed data
 end; // TACRPartialTemporaryBLOBDescriptor - 8 bytes
 PACRPartialTemporaryBLOBDescriptor = ^TACRPartialTemporaryBLOBDescriptor;


 TACRCompressedStreamBlockHeader = packed record
 ...
 TACRCompressedStreamBlockHeadersArray = class
    private
     AllocBy:             Int64;
     DeAllocBy:           Int64;
     MaxAllocBy:          Int64;
     AllocItemCount:      Int64;
    public
     Items:               array of TACRCompressedStreamBlockHeader;
    ...
    public
     ...
     destructor Destroy; override;
     procedure SetSize(NewSize: Int64);
     procedure AppendItem(Value: TACRCompressedStreamBlockHeader; Pos: Int64);
     function FindPosition(Pos: Int64) : Integer;
   end; // TACRCompressedStreamBlockHeadersArray
...
////////////////////////////////////////////////////////////////////////////////
  ...
  function ACRGetRecordCountByBufferSize(BufferSize, RecordSize: Integer): Integer;
  // return true if RecordID1 = RecordID2
  function ACRIsEqualRecordID(const RecordID1, RecordID2: TACRRecordID): Boolean;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  function ACRGetTickCountDiff(NewTime, OldTime: Cardinal): Cardinal;
  // clear string with private data (passwords, keys, etc.)
  procedure ACRClearString(var Value: AnsiString; EncryptedDBOnly: Boolean = False); overload;
  procedure ACRClearString(var Value: WideString; EncryptedDBOnly: Boolean = False); overload;
  function ACRGetLastTableOpertaion(operation: TACRLastTableOperation): WideString;
  // Add CRC of multiple objects to get unique overall CRC
  // so ADDCRC(A,B) <> ADD CRC(B,A) in most cases
  function ACRAddCRC(value1,value2: Cardinal; Order: Byte): Cardinal;
  // return true if string contains any Unicode characters
  function ACRIsUnicodeString(Value: WideString): Boolean;
  // return 0 if value1 = value2, 1 if value1 > value2, -1 if value1 < value2
  function ACRCompareTableInfo(const value1, value2: TACRTableInfo): Integer;
  procedure ACRSwapTableInfoElements(tablesInfo: TACRTableInfoArray; index1, index2: Integer);
  procedure ACRSortTableInfo(tablesInfo: TACRTableInfoArray; min,max: Integer);
  function ACRGetTableStateAsString(const State: TACRTableState): AnsiString;
  function ACRGetLockModeName(const LockType: TACRLockType): AnsiString;
  // return maximum time to wait for the lock before exception will be raised
  function ACRGetMaxWaitTime(LockParams: TACRLockParams): Cardinal;
  ...
  function ACRGetWaitLevel(const StartWaitTime, MaxWaitTime: Cardinal): Byte;
  procedure ACRParseFieldNames(FieldNames: WideString; FieldNamesList: TACRWideStringList);
  // return maximum day number (28,29,30,31)
  function ACRGetMaxDayOfMonth(month,year: Word): Word;

implementation

uses Math, TypInfo
     ,ACRExpressions, ACRCompression, ACRCriticalSection
     ,ACRVariant
     ,ACRMemory // last
;
...
// Create
//------------------------------------------------------------------------------
constructor TACRScanSearchCondition.Create;
begin
  KeyRecordBuffer := nil;
  IndexID := INVALID_OBJECT_ID;
  Expression := nil;
{$IFDEF RECORD_SEARCH_CACHE_IN_CURSOR}
  ParamIndexes := TACRIntegerArray.Create;
  KeyFieldCount := 0;
  OwnKeyBuffer := False;
  OwnExpression := False;
{$ENDIF}
...
// Destroy
//------------------------------------------------------------------------------
destructor TACRScanSearchCondition.Destroy;
begin
{$IFDEF RECORD_SEARCH_CACHE_IN_CURSOR}
  ParamIndexes.Free;
  ...
  if (OwnExpression) then
    ...
    begin
     TACRExpression(Expression).Free;
     Expression := nil;
    end;
{$ENDIF}
  inherited;
...
// Assign
//------------------------------------------------------------------------------
procedure TACRScanSearchCondition.Assign(ScanSearchCondition: TACRScanSearchCondition);
begin
...
// Create
//------------------------------------------------------------------------------
constructor TACRScanSearchConditionArray.Create;
begin
  Items := nil;
  Count := 0;
end;// Create
...
// Destroy
//------------------------------------------------------------------------------
destructor TACRScanSearchConditionArray.Destroy;
var
  i: Integer;
begin
...
// Delete
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionArray.Delete(ItemNo: Integer);
begin
 if (ItemNo < 0) or (ItemNo >= Count) then
  raise EACRException.Create(10413,ErrorLInvalidItemNumber);
 if (Items[ItemNo] = nil) then
  raise EACRException.Create(10414,ErrorLNilPointer);
 Items[ItemNo].Free; 
 if (ItemNo < Count - 1) then
  Move(Items[ItemNo+ 1 ],Items[ItemNo],
      (Count - ItemNo-1) * SizeOf(TACRScanSearchCondition));
 Dec(Count);
...
// AddExpression
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionArray.AddExpression(Expression: Pointer);
begin
  SetLength(Items, Count+1);
  Items[Count] := TACRScanSearchCondition.Create;
...
// AddCondition
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionArray.AddCondition(
                  ScanSearchCondition:  TACRScanSearchCondition
                             );
begin
  if ((ScanSearchCondition.Expression = nil) and
      (ScanSearchCondition.IndexID = INVALID_ID4)) then
   raise EACRException.Create(10388,ErrorLInvalidScanConditionNoIndex,
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionArray.ExtractConditionsFromExpressions(
                                     IndexDefs:               TObject;
...
begin
  for i := 0 to Count-1 do
   if (Items[i].Expression <> nil) then
    begin
      TACRExpression(Items[i].Expression).ExtractIndexScanConditions(
          IndexDefs,
          Self,
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionArray.ReturnConditionsToExpressions(ExtractedConditionsInfo: TList);
var
  i: Integer;
begin
  for i := 0 to ExtractedConditionsInfo.Count-1 do
   begin
    if (PACRExtractedConditionInfo(ExtractedConditionsInfo[i])^.Expression = nil) then
     raise EACRException.Create(20060, ErrorANilPointer);
    if (PACRExtractedConditionInfo(ExtractedConditionsInfo[i])^.
          ExtractedExpressionNode = nil) then
     raise EACRException.Create(20061, ErrorANilPointer);
    if (PACRExtractedConditionInfo(ExtractedConditionsInfo[i])^.KeyRecordBuffer = nil) then
     raise EACRException.Create(20062, ErrorANilPointer);

    TACRExpression(PACRExtractedConditionInfo(ExtractedConditionsInfo[i])^.
        Expression).AddNode(
        PACRExtractedConditionInfo(ExtractedConditionsInfo[i])^.
...
// create
//------------------------------------------------------------------------------
constructor TACRScanSearchConditionCache.Create;
begin
  FConditions := TACRList.Create;
  FLocateConditions := TACRList.Create;
...
// destroy
//------------------------------------------------------------------------------
destructor TACRScanSearchConditionCache.Destroy;
begin
  Clear;
  FConditions.Free;
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionCache.Clear;
var i,j:              Integer;
...
begin
  ...
  begin
    pLocateCondition := PACRLocateCondition(FLocateConditions.Items[i]);
  ...
  begin
    // filter conditions
    SearchCondition := TACRScanSearchCondition(FConditions.Items[i]);
...
//------------------------------------------------------------------------------
function TACRScanSearchConditionCache.FindLocateCondition(crc: Cardinal; CaseInsensitive,PartialKey: Boolean): PACRLocateCondition;
var i,j:  Integer;
begin
  ...
  for i := 0 to FLocateConditions.Count-1 do
  begin
    Result := PACRLocateCondition(FLocateConditions.Items[i]);
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionCache.AddLocateCondition(pLocateCondition: PACRLocateCondition);
var i:      Integer;
begin
...
//------------------------------------------------------------------------------
function TACRScanSearchConditionCache.CreateLocateCondition: PACRLocateCondition;
begin
  new(Result);
  Result^.Params := TACRSQLParams.Create;
  Result^.Conditions := TACRList.Create;
  Result^.FieldDefsNumbers := TACRIntegerArray.Create;
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionCache.FreeLocateCondition(pLocateCondition: PACRLocateCondition);
var i:      Integer;
begin
  // add search conditions
  for i := 0 to pLocateCondition^.Conditions.Count-1 do
  begin
    TACRScanSearchCondition(pLocateCondition^.Conditions.Items[i]).Free;
  end;
...
//------------------------------------------------------------------------------
function TACRScanSearchConditionCache.GetFindKeySearchCondition: TACRScanSearchCondition;
begin
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionCache.AddFindKeySearchCondition(KeyCondition: TACRScanSearchCondition);
begin
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionCache.PrepareForFilter;
begin
  FConditions.Clear;
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionCache.AddFilterCondition(Condition: TACRScanSearchCondition);
begin
...
//------------------------------------------------------------------------------
procedure TACRScanSearchConditionCache.AddFilterExpression(Expression,IndexDefs: TObject);
var expr:        TACRExpression;
    cond:        TACRScanSearchCondition;
begin
  expr := TACRExpression(Expression);
  ...
  begin
    cond := TACRScanSearchCondition.Create;
...
//------------------------------------------------------------------------------
constructor TACRIntegerArray.Create(
  size:           Integer;
...
begin
 ...
 AllocItemCount := 0;
 SetSize(size);
end;//TACRIntegerArray.Create


//------------------------------------------------------------------------------
// Destruct array (free mem)
//------------------------------------------------------------------------------
destructor TACRIntegerArray.Destroy;
begin
 Items := nil;
 inherited Destroy;
end;//TACRIntegerArray.Destroy;


//------------------------------------------------------------------------------
// Set length of array to specified size
//------------------------------------------------------------------------------
procedure TACRIntegerArray.Assign(v: TACRIntegerArray);
var
  i: Integer;
begin
  SetSize(v.ItemCount);
...
// Set length of array to specified size
//------------------------------------------------------------------------------
procedure TACRIntegerArray.SetSize(newSize: Integer);
begin
 if (newSize = 0) then
  begin
   ItemCount := 0;
 ...
 ItemCount := newSize;
end;//TACRIntegerArray.SetSize


//------------------------------------------------------------------------------
// inserts an element to the end of items array
//------------------------------------------------------------------------------
procedure TACRIntegerArray.Append(value: Integer);
begin
 Inc(ItemCount);
 SetSize(itemCount);
 Items[itemCount-1] := value;
end;//TACRIntegerArray.Append


//------------------------------------------------------------------------------
// Remove first item = value
//------------------------------------------------------------------------------
procedure TACRIntegerArray.Remove(value: Integer);
var
  j: Integer;
begin
 j := IndexOf(value);
 if j > -1 then
   Delete(j);
end;//TACRIntegerArray.Remove


//------------------------------------------------------------------------------
// Insert an element into specified position
//------------------------------------------------------------------------------
procedure TACRIntegerArray.Insert(itemNo: Integer; value: Integer);
begin
 inc(ItemCount);
 SetSize(ItemCount);
 if (itemCount <= 1) then
...
end;//TACRIntegerArray.Insert


//------------------------------------------------------------------------------
// Delete an element at specified position
//------------------------------------------------------------------------------
procedure TACRIntegerArray.Delete(itemNo: Integer);
begin
 ...
 dec(ItemCount);
 SetSize(ItemCount);
end;//TACRIntegerArray.Delete


//------------------------------------------------------------------------------
// moves element to new position
//------------------------------------------------------------------------------
procedure TACRIntegerArray.MoveTo(itemNo, newItemNo: Integer);
var value : Integer;
begin
...
//------------------------------------------------------------------------------
procedure TACRIntegerArray.CopyTo(
                      var ar: array of Integer;
                      itemNo, iCount: Integer
                             );
begin
...
//------------------------------------------------------------------------------
function TACRIntegerArray.IsValueExists(value: Integer): Boolean;
begin
...
//------------------------------------------------------------------------------
function TACRIntegerArray.IndexOf(value: Integer): Integer;
var i: Integer;
begin
 Result := -1;
 for i := 0 to ItemCount-1 do
...
//------------------------------------------------------------------------------
function TACRIntegerArray.Add(value: Integer): Boolean;
begin
...
// Construct array of specified size
//------------------------------------------------------------------------------
constructor TACRInt64Array.Create(
  size: Int64;
  DefaultAllocBy: Integer;
  MaximumAllocBy: Integer
  );
begin
 ...
 AllocItemCount := 0;
 SetSize(size);
end;//TACRInt64Array.Create


//------------------------------------------------------------------------------
// Destruct array (free mem)
//------------------------------------------------------------------------------
destructor TACRInt64Array.Destroy;
begin
 Items := nil;
 inherited Destroy;
end;//TACRInt64Array.Destroy;


//------------------------------------------------------------------------------
// Set length of array to specified size
//------------------------------------------------------------------------------
procedure TACRInt64Array.Assign(v: TACRInt64Array);
var
  i: Int64;
begin
  SetSize(v.ItemCount);
...
// Set length of array to specified size
//------------------------------------------------------------------------------
procedure TACRInt64Array.SetSize(newSize: Int64);
begin
 if (newSize = 0) then
  begin
   ItemCount := 0;
 ...
 ItemCount := newSize;
end;//TACRInt64Array.SetSize


//------------------------------------------------------------------------------
// reallocates the array
//------------------------------------------------------------------------------
procedure TACRInt64Array.Realloc(NewItemCount, NewAllocItemCount: Integer);
begin
...
// inserts an element to the end of items array
//------------------------------------------------------------------------------
procedure TACRInt64Array.Append(value: Int64);
begin
 Inc(ItemCount);
 SetSize(itemCount);
 Items[itemCount-1] := value;
end;//TACRInt64Array.Append


//------------------------------------------------------------------------------
// Remove first item = value
//------------------------------------------------------------------------------
procedure TACRInt64Array.Remove(value: Int64);
var
   j: Int64;
begin
 j := IndexOf(value);
 if j > -1 then
   Delete(j);
end;//TACRInt64Array.Remove


//------------------------------------------------------------------------------
// Insert an element into specified position
//------------------------------------------------------------------------------
procedure TACRInt64Array.Insert(itemNo: Int64; value: Int64);
begin
 inc(ItemCount);
 SetSize(ItemCount);
 if (itemCount <= 1) then
...
end;//TACRInt64Array.Insert


//------------------------------------------------------------------------------
// Delete an element at specified position
//------------------------------------------------------------------------------
procedure TACRInt64Array.Delete(itemNo: Int64);
begin
 if (itemNo < itemCount-1) then
  Move(items[itemNo+1],items[itemNo],
      (itemCount - itemNo-1) * sizeOf(Int64));
 dec(ItemCount);
 SetSize(ItemCount);
end;//TACRInt64Array.Delete


//------------------------------------------------------------------------------
// moves element to new position
//------------------------------------------------------------------------------
procedure TACRInt64Array.MoveTo(itemNo, newItemNo: Int64);
var value : Int64;
begin
 if (itemNo = newItemNo) then
  Exit;
...
// copies itemCount elements to ar from ItmeNo
//------------------------------------------------------------------------------
procedure TACRInt64Array.CopyTo(
                      var ar: array of Int64;
                      itemNo, iCount: Int64
                             );
begin
...
// returns true if value exists in Items array
//------------------------------------------------------------------------------
function TACRInt64Array.IsValueExists(value: Int64): Boolean;
begin
 Result := (IndexOf(value) >= 0);
end; // IsValueExists
...
// returns index in Items or -1 if not found
//------------------------------------------------------------------------------
function TACRInt64Array.IndexOf(value: Int64): Int64;
var i: Int64;
begin
 Result := -1;
 i := 0;
...
// adds value if it is not existing, otherwise returns false
//------------------------------------------------------------------------------
function TACRInt64Array.Add(value: Int64): Boolean;
begin
 Result := not (IsValueExists(value));
 if Result then
   Append(value);
...
// get item
//------------------------------------------------------------------------------
function TACRList.GetItem(Index: Integer): Pointer;
begin
  if ((Index < FCount) and (Index >= 0)) then
    Result := FItems[Index]
  else
   begin
    raise EACRException.Create(11611,ErrorLInvalidIndex,[Index,FCount]);
   end;
end; // GetItem
...
// set item
//------------------------------------------------------------------------------
procedure TACRList.SetItem(Index: Integer; Item: Pointer);
begin
  if ((Index < FCount) and (Index >= 0)) then
    FItems[Index] := Item
  else
   begin
//    FCount := FCount;
    raise EACRException.Create(11610,ErrorLInvalidIndex,[Index,FCount]);
   end;
end; // SetItem
...
// set count
//------------------------------------------------------------------------------
procedure TACRList.SetCount(value: Integer);
begin
 if (value = 0) then
  SetCapacity(0)
 else
...
// set capacity
//------------------------------------------------------------------------------
procedure TACRList.SetCapacity(value: Integer);
begin
 if (value < 0) then
  raise EACRException.Create(11613,ErrorLInvalidCapacity,[value]);
 if (value = 0) then
  begin
   FCount := 0;
   FCapacity := 0;
...
// IndexOf
//------------------------------------------------------------------------------
function TACRList.IndexOf(Item: Pointer): Integer;
var i: Integer;
begin
  Result := -1;
...
// add
//------------------------------------------------------------------------------
procedure TACRList.Add(Item: Pointer);
begin
...
// remove
//------------------------------------------------------------------------------
procedure TACRList.Remove(Item: Pointer);
var Index: Integer;
begin
  Index := IndexOf(Item);
  if (Index >= 0) then
...
// delete
//------------------------------------------------------------------------------
procedure TACRList.Delete(Index: Integer);
begin
  if ((Index < 0) or (Index >= FCount)) then
   begin
    raise EACRException.Create(11612,ErrorLInvalidIndex,[Index,FCount]);
   end;
  if (FCount = 1) then
   SetCapacity(0)
  else
...
// Construct array of specified size
//------------------------------------------------------------------------------
constructor TACRRecordIDArray.Create(
  Size:           Integer;
  DefaultAllocBy: Integer;
  MaximumAllocBy: Integer
  );
begin
 ...
 SetSize(Size);
 FSorted := True;
end;//TACRRecordIDArray.Create


//------------------------------------------------------------------------------
// Destruct array (free mem)
//------------------------------------------------------------------------------
destructor TACRRecordIDArray.Destroy;
begin
 Items := nil;
 inherited Destroy;
end;//TACRRecordIDArray.Destroy;


//------------------------------------------------------------------------------
// Set length of array to specified size
//------------------------------------------------------------------------------
procedure TACRRecordIDArray.Assign(v: TACRRecordIDArray);
var
  i: Integer;
begin
  SetSize(v.ItemCount);
...
// Set length of array to specified size
//------------------------------------------------------------------------------
procedure TACRRecordIDArray.SetSize(newSize: Integer);
begin
 if (newSize = 0) then
  begin
   ItemCount := 0;
 ...
 ItemCount := newSize;
end;//TACRRecordIDArray.SetSize


//------------------------------------------------------------------------------
// reallocates the array
//------------------------------------------------------------------------------
procedure TACRRecordIDArray.Realloc(NewItemCount, NewAllocItemCount: Integer);
begin
  AllocItemCount := NewAllocItemCount;
  SetLength(Items,AllocItemCount);
  if (AllocItemCount > MaxAllocBy) then
...
// inserts an element to the end of items array
//------------------------------------------------------------------------------
procedure TACRRecordIDArray.Append(var value: TACRRecordID);
begin
 Inc(ItemCount);
 SetSize(itemCount);
 Items[itemCount-1] := value;
end;//TACRRecordIDArray.Append


//------------------------------------------------------------------------------
// Insert an element into specified position
//------------------------------------------------------------------------------
procedure TACRRecordIDArray.Insert(itemNo: Integer; const value: TACRRecordID);
begin
 inc(ItemCount);
 SetSize(ItemCount);
 if (itemCount <= 1) then
 ...
 if (itemNo >= itemCount-1)
  then
   ...
   begin
    Move(items[itemNo],items[itemNo+1],
        (itemCount - itemNo-1) * sizeOf(TACRRecordID));
    items[itemNo] := value;
   end;
end;//TACRRecordIDArray.Insert


//------------------------------------------------------------------------------
// Delete an element at specified position
//------------------------------------------------------------------------------
procedure TACRRecordIDArray.Delete(itemNo: Integer);
begin
 if (itemNo < itemCount-1) then
  Move(items[itemNo+1],items[itemNo],
      (itemCount - itemNo-1) * sizeOf(TACRRecordID));
 dec(ItemCount);
 SetSize(ItemCount);
end;//TACRRecordIDArray.Delete


//------------------------------------------------------------------------------
// return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
//------------------------------------------------------------------------------
function TACRRecordIDArray.CompareRecordIDMemory(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
begin
...
// return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
//------------------------------------------------------------------------------
function TACRRecordIDArray.CompareRecordIDTemporary(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
...
// return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
//------------------------------------------------------------------------------
function TACRRecordIDArray.CompareRecordIDDisk(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
begin
...
// return 0 if id1 = id2, 1 if id1>id2, -1 if id1<id2
//------------------------------------------------------------------------------
function TACRRecordIDArray.CompareRecordID(const RecordID1: TACRRecordID; const RecordID2: TACRRecordID): Integer;
begin
  if (FInMemory) then
...
// return -1 if not found, otherwise return position in Items array [0..ItemCount-1]
//------------------------------------------------------------------------------
function TACRRecordIDArray.FindRecordByIDSorted(const RecordID: TACRRecordID): Integer;
var
...
//------------------------------------------------------------------------------
procedure TACRRecordIDArray.Sort;
var
 aLo, aHi : Integer;
  ...
  begin
    ...
    if (Hi > iLo) then
     begin
      ...
      if (iHi = Hi) then
       raise EACRException.Create(11329,ErrorLErrorSoringRecordsByID,[Hi,Lo,iHi,iLo,ItemCount]);
      QuickSort(iLo, Hi);
     end;
    if (Lo < iHi) then
      QuickSort(Lo, iHi);
...
// return -1 if not found, otherwise return position in Items array [0..ItemCount-1]
//------------------------------------------------------------------------------
function TACRRecordIDArray.FindRecordByID(const RecordID: TACRRecordID): Integer;
var i: Integer;
begin
  Result := -1;
...
//------------------------------------------------------------------------------
function TACRHashArray.GetKeyElementFromHash(
  const     HashValue: TACRRecordHashValue;     // Hash value
  const     Count:     TACRRecordHashValue = 0  // SetCount
          ): TACRRecordHashValue;
begin
  if (Count > FMaxElementCount) then
   raise EACRException.Create(11663,ErrorLInvalidCount,[Count,FMaxKeyCount]);
  if (@FGetHashKeyPtr <> nil) then
   Result := FGetHashKeyPtr(HashValue,Count)
  else
   begin
    // result - XXYY, where XX - key bits, YY - count bits
    Result := (HashValue and FKeyMask) shl (ACR_MAX_HASH_KEY_SIZE - FKeySize - FKeyStartBitNo)
              or Count;
   end;
end; // GetKeyFromHash
...
//------------------------------------------------------------------------------
function TACRHashArray.GetCountFromKeyElement(
      const     HashValue: TACRRecordHashValue      // Hash value
              ): TACRRecordHashValue;
begin
...
//------------------------------------------------------------------------------
function TACRHashArray.SetCountToKeyElement(
          const     HashValue: TACRRecordHashValue;     // Hash value
          const     Count:     TACRRecordHashValue = 0  // SetCount
        ): TACRRecordHashValue;
begin
  if (Count > FMaxElementCount) then
   raise EACRException.Create(11664,ErrorLInvalidCount,[Count,FMaxKeyCount]);
  if (@FSetCountToHashKeyPtr <> nil) then
   ...
   begin
    ...
    Result := (HashValue and ($FFFFFFFF shl (ACR_MAX_HASH_KEY_SIZE - FKeySize)))
              or Count;
   end;
...
//------------------------------------------------------------------------------
function TACRHashArray.IsHashKeysEqual(
          const     HashValue: TACRRecordHashValue;     // Hash value
          const     HashKey:   TACRRecordHashValue     // Hash key value
        ): Boolean;
begin
...
//------------------------------------------------------------------------------
procedure TACRHashArray.Init;
var i:            Cardinal;
    keyPtr, buf:  PAnsiChar;
    key:          TACRRecordHashValue;
begin
 if (FKeyStartBitNo = 0) then
  FKeyMask := ($FFFFFFFF shl FKeySize)
 else
 if (FKeyStartBitNo = (ACR_MAX_HASH_KEY_SIZE-1)) then
 ...
 while (i <= FMaxKeyCount) do
   begin
    ...
    key := (i shl (ACR_MAX_HASH_KEY_SIZE - FKeySize));
    Move(key,keyPtr^,ACR_HASH_VALUE_SIZE);
    Inc(i);
...
//------------------------------------------------------------------------------
function TACRHashArray.FindHashValue(
              const buf: PAnsiChar;
              const numElements: Cardinal;
              const HashValue: TACRRecordHashValue
               ): Boolean;
...
begin
 ...
 while (i < numElements) do
  begin
   if (PACRRecordHashValue(ptr)^ = HashValue) then
    begin
     Result := True;
     Exit;
    end;
...
// debug
//------------------------------------------------------------------------------
procedure TACRHashArray.WriteStats;
begin
{$IFDEF DEBUG_LOG_HASH_ARRAY}
  aaWriteToLog(#13#10+'TACRHashArray.Destroy: Total memory used = '+#13#10+IntToStr(FTotalMemAllocated)
+#13#10+'TACRHashArray.Destroy: Key memory used = '+#13#10+IntToStr(FKeyMemAllocated)
+#13#10+'TACRHashArray.Destroy: Data memory used = '+#13#10+IntToStr(FDataMemAllocated)
+#13#10+'TACRHashArray.Destroy: ReallocMem count = '+#13#10+IntToStr(FReallocMemCount)
+#13#10+'TACRHashArray.Destroy: Key element count = '+#13#10+IntToStr(FKeyElementCount)
+#13#10+'TACRHashArray.Destroy: Value element count = '+#13#10+IntToStr(FValueElementCount)
+#13#10+'TACRHashArray.Destroy: Maximum key count = '+#13#10+IntToStr(FMaxKeyCount));
{$ENDIF}
end; // WriteStats
...
// create
//------------------------------------------------------------------------------
constructor TACRHashArray.Create(
                  DataSize:       Integer;
                  AllocBy:        Integer;
...
begin
  if ((KeySize <= 0) or (KeySize >= ACR_MAX_HASH_KEY_SIZE)) then
   raise EACRException.Create(11662,ErrorLInvalidKeySize,[KeySize,ACR_MAX_HASH_KEY_SIZE]);
  FDataSize := DataSize;
  FAllocBy := AllocBy;
  ...
  FKeyElementSize := ACR_HASH_VALUE_SIZE + ACR_POINTER_SIZE; // 8
  FValueElementSize := ACR_HASH_VALUE_SIZE + FDataSize;
  FPageSize := FAllocBy * FValueElementSize;
  FMaxKeyCount := ((1 shl FKeySize) - 1);
  FMaxElementCount := (1 shl (ACR_MAX_HASH_KEY_SIZE - FKeySize));
...
// Destroy
//------------------------------------------------------------------------------
destructor TACRHashArray.Destroy;
...
{$ENDIF}
  ...
  keyPtr := PAnsiChar(FKeyData+ACR_HASH_VALUE_SIZE);
  i := 0;
  try
    while (i <= FMaxKeyCount) do
     begin
      Move(keyPtr^,buf,ACR_POINTER_SIZE);
      if (buf <> nil) then
...
// returns false if hash value already exists
//------------------------------------------------------------------------------
function TACRHashArray.Append(HashValue: TACRRecordHashValue; Data: PAnsiChar = nil): Boolean;
var bufSize:  Cardinal;
    offset:   Cardinal;
    keyPtr:   PAnsiChar;
    buf:      PAnsiChar;
    newKey:   TACRRecordHashValue;
    key:      TACRRecordHashValue;
    count:    TACRRecordHashValue;
begin
...
{$ENDIF}
  ...
  keyPtr := FKeyData +
           ((newKey shr (ACR_MAX_HASH_KEY_SIZE -  FKeySize)) * FKeyElementSize);
  Move(PAnsiChar(KeyPtr+ACR_HASH_VALUE_SIZE)^,buf,ACR_POINTER_SIZE);
  // empty data
  if (Integer(buf) = 0) then
   begin
    ...
    Move(newKey,keyPtr^,ACR_HASH_VALUE_SIZE);
    ...
    Move(buf,PAnsiChar(KeyPtr+ACR_HASH_VALUE_SIZE)^,ACR_POINTER_SIZE);
    // move hash value to first value element
    Move(HashValue,buf^,ACR_HASH_VALUE_SIZE);
    if (Data <> nil) then
     begin
      if (FDataSize <= 0) then
       raise EACRException.Create(11665,ErrorLInvalidDataSizeSimple,[FDataSize]);
      Move(Data^,PAnsiChar(buf+ACR_HASH_VALUE_SIZE)^,FDataSize);
     end;
   ...
   begin
    // get key element
    Move(KeyPtr^,key,ACR_HASH_VALUE_SIZE);
    ...
    if (Result) then
     begin
      ...
      Move(HashValue,PAnsiChar(buf+offset)^,ACR_HASH_VALUE_SIZE);
      if (Data <> nil) then
       begin
        if (FDataSize <= 0) then
         raise EACRException.Create(11665,ErrorLInvalidDataSizeSimple,[FDataSize]);
        Inc(offset,ACR_HASH_VALUE_SIZE);
...
//------------------------------------------------------------------------------
function TACRObjectNameArray.GetItemCount: Integer;
begin
...
// set wide string
//------------------------------------------------------------------------------
procedure TACRObjectNameArray.SetString(ItemIndex: Integer; Name: WideString);
...
// return Ansi string
//------------------------------------------------------------------------------
function TACRObjectNameArray.GetString(ItemIndex: Integer): WideString;
...
// set size
//------------------------------------------------------------------------------
procedure TACRObjectNameArray.InternalSetSize(NewSize: Integer);
...
begin
  ...
  if (NewSize > FAllocatedSize) then
   begin
{$IFDEF DEBUG_TRACE_TACRObjectNameArray_Add}
aaStopTime(time2);
...
// update offsets
//------------------------------------------------------------------------------
procedure TACRObjectNameArray.UpdateOffsets(StartItemIndex,Delta: Integer);
var i: Integer;
begin
...
// create
//------------------------------------------------------------------------------
constructor TACRObjectNameArray.Create;
begin
  Clear;
  FItemOffsets := TACRIntegerArray.Create;
end; // Create
...
// destroy
//------------------------------------------------------------------------------
destructor TACRObjectNameArray.Destroy;
begin
...
// clear
//------------------------------------------------------------------------------
procedure TACRObjectNameArray.Clear;
begin
  if (FBuffer <> nil) then
   MemoryManager.FreeAndNilMem(FBuffer);
...
// assign
//------------------------------------------------------------------------------
procedure TACRObjectNameArray.Assign(Source: TACRObjectNameArray);
begin
  if (Source is TACRObjectNameArray) then
  begin
...
// add element and return its number
//------------------------------------------------------------------------------
function TACRObjectNameArray.Add(const Name: WideString): Integer;
...
// delete element
//------------------------------------------------------------------------------
procedure TACRObjectNameArray.Delete(ItemIndex: Integer);
...
// save to stream
//------------------------------------------------------------------------------
function TACRObjectNameArray.SaveToStream(Stream: TStream; CountSize: Boolean = False): Integer;
begin
  if (CountSize) then
...
// load from stream
//------------------------------------------------------------------------------
procedure TACRObjectNameArray.LoadFromStream(Stream: TStream);
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.SetSize(NewSize: Integer);
var i: Integer;
begin
 if (NewSize = 0) then
  begin
   for i := 0 to FItemCount-1 do
    ACRClearString(FItems[i],True);
   FItemCount := 0;
   FAllocItemCount := 0;
   FItems := nil;
   Exit;
...
// get string
//------------------------------------------------------------------------------
function TACRWideStringList.InternalGetStrings(Index: Integer): WideString;
begin
...
// set string
//------------------------------------------------------------------------------
procedure TACRWideStringList.InternalSetStrings(Index: Integer; const Value: WideString);
begin
 if ((Index >= 0) and (Index < FitemCount)) then
  begin
   ACRClearString(Fitems[Index],True);
   FItems[Index] := Value;
   if (not FUnicode) then
    FUnicode := ACRIsUnicodeString(Value);
  end;
end; // SetStrings
...
// get text
//------------------------------------------------------------------------------
function TACRWideStringList.InternalGetText: WideString;
var
    i, tSize, l: Integer;
    p:           PWideChar;
    s:           WideString;
...
//aaWriteToLog('< TACRWideStringList.InternalGetText, Result = '+Result);
end; // InternalGetText
...
// set text
//------------------------------------------------------------------------------
procedure TACRWideStringList.InternalSetText(const Value: WideString);
var
    i, l, len:  Integer;
    p,ps,pd:    PWideChar;
    s:          WideString;
begin
//aaWriteToLog('> TACRWideStringList.InternalSetText, Value = '+Value);
  SetSize(0);
  if (Length(Value) <= 0) then
   Exit;
  p := Pointer(Value);
  if (p = nil) then
    raise EACRException.Create(11948,ErrorLNilPointer);
  len := Length(Value);
  i := 0;
  while (i < len) do
   begin
...
// get string
//------------------------------------------------------------------------------
function TACRWideStringList.GetStrings(Index: Integer): WideString;
begin
 Lock(False);
 try
...
// set string
//------------------------------------------------------------------------------
procedure TACRWideStringList.SetStrings(Index: Integer; const Value: WideString);
begin
 try
   Lock(True);
   try
...
// get text
//------------------------------------------------------------------------------
function TACRWideStringList.GetText: WideString;
begin
 Lock(False);
 try
...
// set text
//------------------------------------------------------------------------------
procedure TACRWideStringList.SetText(const Value: WideString);
begin
 try
   Lock(True);
   try
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.DoOnChange;
begin
...
// internal add
//------------------------------------------------------------------------------
function TACRWideStringList.InternalAdd(const Value: WideString): Integer;
begin
//aaWriteToLog('TACRWideStringList.InternalAdd, Value = '+Value);
  Result := FItemCount;
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.InternalDelete(Index: Integer);
...
begin
  ...
  begin
    if (FUnicode) then
      if (ACRIsUnicodeString(FItems[Index])) then
        bCheckUnicode := True;
    ACRClearString(FItems[Index],True);
  ...
  FUnicode := ACRIsUnicodeString(GetText);
end; // InternalDelete
...
//------------------------------------------------------------------------------
function TACRWideStringList.InternalIndexOf(const Value: WideString): Integer;
var i: Integer;
{$IFDEF DEBUG_LOG}
{DEFINE DEBUG_TRACE_TACRWideStringList_InternalIndexOf}
{$ENDIF}
begin
{$IFDEF DEBUG_TRACE_TACRWideStringList_InternalIndexOf}
aaWriteToLog('> TACRWideStringList.InternalIndexOf Value = '+Value+#13#10+'FItemCount = '+IntToStr(FItemCount));
try
{$ENDIF}
  Result := -1;
  for i := 0 to FItemCount-1 do
   begin
{$IFDEF DEBUG_TRACE_TACRWideStringList_InternalIndexOf}
aaWriteToLog('1. TACRWideStringList.InternalIndexOf Value = '+Value+#13#10+'i = '+IntToStr(i));
...
{$IFDEF DEBUG_TRACE_TACRWideStringList_InternalIndexOf}
aaWriteToLog('< TACRWideStringList.InternalIndexOf Value = '+Value+#13#10+'Result = '+IntToStr(Result));
except
 on e: Exception do
  begin
    aaWriteToLog('Error in TACRWideStringList.InternalIndexOf Value = '+Value+#13#10+e.Message);
    raise;
  end;
end;
{$ENDIF}
...
// quick sort
//------------------------------------------------------------------------------
procedure TACRWideStringList.DoQuickSort(min,max: Integer);
...
begin
  ...
  repeat
    ...
    p := (min + max) shr 1;
    repeat
      while (ACRCompareWideString(FItems[i],FItems[p]) < 0) do
        Inc(i);
      while (ACRCompareWideString(FItems[j],FItems[p]) > 0) do
        Dec(j);
      if (i <= j) then
       begin
        if (i <> j) then
    ...
    if (min < j) then
     begin
      // check infinite recurse
      if (j = max) then
       raise EACRException.Create(12130,ErrorLErrorSortingWideStringList,[i,j,min,max,FItemCount]);
      DoQuickSort(min,j);
     end;
    min := i;
  until (i >= max);
...
// Create
//------------------------------------------------------------------------------
constructor TACRWideStringList.Create(
                     Size:            Integer = 0;
                     DefaultAllocBy:  Integer = 10;
                     MaximumAllocBy:  Integer = 100
                   );
...
// Destroy
//------------------------------------------------------------------------------
destructor TACRWideStringList.Destroy;
begin
  SetSize(0);
  FOnChange := nil;
  FThreadSync.Free;
...
// Clear
//------------------------------------------------------------------------------
procedure TACRWideStringList.Clear;
begin
 try
   Lock(True);
   try
...
// assign
//------------------------------------------------------------------------------
procedure TACRWideStringList.Assign(Source: TPersistent);
var i: Integer;
begin
  try
   Lock(True);
   try
     if (Source = nil) then
       raise EACRException.Create(11947,ErrorLNilPointer);
     if (Source is TACRWideStringList) then
      begin
        if (FItemCount > 0) then
         SetSize(0);
        SetSize(TACRWideStringList(Source).Count);
        for i := 0 to FItemCount-1 do
         FItems[i] := TACRWideStringList(Source).Strings[i];
      end
     else
      ...
      raise EACRException.Create(11946,ErrorLInvalidSourceObject,[IntToHex(Integer(Source),8),Source.ClassName]);
   finally
     Unlock;
   end;
 finally
...
// add new string
//------------------------------------------------------------------------------
function TACRWideStringList.Add(const Value: WideString; const bAddIfNotExists: Boolean): Integer;
{$IFDEF DEBUG_LOG}
{DEFINE DEBUG_TRACE_TACRWideStringList_Add}
{$ENDIF}
begin
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('> TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True));
try
{$ENDIF}
  Result := -1;
  try
   ...
   try
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('1. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True));
{$ENDIF}
    if (bAddIfNotExists) then
     begin
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('2. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True));
{$ENDIF}
      if (InternalIndexOf(Value) < 0) then
       begin
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('2.1. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True));
{$ENDIF}
        Result := InternalAdd(Value);
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('3. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
{$ENDIF}
       end;
     end
    else
     begin
      Result := InternalAdd(Value);
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('4. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
{$ENDIF}
     end;
    if (not FUnicode) then
     begin
       FUnicode := ACRIsUnicodeString(Value);
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('5. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
{$ENDIF}
     end;
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('6. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
{$ENDIF}
   finally
    Unlock;
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('7. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
{$ENDIF}
   end;
 finally
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('8. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
{$ENDIF}
  DoOnChange;
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('9. TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
{$ENDIF}
 end;
{$IFDEF DEBUG_TRACE_TACRWideStringList_Add}
aaWriteToLog('< TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+'Result = '+IntToStr(Result));
except
 on e: Exception do
  begin
   aaWriteToLog('Error in  TACRWideStringList.Add. Value = '+Value+#13#10+'bAddIfNotExists = '+BoolToStr(bAddIfNotExists,True)+#13#10+e.Message);
   raise;
  end;
end;
{$ENDIF}
...
// delete string
//------------------------------------------------------------------------------
procedure TACRWideStringList.Delete(Index: Integer);
begin
  try
   Lock(True);
   try
...
// deletes first occurrence of the Value
//------------------------------------------------------------------------------
procedure TACRWideStringList.Remove(const Value: WideString);
var idx: Integer;
begin
 Lock(True);
 try
...
//------------------------------------------------------------------------------
function TACRWideStringList.IndexOf(const Value: WideString): Integer;
begin
 Lock(False);
 try
...
//------------------------------------------------------------------------------
{$IFDEF D6H}
procedure TACRWideStringList.LoadFromFile(const FileName: WideString);
{$ELSE}
procedure TACRWideStringList.LoadFromFile(const FileName: WideString; Dummy: ByteBool);
{$ENDIF}
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.LoadFromFile(const FileName: AnsiString);
var fs: TACRFileStream;
begin
  fs := TACRFileStream.Create(FileName,fmOpenRead or fmShareDenyWrite);
  try
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.LoadFromStream(Stream: TStream);
var newSize:  Integer;
    s:        WideString;
    ansiS:    AnsiString;
    buf:      Pointer;
    ...
    try
      ...
      if (newSize >= 2) then
        bUnicode := ((newSize mod 2) = 0) and
                    (PAnsiChar(buf)^ = ACRTextFileSignature_Unicode16[1]) and
                    (PAnsiChar(PAnsiChar(buf)+1)^ = ACRTextFileSignature_Unicode16[2]);
      if (bUnicode) then
       begin
        // Unicode 16 text
        i := (newSize shr 1) - 1;
...
//------------------------------------------------------------------------------
{$IFDEF D6H}
procedure TACRWideStringList.SaveToFile(const FileName: WideString);
{$ELSE}
procedure TACRWideStringList.SaveToFile(const FileName: WideString; Dummy: ByteBool);
{$ENDIF}
var fs: TACRFileStream;
begin
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.SaveToFile(const FileName: AnsiString);
var fs: TACRFileStream;
begin
  fs := TACRFileStream.Create(FileName,fmCreate);
  try
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.SaveToStream(Stream: TStream);
var s:    WideString;
    l:    Integer;
    ansi: AnsiString;
begin
 ...
 try
   if (FUnicode) then
    begin
      s := GetText;
      SaveDataToStream(Pointer(ACRTextFileSignature_Unicode16)^,2,Stream,11950);
      l := Length(s);
      if (l > 0) then
       begin
        // skip last CRLF
...
// lock
//------------------------------------------------------------------------------
procedure TACRWideStringList.Lock(Exclusive: Boolean);
begin
  TACRReadWriteThreadSync(FThreadSync).Lock(Exclusive);
end; // Lock
...
// unlock
//------------------------------------------------------------------------------
procedure TACRWideStringList.Unlock;
begin
  TACRReadWriteThreadSync(FThreadSync).Unlock;
end; // Unlock
...
//------------------------------------------------------------------------------
procedure TACRWideStringList.BeginUpdate;
begin
  Lock(True);
end; // BeginUpdate
...
// for compatibility with TStringList
//------------------------------------------------------------------------------
procedure TACRWideStringList.EndUpdate;
begin
  Unlock;
end; // EndUpdate
...
// sort alphabetically using current locale
//------------------------------------------------------------------------------
procedure TACRWideStringList.Sort;
begin
  Lock(True);
  try
...
// export all strings to TSTrings descendant object
//------------------------------------------------------------------------------
procedure TACRWideStringList.ExportToTstrings(Destination: TStrings);
var i: Integer;
begin
  if (Destination = nil) then
    raise EACRException.Create(12110,ErrorLNilPointer);
  for i := 0 to FItemCount-1 do
...
////////////////////////////////////////////////////////////////////////////////
//
// TACRCompressedStreamBlockHeadersArray
//
////////////////////////////////////////////////////////////////////////////////
...
// Construct array of specified size
//------------------------------------------------------------------------------
constructor TACRCompressedStreamBlockHeadersArray.Create;
begin
...
// Destruct array (free mem)
//------------------------------------------------------------------------------
destructor TACRCompressedStreamBlockHeadersArray.Destroy;
begin
 SetSize(0);
 inherited Destroy;
end;//Destroy;
...
// Set length of array to specified size
//------------------------------------------------------------------------------
procedure TACRCompressedStreamBlockHeadersArray.SetSize(NewSize: Int64);
begin
 if (NewSize = 0) then
  begin
   ItemCount := 0;
...
//------------------------------------------------------------------------------
function TACRCompressedStreamBlockHeadersArray.FindPosition(Pos: Int64) : Integer;
var i,dx,f,
    oldRes,res: Int64;
...
// Insert an element into specified position
//------------------------------------------------------------------------------
procedure TACRCompressedStreamBlockHeadersArray.AppendItem(Value: TACRCompressedStreamBlockHeader; Pos: Int64);
begin
 Inc(ItemCount);
 SetSize(ItemCount);
 Items[ItemCount-1] := value;
...
//------------------------------------------------------------------------------
function TACRBitsArray.GetNonZeroBitCount: Integer;
var i: Integer;
begin
 Result := 0;
 if (FBitCount > 0) then
...
// set new size
//------------------------------------------------------------------------------
procedure TACRBitsArray.SetSize(NewSize: Integer);
var
  SizeInBytes: Integer;
begin
...
// load from stream
//------------------------------------------------------------------------------
procedure TACRBitsArray.LoadFromStream(Stream: TStream);
...
// save to stream
//------------------------------------------------------------------------------
procedure TACRBitsArray.SaveToStream(Stream: TStream);
var aBitsSize: Integer;
begin
 ...
 if (FBitCount > 0) then
  begin
   if (FBits = nil) then
    raise EACRException.Create(10399,ErrorLNilPointer);
   aBitsSize := FBitCount div 8;
   if ((FBitCount mod 8) > 0) then
    Inc(aBitsSize);
...
// create
//------------------------------------------------------------------------------
constructor TACRBitsArray.Create;
var
    i,c: Byte;
begin
 FBits := nil;
...
// destroy
//------------------------------------------------------------------------------
destructor TACRBitsArray.Destroy;
begin
 if (FBits <> nil) then
  MemoryManager.FreeAndNilMem(FBits);
end;// Destroy
...
// get bit value
//------------------------------------------------------------------------------
function TACRBitsArray.GetBit(BitNo: Integer): Boolean;
var
{$I ACR_check_null_flag_var.inc}
begin
  if (BitNo >= FBitCount) then
   raise EACRException.Create(10394,ErrorLInvalidBitNo,[BitNo,FBitCount]);
...
// set bit value
//------------------------------------------------------------------------------
procedure TACRBitsArray.SetBit(BitNo: Integer; Value: Boolean);
var Bit: Boolean;
{$I ACR_check_null_flag_var.inc}
{$I ACR_set_null_flag_var.inc}
begin
  if (BitNo >= FBitCount) then
   raise EACRException.Create(10395,ErrorLInvalidBitNo,[BitNo,FBitCount]);
  CHECK_NULL_FLAG_BitNo := BitNo;
  CHECK_NULL_FLAG_NullFlags := FBits;
  {$I ACR_check_null_flag.inc}
...
// returns number of bit = 1 in FBits array by bit position
//------------------------------------------------------------------------------
function TACRBitsArray.GetBitNoByBitPosition(BitPosition: Integer): Integer;
var i,n:    Integer;
    b,k,l:  Byte;
begin
 if (BitPosition >= FNonZeroBitCount) then
  raise EACRException.Create(10428,ErrorLInvalidBitNo,
 ...
 if (Result > FBitCount) then
  raise EACRException.Create(10456,ErrorLInvalidBitNo,[BitPosition,FBitCount]);
 b := PByte(FBits + i)^;
 l := 7;
...
// returns position of bit = 1 by bit no in FBits array
//------------------------------------------------------------------------------
function TACRBitsArray.GetBitPositionByBitNo(BitNo: Integer): Integer;
var i,j:    Integer;
    b,k:      Byte;
begin
 if (BitNo >= FBitCount) then
  raise EACRException.Create(10429,ErrorLInvalidBitNo,
    [BitNo,FBitCount]);
 if (FBitCount = FNonZeroBitCount) then
  begin
   Result := BitNo;
...
// set all bits to 1
//------------------------------------------------------------------------------
procedure TACRBitsArray.SetAllBits;
var
    SizeInBytes: Integer;
begin
 if (FBitCount > 0) then
...
end; // ACRGetRecordCountByBufferSize
...
//------------------------------------------------------------------------------
function ACRGetTickCountDiff(NewTime, OldTime: Cardinal): Cardinal;
const ACRMaxCardinal: Cardinal = $FFFFFFFF;
begin
  if (NewTime >= OldTime) then
   Result := NewTime - OldTime
  else
   Result := Cardinal((ACRMaxCardinal-OldTime+1)+NewTime);
end; // ACRGetTickCountDiff


//------------------------------------------------------------------------------
// clear string with private data (passwords, keys, etc.)
//------------------------------------------------------------------------------
procedure ACRClearString(var Value: AnsiString; EncryptedDBOnly: Boolean = False);
var l: Integer;
begin
 if (EncryptedDBOnly and (not ACR_ENCRYPTED_DB_USED)) then
  Value := ''
 else
  begin
    l := Length(Value);
...
procedure ACRClearString(var Value: WideString; EncryptedDBOnly: Boolean = False);
var l: Integer;
begin
 if (EncryptedDBOnly and (not ACR_ENCRYPTED_DB_USED)) then
  Value := ''
...
function ACRGetLastTableOpertaion(operation: TACRLastTableOperation): WideString;
begin
  if (Integer(Operation) < Length(ACRLastTableOperationNames)) then
   Result := ACRLastTableOperationNames[Integer(Operation)]
  else
   Result := 'UNKNOWN OPERATION';
end; // ACRGetLastTableOpertaion
...
begin
  ...
  Result := ACRCompareWideString(value1.TableName,value2.TableName,False);
end; // ACRCompareTableInfo
...
begin
  ...
  repeat
    ...
    p := (min + max) shr 1;
    repeat
      while (ACRCompareTableInfo(tablesInfo[i],tablesInfo[p]) < 0) do
        Inc(i);
      while (ACRCompareTableInfo(tablesInfo[j],tablesInfo[p]) > 0) do
        Dec(j);
      if (i <= j) then
       begin
        if (i <> j) then
         ACRSwapTableInfoElements(tablesInfo,i,j);
        if (p = i) then
          p := j
        else
        if (p = j) then
    ...
    if (min < j) then
     begin
      if (j = max) then
       raise EACRException.Create(12131,ErrorLErrorSortingTableInfo,[i,j,min,max,Length(tablesInfo)]);
      ACRSortTableInfo(tablesInfo,min,j);
     end;
    min := i;
  until (i >= max);
end; // ACRSortTableInfo


function ACRGetTableStateAsString(const State: TACRTableState): AnsiString;
begin
  Result := #13#10+'TableState = '+IntToStr(State.TableState)
            +#13#10+'LastTableOperation = '+ACRLastTableOperationNames[Integer(State.LastTableOperation)]
...
begin
  ...
  begin
    Result := ACR_MAX_WAIT_LEVEL;
{$IFDEF DEBUG_TRACE_ACRGetWaitLevel}
aaWriteToLog('ACRGetWaitLevel' + #13#10 + 'StartWaitTime = ' + IntToStr(StartWaitTime) + #13#10 + 'MaxWaitTime = ' + IntToStr(MaxWaitTime)+ #13#10 + 'Result = ' + IntToStr(Result));
{$ENDIF}
  end
  else
  begin
    dt := ACRGetTickCountDiff(aaGetTickCount, StartWaitTime);
    // fixed in v.5.10
    if (dt <= 1) then
      Result := 0
    else
      ...
      begin
        if (MaxWaitTime <= ACR_MAX_WAIT_LEVEL) then
          levLength := 1
        else
          levLength := MaxWaitTime div ACR_MAX_WAIT_LEVEL + 1;
        Result := Byte(Cardinal(dt) div levLength);
      end;
{$IFDEF DEBUG_TRACE_ACRGetWaitLevel}
aaWriteToLog('ACRGetWaitLevel' + #13#10 + 'StartWaitTime = ' + IntToStr(StartWaitTime) + #13#10 + 'MaxWaitTime = ' + IntToStr(MaxWaitTime)
+ #13#10 + 'dt = ' + IntToStr(dt) + #13#10 + 'ACR_MAX_WAIT_LEVEL = ' +IntToStr(ACR_MAX_WAIT_LEVEL)
+ #13#10 +'dt * ACR_MAX_WAIT_LEVEL div MaxWaitTime = ' + IntToStr(dt * ACR_MAX_WAIT_LEVEL div MaxWaitTime) + #13#10 + 'Result = ' + IntToStr(Result));
{$ENDIF}
  end;
end; // ACRGetWaitLevel
...
begin
  ...
  if (month in [4,6,9,11]) then
   ...
   Result := 31;
end; // ACRGetMaxDayOfMonth


initialization

{$IFDEF DEBUG_LOG_INIT}
aaWriteToLog('< ACRTypes initialized');
{$ENDIF}
