...
const
EACRTokenType: array [0..13] of WideString =
(
...
//  Text:           AnsiString;
 ...
 TACRLexer = class (TObject)
  ...
  public
   constructor Create(SQLScript: WideString; SQLParams: TACRSQLParams = nil);
   ...
   function ACRIsBoooleanOperator: Boolean;
  public
 ...
 function ACRIsDelimiter(c: WideChar): Boolean;
 // Token -> SQL script
 function ACRGetTokenText(const Token: TToken): WideString;
 // return true if the token is the operator token
 function ACRIsOperatorSign(const token: TToken; const operatorName: WideString): Boolean;
...
//------------------------------------------------------------------------------
procedure TACRLexer.FreeCurrentCommand;
var j: Integer;
begin
  for j := 0 to FCurrentCommand.NumTokens-1 do
   begin
     ...
     ACRClearString(FCurrentCommand.Tokens[j].Text);
   end;
...
// Parse SQL script
//------------------------------------------------------------------------------
function TACRLexer.Parse: Boolean;
var i,l:                    Integer;
    c,priorSymbol,
 ...
 begin
  ...
  for i := 0 to ACRMaxSQLReservedWords do
   if (crc = ACRSQLReservedWordsCRC[i]) then
     begin
      Result := i;
      break;
     end;
     ...
     begin
        ...
        if (FSQLParams <> nil) then
         begin
          ...
          if (Param <> nil) then
            begin
              FCurrentCommand.Tokens[token].ParamValue := TACRVariant.Create;
...
//            raise EACRException.Create(30358, ErrorGParameterValueNotFound, [ParamName]);
            begin
...
//  line := FCurrentLine;
 ...
 begin
  ...
  if (numParenthesis > 0) then
    raise EACRException.Create(30059, ErrorGMissingRightParenthesis,
                                  [LeftParenthesisLine,LeftParenthesisColumn]);

  if (numParenthesis < 0) then
    raise EACRException.Create(30060, ErrorGUnexpectedRightParenthesis,
                                  [LeftParenthesisLine,LeftParenthesisColumn]);
  priorSymbol := ' ';

  if (bQuoteNotClosed) then
    raise EACRException.Create(30061, ErrorGUnterminatedString,
    ...
    begin
     ...
     if FCurrentCommand.Tokens[token].TokenType in [tktInt,tktFloat] then
      if not ((c >= '0') and (c <= '9') or (c = Dot) or
              ...
              (c = '+') or (c = '-') or (c = 'x')) then
        raise EACRException.Create(30062, ErrorGInvalidNumericSymbol,
                                                              [c,line,column]);
      // check float type .22
...
// create
//------------------------------------------------------------------------------
constructor TACRLexer.Create(SQLScript: WideString; SQLParams: TACRSQLParams);
begin
  line := 1;
  FSQL := SQLScript;
...
// makes next command current
//------------------------------------------------------------------------------
function TACRLexer.GetNextCommand: Boolean;
begin
...
// gets next token in current command
//------------------------------------------------------------------------------
function TACRLexer.GetNextToken(var Token: TToken): Boolean;
begin
...
// gets current token in current command
//------------------------------------------------------------------------------
function TACRLexer.GetCurrentToken(var Token: TToken): Boolean;
begin
...
//------------------------------------------------------------------------------
function TACRLexer.LookNextToken(var Token: TToken): Boolean;
begin
...
// gets current token No
//------------------------------------------------------------------------------
function TACRLexer.GetCurrentTokenNo: integer;
begin
...
// sets current token No
//------------------------------------------------------------------------------
function TACRLexer.SetCurrentTokenNo(TokenNo: integer; var Token: TToken): Boolean;
begin
...
// sets new number of tokens in current command, CurrentTokenNo := 0
//------------------------------------------------------------------------------
procedure TACRLexer.SetNumTokensInCurrentCommand(NewNumTokens: Integer);
begin
...
// starts save SQL script: all GetNextToken,GetNextToken will add all token's texts to SavedScript
//------------------------------------------------------------------------------
procedure TACRLexer.StartSaveScript;
var c: WideChar;
begin
...
// stops save SQL script and return the saved script
//------------------------------------------------------------------------------
function TACRLexer.StopSaveScript(GetResult: Boolean): WideString;
var l:  Integer;
    wc: WideChar;
begin
  Result := '';
...
//------------------------------------------------------------------------------
function TACRLexer.ACRIsBoooleanOperator: Boolean;
var token: TToken;
begin
  Result := False;
...
begin
 ...
 bModified := False;
 n := -1;
 lexer := TACRLexer.Create(SQLText,nil);
 try
      ...
      repeat
       ...
       if ((token.TokenType = tktString) and (token.Text = '?')) then
        begin
         Inc(n);
         bModified := true;
         Result := Result + ':ACR_MS_Param'+IntToStr(n)+ Space;
        end
       else
...
aaWriteToLog('ACRLexer> initialized');
{$ENDIF}
