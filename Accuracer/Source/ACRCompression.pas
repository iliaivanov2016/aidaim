...
{$IFNDEF X64_ON}
const ACRCompressionAlgorithmNames:array[0..4] of AnsiString = ('NONE', 'ZLIB','BZIP','PPM','PPMI');
{$ELSE}
const ACRCompressionAlgorithmNames:array[0..3] of AnsiString = ('NONE', 'ZLIB','BZIP','PPMI');
{$ENDIF}
const ACR_MAX_COMPRESSION_ALGORITHM = 3;
const ACR_MAX_COMPRESSION_MODE = 9;

type
 TACRCompressionMode = Byte; // 0-9
 TACRCompressionLevel = (aclNone,aclFastest,aclNormal,aclMaximum); // 0,1,5,9

 TACRCompression = packed record
  CompressionAlgorithm: TACRCompressionAlgorithm;
  CompressionMode:      TACRCompressionMode;
  CompressionLevel:     TACRCompressionLevel;
 end;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  TACRStream = class (TStream)
  private
    FThreadSync:        TACRReadWriteThreadSync;
    FBlockSize:         Integer;
    FOnProgress:        TACRProgressEvent; // progress for bulk operations
    FModified:          Boolean;
    FAbort:             Boolean;
   protected
    // on progress
   ...
   public
    ...
    // Progress Event
    property OnProgress: TACRProgressEvent read FOnProgress write FOnProgress;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  TACRMemoryStream = class (TACRStream)
   private
    FBuffer:            PAnsiChar;
    FPosition:          Int64;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  TACRFileStream = class (TACRStream)
   private
    FHandle:          Integer;
...
{$ENDIF}
  ...
  TACRTempFileStream = class (TACRStream)
   private
    FFileStream:        TACRFileStream;
...
{$ENDIF}
   ...
   public
    ...
    {$ENDIF}
   public
    constructor Create(Source: TACRStream);
    destructor Destroy; override;
  end; // TACRTempFileStream
...
////////////////////////////////////////////////////////////////////////////////
  ...
  TACRTemporaryStream = class (TACRStream)
   private
    FMemoryLimit:        Integer;
    FMemoryStream:       TACRMemoryStream;
    FFileStream:         TACRTempFileStream;
   ...
   public
    ...
    destructor Destroy; override;
   public
    property FileStream: TACRTempFileStream read FFileStream;
    property MemoryStream: TACRMemoryStream read FMemoryStream;
    property FileName: WideString read FFileName;
    property InMemory: Boolean read FInMemory;
    property MemoryLimit: Integer read FMemoryLimit write FMemoryLimit;
  end; // TACRTemporaryStream


////////////////////////////////////////////////////////////////////////////////
//
// TACRCompressedBLOBStream
//
////////////////////////////////////////////////////////////////////////////////


  // Accuracer BLOB stream with optional compression
  // when compression algorithm <> acaNone Write allowed
  // only ot the end of stream
  TACRCompressedBLOBStream = class (TACRStream)
   private
    FRepair:                Boolean;
    FHeaders:               TACRCompressedStreamBlockHeadersArray;
    FUncompressedSize:      Int64;
    FCompressedSize:        Int64;
    FStartPosition:         Int64;
    FCurrentHeader:         Integer;
    FPosition:              Int64;
    FCompressionMode:       TACRCompressionMode;
    ...
    FBLOBDescriptor:        TACRBLOBDescriptor;
   private
    ...
    procedure PrepareBufferForWriting(
                                      ...
                                      var Header:   TACRCompressedStreamBlockHeader
                                     );
   ...
   public
    ...
    property CompressionAlgorithm: TACRCompressionAlgorithm read FCompressionAlgorithm;
    // compression mode
    property CompressionMode: Byte read FCompressionMode;
    // compressed size
    property CompressedSize: Int64 read GetCompressedSize;
    property BLOBDescriptor: TACRBLOBDescriptor read FBLOBDescriptor;
  end; // TACRCompressedBLOBStream
 ...
 //------------------------------------------------------------------------------
 function ACRInternalCompressBuffer(
                          CompressionAlgorithm:   TACRCompressionAlgorithm;
 ...
 // OutBuf - pointer to compressed data
 // OutSize - size of compressed data
 function ACRInternalDecompressBuffer(
                          CompressionAlgorithm:   TACRCompressionAlgorithm;
                          InBuf:                  PAnsiChar;
                          InSize:                 Integer;
                          out OutBuf:             PAnsiChar;
 ...
 function ACRGetDefaultTempDir: WideString;
 ...
 procedure SaveCryptoParamsToStream(const CryptoParams: TACRCryptoParams; Stream: TStream; ErrorCode: Integer; DoNotSaveKeyAndPassword: Boolean = false);
 procedure LoadCryptoParamsFromStream(var CryptoParams: TACRCryptoParams; Stream: TStream; ErrorCode: Integer; DoNotSaveKeyAndPassword: Boolean = false);
 ...
 procedure SaveTACRWideStringListToStream(List: TACRWideStringList; Stream: TStream; ErrorCode: Integer);
 procedure LoadTACRWideStringListFromStream(List: TACRWideStringList; Stream: TStream; ErrorCode: Integer);

 procedure SetStreamPosition(Stream: TStream; NewPosition: Int64; ErrorCode: Integer);

 function GetCompressionAlgorithm(Name: AnsiString): TACRCompressionAlgorithm;
 function GetCompressionAlgorithmSQLName(CompressionAlgorithm: TACRCompressionAlgorithm): AnsiString;
//------------------------------------------------------------------------------
// returns true if file exists
//------------------------------------------------------------------------------
 function ACRFileExistsAnsi(FileName: PAnsiChar): Boolean;
 function ACRFileExistsUnicode(FileName: PWideChar): Boolean;
 function ACRFileExists(FileNameA: PAnsiChar; FileNameW: PWideChar = nil): Boolean; overload;
 function ACRFileExists(FileNameA: AnsiString; FileNameW: WideString = ''): Boolean; overload;
...
// on progress
//------------------------------------------------------------------------------
procedure TACRStream.DoOnProgress(Progress: Double);
begin
...
// lock
//------------------------------------------------------------------------------
procedure TACRStream.Lock{(WriteMode: Boolean)};
begin
...
//------------------------------------------------------------------------------
procedure TACRStream.SaveToStream(Stream: TStream);
...
begin
 if (FBlockSize = 0) then
  raise EACRException.Create(10418,ErrorLZeroBlockSizeIsNotAllowed);
 OldPos := Position;
   ...
   while ((not FAbort) and (OutBytes < InSize)) do
    begin
     ...
     if (ReadBytes <> OutSize) then
      raise EACRException.Create(10146,ErrorLCannotReadFromStream,
        [Pos,Self.Size,OutSize,ReadBytes]);

     Pos := Stream.Position;
     WriteBytes := Stream.Write(Buf^,OutSize);
     if (WriteBytes <> OutSize) then
      raise EACRException.Create(10147,ErrorLCannotWriteToStream,
...
//------------------------------------------------------------------------------
procedure TACRStream.LoadFromStream(Stream: TStream);
begin
...
// load from stream
//------------------------------------------------------------------------------
procedure TACRStream.LoadFromStreamWithPosition(
                    Stream:       TStream;
                    FromPosition: Int64;
                    StreamSize:   Int64
                    );
...
begin
 if (FBlockSize = 0) then
  raise EACRException.Create(10419,ErrorLZeroBlockSizeIsNotAllowed);
 OldPos := Position;
 OldPos1 := Stream.Position;
 ...
 try
   while ((not FAbort) and (Stream.Position < FromPosition + StreamSize)) do
    begin
     ...
     if (ReadBytes <> OutSize) then
      raise EACRException.Create(10148,ErrorLCannotReadFromStream,
        [Pos,Stream.Size,OutSize,ReadBytes]);

     Pos := Self.Position;
     WriteBytes := Self.Write(Buf^,OutSize);
     if (WriteBytes <> OutSize) then
      raise EACRException.Create(10149,ErrorLCannotWriteToStream,
...
//------------------------------------------------------------------------------
procedure TACRStream.LoadFromFile(const FileName: AnsiString);
var
  Stream: TACRStream;
begin
...
//------------------------------------------------------------------------------
procedure TACRStream.SaveToFile(const FileName: AnsiString);
var
  Stream: TACRStream;
begin
...
// return maximum value of FReallocDelta
//------------------------------------------------------------------------------
function TACRMemoryStream.GetMaxDelta: Int64;
begin
  ...
  if (FBufferSize = 4) then
   ...
   Result := ACRGetReallocDelta(FBufferSize);
end; // GetMaxDelta
...
// sets new realloc delta with range check
//------------------------------------------------------------------------------
procedure TACRMemoryStream.SetReallocDelta(const NewDelta: Int64);
var MaxDelta: Int64;
begin
  if (NewDelta <= 0) then
   FReallocDelta := 0
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRMemoryStream.InternalSetSize(const NewSize: Int64);
...
// seek
//------------------------------------------------------------------------------
function TACRMemoryStream.InternalSeek(NewPosition: Int64): Int64;
begin
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRMemoryStream.SetSize(NewSize: Longint);
begin
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRMemoryStream.SetSize(const NewSize: Int64);
begin
 InternalSetSize(NewSize);
end; // SetSize
{$ENDIF}
...
//------------------------------------------------------------------------------
function TACRMemoryStream.Read(var Buffer; Count: Longint): Longint;
...
// set size of compressed stream
//------------------------------------------------------------------------------
function TACRMemoryStream.Write(const Buffer; Count: Longint): Longint;
begin
...
// set size of compressed stream
//------------------------------------------------------------------------------
function TACRMemoryStream.Seek(Offset: Longint; Origin: Word): Longint;
...
// set size of compressed stream
//------------------------------------------------------------------------------
function TACRMemoryStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
...
// set size of compressed stream
//------------------------------------------------------------------------------
constructor TACRMemoryStream.Create(Buffer: PAnsiChar = nil; BufferSize: Integer = -1);
begin
 FBuffer := nil;
 FBufferSize := 0;
 FSize := 0;
...
// set size of compressed stream
//------------------------------------------------------------------------------
destructor TACRMemoryStream.Destroy;
begin
  InternalSetSize(0);
  inherited;
end; // Destroy
...
//------------------------------------------------------------------------------
procedure TACRMemoryStream.SetBuffer(aBuffer: PAnsiChar; aBufferSize: Integer);
begin
  FPosition := 0;
  FSize := aBufferSize;
...
// added in 4.98 for pre-allocating buffer for saving data
//------------------------------------------------------------------------------
procedure TACRMemoryStream.SetApproximateSize(NewSize: Integer);
begin
...
//------------------------------------------------------------------------------
procedure TACRFileStream.ConvertFileModes(
                                    const Mode:     Word;
                                    out AccessMode: Cardinal;
                                    out ShareMode:  Cardinal;
                                    out CreateMode: Cardinal
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRFileStream.InternalSetSize(const NewSize: Int64);
var OldPos: Int64;
{$IFDEF LINUX}
  SysErrorCode: DWORD;
{$ENDIF}
...
{$IFDEF LINUX}
 if (lseek64(FHandle, NewSize, SEEK_SET) <> NewSize) then
   begin
     ...
     raise EACRException.Create(40019, ErrorRCannotSetNewSize,
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRFileStream.SetSize(NewSize: Longint);
begin
 InternalSetSize(NewSize);
end; // SetSize
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRFileStream.SetSize(const NewSize: Int64);
begin
 InternalSetSize(NewSize);
end; // SetSize
{$ENDIF}
...
// read
//------------------------------------------------------------------------------
function TACRFileStream.Read(var Buffer; Count: Longint): Longint;
begin
...
// write
//------------------------------------------------------------------------------
function TACRFileStream.Write(const Buffer; Count: Longint): Longint;
begin
...
// seek
//------------------------------------------------------------------------------
function TACRFileStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
...
// seek
//------------------------------------------------------------------------------
function TACRFileStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
...
// Create
//------------------------------------------------------------------------------
constructor TACRFileStream.Create(const FileName: AnsiString; Mode: Word);
begin
 inherited Create;
 ...
 if (Mode = fmCreate) then
  begin
   ...
   if (FHandle < 0) then
    raise EACRException.Create(10104,ErrorLCannotCreateFile,[FileName]);
  end
 else
  begin
   FHandle := FileOpen(FileName,Mode);
   if (FHandle < 0) then
    raise EACRException.Create(10105,ErrorLCannotOpenFile,[FileName,Mode]);
  end;
end; // Create


{$IFDEF D6H}
constructor TACRFileStream.Create(const FileName: WideString; Mode: Word);
...
{$ELSE}
constructor TACRFileStream.Create(const FileName: WideString; Mode: Word; Dummy: ByteBool);
var ShareMode, AccessMode, CreateMode: Cardinal;
begin
 inherited Create;
 FBlockSize := DefaultFileBlockSize;
...
// Destroy
//------------------------------------------------------------------------------
destructor TACRFileStream.Destroy;
begin
 if FHandle >= 0 then
  FileClose(FHandle);
 inherited;
...
// fills the entire file with zero bytes
//------------------------------------------------------------------------------
procedure TACRFileStream.ZeroFill;
const BufSize = 4096;
var sz,pos: Int64;
    buf:    PAnsiChar;
    n:      Integer;
...

{
   ...
   if (ACR_ENCRYPTED_DB_USED and (Count > 0)) then
    begin
     buf := MemoryManager.GetMem(Count);
     try
       Move(Buffer,buf^,Count);
       ACREncryptBuffer(FCryptoInfo,buf,Count);
...
// load block to cache
//------------------------------------------------------------------------------
procedure TACRTempFileStream.LoadBlock(BlockNo: Int64);
 ...
 if (FSize < FFileStream.Position + 1) then
  ...
  begin
   ...
   if (BytesRead <> Count) then
    raise EACRException.Create(11697,ErrorLCannotReadFromStream,
      [FPosition,FSize,FBlockSize,BytesRead]);
   FCachedBytes := Count;
   {$IFNDEF SQLMEMTABLE}
   if (ACR_ENCRYPTED_DB_USED) then
    ACRDecryptBuffer(FCryptoInfo,FCachedBlock,Count);
   {$ENDIF}
  end;
...
//------------------------------------------------------------------------------
procedure TACRTempFileStream.SaveBlock(BlockNo: Int64);
  ...
  begin
   ...
   if (ACR_ENCRYPTED_DB_USED) then
    begin
      ...
      ACREncryptBuffer(FCryptoInfo,FTempBlock,Count);
    end;
   if (ACR_ENCRYPTED_DB_USED) then
   ...
   if (BytesWrite <> Count) then
    raise EACRException.Create(11698,ErrorLCannotWriteToStream,
...
// clear cache
//------------------------------------------------------------------------------
procedure TACRTempFileStream.ClearCache;
begin
  FCachedBlockNo := -1;
...
//------------------------------------------------------------------------------
procedure TACRTempFileStream.WriteCachedBlock;
begin
...
//------------------------------------------------------------------------------
procedure TACRTempFileStream.ExtendEncryptedStream(NewSize: Int64);
...
//------------------------------------------------------------------------------
procedure TACRTempFileStream.ShrinkEncryptedStream(NewSize: Int64);
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRTempFileStream.InternalSetSize(const NewSize: Int64);
begin
 WriteCachedBlock;
 if (NewSize <> FSize) then
  begin
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRTempFileStream.SetSize(NewSize: Longint);
begin
...
//------------------------------------------------------------------------------
{$IFDEF D6H}
procedure TACRTempFileStream.SetSize(const NewSize: Int64);
begin
  InternalSetSize(NewSize);
end; // SetSize
{$ENDIF}
...
// Read
//------------------------------------------------------------------------------
function TACRTempFileStream.Read(var Buffer; Count: Longint): Longint;
...
// Write
//------------------------------------------------------------------------------
function TACRTempFileStream.Write(const Buffer; Count: Longint): Longint;
...
// Seek
//------------------------------------------------------------------------------
function TACRTempFileStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
 case (Origin) of
  soFromBeginning:
    FPosition := Offset;
...
// Seek
//------------------------------------------------------------------------------
function TACRTempFileStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
 case (Origin) of
  soBeginning:
    FPosition := Offset;
...
// constructor
//------------------------------------------------------------------------------
constructor TACRTempFileStream.Create(Source: TACRStream);
begin
  FFileStream := TACRFileStream.Create(GetTempFileName,fmCreate);
...
{$IFNDEF SQLMEMTABLE}
 if (ACR_ENCRYPTED_DB_USED) then
  begin
   ...
   // generate random key
   FCryptoInfo.KeyInfo.KeySize := ACR_MAX_KEY+1;
   ACRGenerateRandomBuffer(@FCryptoInfo.KeyInfo.Key[0],FCryptoInfo.KeyInfo.KeySize);
   FCryptoInfo.CryptoAlgorithm := ACR_Cipher_Blowfish;
   FCryptoInfo.CryptoMode := ACR_Cipher_Mode_CTS;
...
// destructor
//------------------------------------------------------------------------------
destructor TACRTempFileStream.Destroy;
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRTemporaryStream.InternalSetSize(const NewSize: Int64);
begin
 if (FInMemory) then
  begin
    if (NewSize <= FMemoryLimit) then
     ...
     begin
      ...
      FFileStream := TACRTempFileStream.Create(FMemoryStream);
      FMemoryStream.Free;
      FMemoryStream := nil;
      FInMemory := False;
      FFileStream.Size := NewSize;
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRTemporaryStream.SetSize(NewSize: Longint);
begin
 InternalSetSize(NewSize);
end; // SetSize
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TACRTemporaryStream.SetSize(const NewSize: Int64);
begin
 InternalSetSize(NewSize);
end; // SetSize
{$ENDIF}
...
// read
//------------------------------------------------------------------------------
function TACRTemporaryStream.Read(var Buffer; Count: Longint): Longint;
var Stream: TStream;
begin
 if (FInMemory) then
...
// write
//------------------------------------------------------------------------------
function TACRTemporaryStream.Write(const Buffer; Count: Longint): Longint;
...
// seek
//------------------------------------------------------------------------------
function TACRTemporaryStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
 if (FInMemory) then
...
// seek
//------------------------------------------------------------------------------
function TACRTemporaryStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
 if (FInMemory) then
  Result := FMemoryStream.Seek(Offset,Origin)
 else
...
// Create
//------------------------------------------------------------------------------
constructor TACRTemporaryStream.Create;
begin
 inherited Create;
 FBlockSize := DefaultTemporaryBlockSize;
 ...
 FFileName := '';
 FInMemory := True;
 FMemoryStream := TACRMemoryStream.Create;
 FFileStream := nil;
end; // Create
...
// Destroy
//------------------------------------------------------------------------------
destructor TACRTemporaryStream.Destroy;
begin
...
// returns block size for creating a compressed blob stream with specified compression level
//------------------------------------------------------------------------------
function TACRCompressedBLOBStream.InternalGetBlockSize(CompressionMode: Byte): Integer;
begin
...
// set size of compressed stream
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.CalculateRate;
var i: 		Integer;
    f,f1:	Extended;
begin
 FCompressedSize := 0;
...
// create
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.InternalCreate(ToCreate: Boolean);
begin
 // compression
 FHeaders := TACRCompressedStreamBlockHeadersArray.Create;
 if (ToCreate) then
  begin
...
// load  block headers
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.LoadHeaders;
var CHeader:    TACRCompressedStreamBlockHeader;
    Pos,NewPos: Int64;
    i,OldPos:   Int64;
begin
 FCompressedSize := 0;
 ...
 if (FCompressedStream.Position <> FBLOBDescriptor.StartPosition) then
  raise EACRException.Create(10080,ErrorLCannotSetPosition,
 ...
 while (i < FBLOBDescriptor.NumBlocks) do
  begin
   ...
   if (
       ...
       sizeof(TACRCompressedStreamBlockHeader)
      ) then
    begin
     // stream too small
     if (FRepair) then
      ...
      raise EACRException.Create(10082,ErrorLStreamSizeTooSmall,
        [FCompressedStream.Size,
        FCompressedStream.Position + sizeof(TACRCompressedStreamBlockHeader)]);
    end; // check if we can read block header from current position
   FCompressedStream.ReadBuffer(CHeader,sizeof(TACRCompressedStreamBlockHeader));
...
//   NewPos := CHeader.OffsetToNextHeader;
   ...
   if (FCompressedStream.Position <> NewPos) then
    raise EACRException.Create(10083,ErrorLCannotSetPosition,
...
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.PrepareBufferForWriting
                                     (
                                      InBuf:        PAnsiChar;
                                      InSize:       Integer;
                                      var OutBuf:   PAnsiChar;
...
begin
  ...
  Header.Crc32 := ACR_CRC32(0,InBuf,InSize);
  if (not ACRInternalCompressBuffer(FCompressionAlgorithm,FCompressionMode,
          InBuf,InSize,OutBuf,Header.CompressedSize)) then
   begin
    if (OutBuf <> nil) then
     FreeMem(OutBuf);
    raise EACRException.Create(10085,ErrorLCompressBufferFailed,
...
// load block from file, decompress it and checks crc
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.LoadBlock(
                                      CurHeader:  Int64;
                                      var OutBuf: PAnsiChar
                                            );
...
begin
  ...
  try
   ...
   if (BytesRead <> CompSize) then
    begin
     raise EACRException.Create(10086,ErrorLCannotReadFromStream,
      [FHeaders.Positions[CurHeader] + sizeof(TACRCompressedStreamBlockHeader),
      FCompressedStream.Size,CompSize,BytesRead]);
    end;

   UncompSize := FHeaders.Items[CurHeader].UncompressedSize;
   if (not ACRInternalDecompressBuffer(FCompressionalgorithm,
           InBuf,CompSize,OutBuf,UncompSize)) then
    begin
     // decompression error
     raise EACRException.Create(10087,ErrorLDecompressBufferFailed,
   ...
   if (FHeaders.Items[CurHeader].UncompressedSize <> UncompSize) then
    begin
     FreeMem(outBuf);
     OutBuf := nil;
     raise EACRException.Create(10088,ErrorLDecompressBufferFailedInvalidSize,
      [UncompSize,FHeaders.Items[CurHeader].UncompressedSize]);
    end;
   // check crc
   CheckSum := ACR_CRC32(0,OutBuf,UncompSize);
   if (FHeaders.Items[CurHeader].Crc32 <> CheckSum) then
    begin
    // decompression crc error
     FreeMem(outBuf);
     OutBuf := nil;
     raise EACRException.Create(10089,ErrorLDecompressBufferFailedInvalidCRC,
...
// set size of compressed stream
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.InternalIncreaseSize(NewSize: Int64);
var Buf:                  PAnsiChar;
...
begin
 ...
 if (ExtensionSize <= 0) then
  raise EACRException.Create(10098,ErrorLInvalidExtensionSize,
  ...
  while (Count < ExtensionSize) do
   begin
    ...
    if (WriteBytes <> WriteSize) then
      raise EACRException.Create(10097,ErrorLCannotWriteToStream,
...
// set size of compressed stream
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.InternalDecreaseSize(NewSize: Int64);
    ...
    if (ExtensionSize > 0) then
     begin
      ...
      if (FCompressedStream.Size <> FHeaders.Positions[CurHdr]) then
       raise EACRException.Create(10093,ErrorLInvalidStreamSize,
        [FCompressedStream.Size,FHeaders.Positions[CurHdr]]);
      FCompressedStream.Position := FCompressedStream.Size;
      if (FCompressedStream.Position <> FCompressedStream.Size) then
       raise EACRException.Create(10094,ErrorLCannotSetPosition,
        [FCompressedStream.Size,FCompressedStream.Position,
         FCompressedStream.Size]);
      ...
      BytesWrite := FCompressedStream.Write(FHeaders.Items[curHdr],
          sizeof(TACRCompressedStreamBlockHeader));
      if (BytesWrite <> sizeof(TACRCompressedStreamBlockHeader)) then
       raise EACRException.Create(10095,ErrorLWriteToStream,
        [OldPos,FCompressedStream.Size,sizeof(TACRCompressedStreamBlockHeader),BytesWrite]);
      OldPos := FCompressedStream.Position;
      BytesWrite := FCompressedStream.Write(OutBuf^,FHeaders.Items[CurHdr].CompressedSize);
      if (BytesWrite <> sizeof(TACRCompressedStreamBlockHeader)) then
       raise EACRException.Create(10096,ErrorLWriteToStream,
        [OldPos,FCompressedStream.Size,sizeof(TACRCompressedStreamBlockHeader),BytesWrite]);
     end;
  finally
   if (OutBuf <> nil) then
...
// set size of compressed stream
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.InternalSetSize(NewSize: Int64);
begin
 if (FCompressionAlgorithm = acaNone) then
  begin
...
// internal seek
//------------------------------------------------------------------------------
function TACRCompressedBLOBStream.InternalSeek(NewPosition: Int64): Int64;
begin
 if (FCompressionAlgorithm = acaNone) then
  begin
...
// set size of compressed stream
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.SetSize(NewSize: Longint);
begin
 InternalSetSize(NewSize);
end; // SetSize
...
//------------------------------------------------------------------------------
{$IFDEF D6H}
procedure TACRCompressedBLOBStream.SetSize(const NewSize: Int64);
begin
 InternalSetSize(NewSize);
end; // SetSize
{$ENDIF}
...
// gets compressed size
//------------------------------------------------------------------------------
function TACRCompressedBLOBStream.GetCompressedSize: Int64;
begin
...
// returns compression rate (100.0 if there is no compression)
//------------------------------------------------------------------------------
function TACRCompressedBLOBStream.GetCompressionRate: Double;
begin
 CalculateRate;
...
// Create
//------------------------------------------------------------------------------
constructor TACRCompressedBLOBStream.Create(
						           Stream:                TStream;
                       BLOBDescriptor:        TACRBLOBDescriptor;
                       ToCreate:              Boolean = false;
                       ToRepair:              Boolean = false
...
begin
 ...
 if (FCompressedStream.Position <> FBLOBDescriptor.StartPosition) then
  raise EACRException.Create(10103,ErrorLCannotSetPosition,
    [FBLOBDescriptor.StartPosition,FCompressedStream.Position,
    FCompressedStream.Size]);
 ...
 if (FCompressionAlgorithm = acaNone) then
  begin
   ...
   if (FCompressedStream.Position <> FBLOBDescriptor.StartPosition) then
    raise EACRException.Create(10079,ErrorLCannotSetPosition,
      [FBLOBDescriptor.StartPosition,FCompressedStream.Position,
       FCompressedStream.Size]);
   // default block size
   if (ToCreate) then
    ...
    begin
     ...
     if (not FRepair) then
      if (FCompressedStream.Size - FCompressedStream.Position <
        ...
        raise EACRException.Create(10081,ErrorLStreamSizeTooSmall,
          [FCompressedStream.Size,
...
// Destroy
//------------------------------------------------------------------------------
destructor TACRCompressedBLOBStream.Destroy;
begin
...
//------------------------------------------------------------------------------
function TACRCompressedBLOBStream.Read(var Buffer; Count: Longint): Longint;
var ReadSize:   Int64;
    OutBuf:     PAnsiChar;
begin
 if (FCompressionAlgorithm = acaNone) then
    ...
    begin
     ...
     while ((FPosition < FUncompressedSize) and (Result < Count)) do
      begin
       ...
       if (ReadSize <= 0) then
        raise EACRException.Create(10090,
...
// write beyond EOF
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.InternalWriteBeyondEOF;
var OldPos: Int64;
begin
 OldPos := FPosition;
 Self.Position := 0;
 ...
 if (Self.Position <> OldPos) then
  raise EACRException.Create(10091,ErrorLCannotSetPosition,
    [OldPos,FPosition,FUncompressedSize]);
 if (FUncompressedSize <> OldPos) then
  raise EACRException.Create(10092,ErrorLInvalidStreamSize,
...
// write block
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.InternalWriteBlock(InBuf: PAnsiChar; InSize: Integer);
var OutBuf:         PAnsiChar;
    WriteBytes:     Integer;
    OldPos:         Int64;
begin
...
//      FHeaders.Positions[FCurrentHeader] +
    ...
    if (FCompressedStream.Position <> FHeaders.Positions[FCurrentHeader]) then
     raise EACRException.Create(10099,ErrorLCannotSetPosition,
      [FHeaders.Positions[FCurrentHeader],
        FCompressedStream.Position,FCompressedStream.Size]);
    ...
    WriteBytes := FCompressedStream.Write(FHeaders.Items[FCurrentHeader],
      sizeof(TACRCompressedStreamBlockHeader));
    if (WriteBytes <> sizeof(TACRCompressedStreamBlockHeader)) then
     raise EACRException.Create(10100,ErrorLCannotWriteToStream,
      [OldPos,FCompressedStream.Size,sizeof(TACRCompressedStreamBlockHeader),WriteBytes]);

    OldPos := FCompressedStream.Position;
    WriteBytes := FCompressedStream.Write(OutBuf^,
      FHeaders.Items[FCurrentHeader].CompressedSize);
    if (WriteBytes <> FHeaders.Items[FCurrentHeader].CompressedSize) then
     raise EACRException.Create(10101,ErrorLCannotWriteToStream,
...
// write prepare
//------------------------------------------------------------------------------
procedure TACRCompressedBLOBStream.InternalWritePrepare(Count, Result: Integer);
var
...
//    NewPos := FHeaders.Items[FCurrentHeader].OffsetToNextHeader;
  ...
  FCompressedStream.Position := NewPos;
  if (FCompressedStream.Position <> NewPos) then
   raise EACRException.Create(10102,ErrorLCannotSetPosition,
...
//------------------------------------------------------------------------------
function TACRCompressedBLOBStream.Write(const Buffer; Count: Longint): Longint;
...
//------------------------------------------------------------------------------
function TACRCompressedBLOBStream.Seek(Offset: Longint; Origin: Word): Longint;
var NewPosition: Int64;
begin
 NewPosition := FPosition;
 case (Origin) of
...
{$IFDEF D6H}
function TACRCompressedBLOBStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
var NewPosition: Int64;
begin
 NewPosition := FPosition;
 case (Origin) of
...
procedure GetDefaultTempFileName;
begin
  ACRDefaultTempDir := ACRGetDefaultTempDir;
...
begin
  ...
  l := GetStrLength(PAnsiChar(@ACRDefaultTempDir[1]),aftWideChar);
  Move(ACRDefaultTempDir[1],TempPath[0],l);
...
begin
  // get temp file name
  s := ACRDefaultTempDir+'ACRXXXXXX';
  StrPCopy(Template, s);
//  TempFileName := AllocMem(MAX_PATH);
  TempFileName := mktemp(Template);
  if TempFileName = nil then
    raise EACRException.Create(40018,ErrorRCannotGetTempFileName);
...
begin
  ...
  if (WriteBytes <> DataSize) then
    raise EACRException.Create(ErrorCode,ErrorLCannotWriteToStream,
...
begin
  ...
  if (ReadBytes <> DataSize) then
    raise EACRException.Create(ErrorCode,ErrorLCannotReadFromStream,
...
//------------------------------------------------------------------------------
procedure SaveCryptoParamsToStream(const CryptoParams:      TACRCryptoParams;
                                   Stream:                  TStream;
                                   ErrorCode:               Integer;
...
begin
 try
  ...
  SaveDataToStream(b,SizeOf(b),Stream,11157);
   ...
   on e: Exception do
    raise EACRException.Create(ErrorCode,ErrorLCannotSaveCryptoParams,[e.Message]);
 end;
...
//------------------------------------------------------------------------------
procedure LoadCryptoParamsFromStream(var CryptoParams:        TACRCryptoParams;
                                     Stream:                  TStream;
                                     ErrorCode:               Integer;
                                     DoNotSaveKeyAndPassword: Boolean = false);
var Len: Integer;
...
begin
 try
  ...
  CryptoParams.UseInitVector := b;
   ...
   on e: Exception do
    raise EACRException.Create(ErrorCode,ErrorLCannotLoadCryptoParams,[e.Message]);
 end;
...
end; // SaveTACRWideStringListToStream
...
end; // LoadTACRWideStringListFromStream
...
begin
 ...
 if (Stream.Position <> NewPosition) then
  raise EACRException.Create(ErrorCode,ErrorLCannotSetPosition,
...
begin
  ...
  for i:=0 to High(ACRCompressionAlgorithmNames) do
    if (ACRCompressionAlgorithmNames[i] = Name) then
      begin
        Result := TACRCompressionAlgorithm(i);
        Exit;
      end;
...
function GetCompressionAlgorithmSQLName(CompressionAlgorithm: TACRCompressionAlgorithm): AnsiString;
begin
  Result := ACRCompressionAlgorithmNames[Integer(CompressionAlgorithm)];
end; // GetCompressionAlgorithmSQLName

{$IFNDEF D12H}
// from Delphi 2009 SysUtils.pas
function ACRFastFileExists(const FileName: WideString): Boolean;
{$IFDEF MSWINDOWS}
...
{$IFDEF D12H}
 Result := SysUtils.FileExists(WideString(FileName));
{$ELSE}
 Result := ACRFastFileExists(FileName);
{
...
// return true if file exists
//------------------------------------------------------------------------------
function ACRFileExists(FileNameA: PAnsiChar; FileNameW: PWideChar = nil): Boolean;
begin
 if (FileNameW = nil) then
  Result := ACRFileExistsAnsi(FileNameA)
 else
  Result := ACRFileExistsUnicode(FileNameW);
end;
...
begin
 Result := False;
 if (FileNameW <> '') then
   Result := ACRFileExists(nil,@FileNameW[1])
 else
 if (FileNameA <> '') then
   Result := ACRFileExists(@FileNameA[1],nil);
end;
...
aaWriteToLog('ACRCompression> initialized');
{$ENDIF}
