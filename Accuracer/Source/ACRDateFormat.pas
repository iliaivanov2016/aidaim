     ...
     dfAMPM:   begin
                if WideUpperCase(copy(str,start,2))='PM' then
                 ...
                 if WideUpperCase(copy(str,start,2))<>'AM' then
                  raise EACRException.Create(30129, ErrorGAmPmExpected, [copy(str,start,2)]);
                Inc(Start,2);
               end;
...
begin
  if (Source = nil) then
    raise EACRException.Create(12200,ErrorLNilPointer);
  if (Self.ClassName <> Source.ClassName) then
    raise EACRException.Create(12201,ErrorLErrorInAssignInvalidClass,
...
aaWriteToLog('ACRDateFormat> init started');
{$ENDIF}
...
{$IFDEF DEBUG_LOG_INIT}
aaWriteToLog('ACRDateFormat> init finished');
{$ENDIF}
