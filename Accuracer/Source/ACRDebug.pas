...
var
    ...
    Counter11:       TACRCounter;
    Counter12:       TACRCounter;
    Counter13:       TACRCounter;
    Counter14:       TACRCounter;
    Counter15:       TACRCounter;
    Counter16:       TACRCounter;
    Counter17:       TACRCounter;
    Counter18:       TACRCounter;
    Counter19:       TACRCounter;
    Counter20:       TACRCounter;
    Counter21:       TACRCounter;
    Counter22:       TACRCounter;
    Counter23:       TACRCounter;
    Counter24:       TACRCounter;
    Counter25:       TACRCounter;
    Counter26:       TACRCounter;
    Counter27:       TACRCounter;
    Counter28:       TACRCounter;
    Counter29:       TACRCounter;
    Counter30:       TACRCounter;
    ...
    time21,time22,time23,time24,time25,time26,time27,time28,time29,time30: TACRTestTime;
...
procedure aaWriteToLog(s : AnsiString; FileName: AnsiString = ''; SkipPrefix: Boolean = False); // writes AnsiString to log file
...
//------------------------------------------------------------------------------
// get time difference even if time was reste once
//------------------------------------------------------------------------------
...
function aaGetTickCount: Cardinal;
begin
  Result := Cardinal(Windows.GetTickCount);
end; // aaGetTickCount
{$ENDIF}
...
begin
 if (timeStarted) then
  Result := ACRGetTickCountDiff(aaGetTickCount,startTime)
 else
  Result := ACRGetTickCountDiff(stopTime,startTime);
end; // aaGetTime

function aaGetTime(var TimeRec: TACRTestTime): Cardinal;
begin
 if (TimeRec.timeStarted) then
  Result := ACRGetTickCountDiff(aaGetTickCount,TimeRec.startTime)
 else
  Result := ACRGetTickCountDiff(TimeRec.stopTime,TimeRec.startTime);
end; // aaGetTime
...
begin
 if (timeRestarted) then
  ...
  if (not timeStarted) then
   begin
    startTime := startTime + ACRGetTickCountDiff(aaGetTickCount,stopTime);
   end;
end;
...
begin
 if (TimeRec.timeRestarted) then
  ...
  if (not TimeRec.timeStarted) then
   begin
    TimeRec.startTime := TimeRec.startTime + ACRGetTickCountDiff(aaGetTickCount,TimeRec.stopTime);
   end;
end;
...
// '.txt';
 ...
 Result := StringReplace(logFileName,'.txt','_pr_'+IntToStr(GetCurrentProcessId)+'_th_'+IntToStr(GetCurrentThreadId)+'.txt',[rfReplaceAll,rfIgnoreCase]);
 {$ELSE}
 Result := StringReplace(logFileName,'.txt','_pr_'+IntToStr(GetCurrentProcessId)+'.txt',[rfReplaceAll,rfIgnoreCase]);
 {$ENDIF}
...
procedure aaWriteToLog(s : AnsiString; FileName: AnsiString; SkipPrefix: Boolean);
var
    s1:   AnsiString;
    name: AnsiString;
    fpos: Int64;
...
//  FileSeek(fh,0,soFromEnd);
  fpos := 0;
  Int64Rec(fpos).Lo := SetFilePointer(THandle(fh), Int64Rec(fpos).Lo,
    @Int64Rec(fpos).Hi,FILE_END);
...
begin
 ...
 aaInitCounter(Counter11,'counter11');
 aaInitCounter(Counter12,'counter12');
 aaInitCounter(Counter13,'counter13');
 aaInitCounter(Counter14,'counter14');
 aaInitCounter(Counter15,'counter15');
 aaInitCounter(Counter16,'counter16');
 aaInitCounter(Counter17,'counter17');
 aaInitCounter(Counter18,'counter18');
 aaInitCounter(Counter19,'counter19');
 aaInitCounter(Counter20,'counter20');
 aaInitCounter(Counter21,'counter21');
 aaInitCounter(Counter22,'counter22');
 aaInitCounter(Counter23,'counter23');
 aaInitCounter(Counter24,'counter24');
 aaInitCounter(Counter25,'counter25');
 aaInitCounter(Counter26,'counter26');
 aaInitCounter(Counter27,'counter27');
 aaInitCounter(Counter28,'counter28');
 aaInitCounter(Counter29,'counter29');
 aaInitCounter(Counter30,'counter30');
end;
...
aaWriteToLog('time21 = '+IntToStr(aaGetTime(time21)));
aaWriteToLog('time22 = '+IntToStr(aaGetTime(time22)));
aaWriteToLog('time23 = '+IntToStr(aaGetTime(time23)));
aaWriteToLog('time24 = '+IntToStr(aaGetTime(time24)));
aaWriteToLog('time25 = '+IntToStr(aaGetTime(time25)));
aaWriteToLog('time26 = '+IntToStr(aaGetTime(time26)));
aaWriteToLog('time27 = '+IntToStr(aaGetTime(time27)));
aaWriteToLog('time28 = '+IntToStr(aaGetTime(time28)));
aaWriteToLog('time29 = '+IntToStr(aaGetTime(time29)));
aaWriteToLog('time30 = '+IntToStr(aaGetTime(time30)));
end;
...
aaWriteToLog('counter11 = '+IntToStr(aaGetCounter(counter11)));
aaWriteToLog('counter12 = '+IntToStr(aaGetCounter(counter12)));
aaWriteToLog('counter13 = '+IntToStr(aaGetCounter(counter13)));
aaWriteToLog('counter14 = '+IntToStr(aaGetCounter(counter14)));
aaWriteToLog('counter15 = '+IntToStr(aaGetCounter(counter15)));
aaWriteToLog('counter16 = '+IntToStr(aaGetCounter(counter16)));
aaWriteToLog('counter17 = '+IntToStr(aaGetCounter(counter17)));
aaWriteToLog('counter18 = '+IntToStr(aaGetCounter(counter18)));
aaWriteToLog('counter19 = '+IntToStr(aaGetCounter(counter19)));
aaWriteToLog('counter20 = '+IntToStr(aaGetCounter(counter20)));
aaWriteToLog('counter21 = '+IntToStr(aaGetCounter(counter21)));
aaWriteToLog('counter22 = '+IntToStr(aaGetCounter(counter22)));
aaWriteToLog('counter23 = '+IntToStr(aaGetCounter(counter23)));
aaWriteToLog('counter24 = '+IntToStr(aaGetCounter(counter24)));
aaWriteToLog('counter25 = '+IntToStr(aaGetCounter(counter25)));
aaWriteToLog('counter26 = '+IntToStr(aaGetCounter(counter26)));
aaWriteToLog('counter27 = '+IntToStr(aaGetCounter(counter27)));
aaWriteToLog('counter28 = '+IntToStr(aaGetCounter(counter28)));
aaWriteToLog('counter29 = '+IntToStr(aaGetCounter(counter29)));
aaWriteToLog('counter30 = '+IntToStr(aaGetCounter(counter30)));
end;
...
// logFileName := aaGetLogFileName;
 ...
 logFileName := ExtractFilePath(ParamStr(0)) + 'acr_log_'+ExtractFileName(ParamStr(0))+
 '.txt';
