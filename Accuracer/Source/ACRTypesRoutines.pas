...
//  Return DataSize
function GetDataSizeInMemory(DataType: TACRBaseFieldType; Length: Integer = 0): Integer; overload;

//  Return DataSize
function GetDataSizeInMemory(DataType: TACRBaseFieldType; Data: Pointer): Integer; overload;

// Allocate Memory and Set Data
procedure AllocMemAndSetData(DataType: TACRBaseFieldType; const Value; out Buffer; out BufferSize: Integer);
...
//  Return DataSize
//------------------------------------------------------------------------------
function GetDataSizeInMemory(DataType: TACRBaseFieldType; Length: Integer): Integer;
begin
  case DataType of
    bftChar,            // AsciZ
    ...
    bftDate:            Result := SizeOf(TACRDate);
    bftTime:            Result := SizeOf(TACRTime);
    bftDateTime:        Result := SizeOf(TACRDateTime);
...
{$ENDIF}
    ...
    bftLogical:         Result := SizeOf(TACRLogical);
    bftCurrency:        Result := SizeOf(TACRCurrency);
    bftBytes:           Result := Length;
    bftVarBytes:        Result := Length;
    else
      raise EACRException.Create(30050, ErrorGUnknownDataType, [BftToStr(DataType)]);
  end;
...
// Allocate Memory and Set Data
//------------------------------------------------------------------------------
procedure AllocMemAndSetData(DataType: TACRBaseFieldType;
                             const Value;
                             out Buffer;
...
// commented in 4.40
    ...
    if Pointer(Buffer) <> nil then
      raise EACRException.Create(30054, ErrorGCannotCopyDataIntoNewMemory);
  end;
...
aaWriteToLog('ACRTypesRoutines> initialized');
{$ENDIF}
