...
BOOL CDataDialog::OnInitDialog()
{
	...
	pBoxOne->SetWindowText("UPDATE Customer SET CustNo=CustNo+1");
...
BEGIN_MESSAGE_MAP(CDataDialog, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_EXECSQL, OnBnClickedExecsql)
END_MESSAGE_MAP()
...
void CDataDialog::OnBnClickedExecsql()
{
 CEdit* pBoxOne;
 char sqlStr[601];
 pBoxOne = (CEdit*) GetDlgItem(IDC_EDIT1);
 pBoxOne->GetWindowText(sqlStr, 600);
 m_prs->m_pDatabase->ExecuteSQL(sqlStr);
