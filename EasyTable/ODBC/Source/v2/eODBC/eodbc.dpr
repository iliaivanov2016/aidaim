...
// LogFileName := ExtractFilePath(LogFileName)+'MemCheck.log';
 LogFileName := 'S:\DelphiProjects\_eodbc-test\eodbc.log';
...
if Stmt.Result.ReadOnly
then WriteToLog('    ReadOnly=True')
else WriteToLog('    ReadOnly=False');
...
Function SQLNumResultCols (
             Stmt:PStmt;//HSTMT;
        		 ColumnCount:PInteger// {UNALIGNED} SWORD
                          ): RETCODE; stdcall;
begin
...
Function SQLGetInfo  (
      Connection:PConnection;//HDBC;
		  ...
		  Info:PTR;
		  BufferLength:SWORD;
      InfoLength: pInteger //{UNALIGNED} SWORD
                      ): RETCODE; stdcall;
...
ODBC_REMOVE_DSN: Remove an existing data source.
 ...
 AddDSN: // Adding DSN
  ...
  if DatabasePath='' then
    ...
    if not SQLWritePrivateProfileString (pchar(DSNstr),pchar('Path'),pchar(DatabasePath),FileName) // Write DatabasePath in Windows NT / Windows 2000
          ...
          if not (DSNstr=DSNsetupForm.DSN.Text) then // Name was changed
            begin
              if not SQLRemoveDSNFromIni(pchar(DSNstr)) then // Delete old DSN
                begin
                  Result:=False; // Deleting was not successful
                  DSNsetupForm.Free;
                  goto Finish;
                end;
              DSNstr:=DSNsetupForm.DSN.Text;
