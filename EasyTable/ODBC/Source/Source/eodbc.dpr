...
// Driver
  ...
  DRIVER_VERSION            = '02.00.00'; // v.2.00 EasyTable 8.00
...
if DataType^=SQL_BIT then // logical type
  DecimalDigits^ := Stmt.TypeInfo.Fields[2].AsInteger;
end;
...
WriteToLog('    SQL_DOUBLE');
            if Stmt.Result.Fields.Fields[Stmt.Bind.FieldByName('ColumnNumber').AsInteger-1].DataType <> ftBoolean then
              ...
              if Stmt.Result.Fields.Fields[Stmt.Bind.FieldByName('ColumnNumber').AsInteger-1].AsBoolean then
                double8:= -1 // True
              else
                double8:= 0; // False
WriteToLog('    Got');
...
// LOGICAL = BIT
  ...
  Stmt.TypeInfo.FieldByName('DATA_TYPE').AsInteger := SQL_BIT; //SQL_SMALLINT; //SQL_INTEGER;
