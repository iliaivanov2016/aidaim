...
//-----------------------------------------------------//
//                                                     //
//  Modified by AidAim Software, 2002                  //
//                                                     //
//-----------------------------------------------------//
...
var
  ...
  ToOemChars,ToAnsi_Chars: array[0..255] of AnsiChar;
//--- Turkish support ---
  TurkishToAnsi_Chars: array [0..255] of AnsiChar=
...
asm
        ...
        PUSH    EAX
        JMP     @@1
@@0:    MOV     CL,BYTE PTR [EDX+ToAnsi_Chars]
        MOV     BYTE PTR [EAX],CL
        INC     EAX
...
{$IFNDEF X64_ON}
  ...
  if (LCID <> 1055) then
   for i:=0 to 255 do
    begin
     ...
     ToAnsi_Chars[i] := AnsiChar(chr(n));
    end
  else
  // Turkish charset
  for i:=0 to 255 do
    ToAnsi_Chars[i] := TurkishToAnsi_Chars[i];

  // ToOemChars
  for i:=0 to 255 do
   ToOemChars[byte(ToAnsi_Chars[i])] := AnsiChar(chr(i));
  // check is table valid
  for i:=0 to 255 do
   begin
    bCodePagesInitialized := false;
    for j:=0 to 255 do
     if (ToAnsi_Chars[j] = AnsiChar(chr(i))) then
      begin
  ...
  if (not bCodePagesInitialized) then
   begin
    ...
    OemToCharBuffA(PAnsiChar(@ToUpperChars),PAnsiChar(@ToAnsi_Chars),256);
   ...
   begin
    for i:=0 to 255 do
     begin
      ...
      ToUpperChars[byte(ToAnsi_Chars[i])] := ToAnsi_Chars[byte(p1^)];
      p1^ := AnsiChar(chr(i));
      // Turkish charset?
      if (LCID <> 1055) then
       CharLowerBuffA(p1,1)
      ...
      ToLowerChars[byte(ToAnsi_Chars[i])] := ToAnsi_Chars[byte(p1^)];
     end;
   end;
  FreeMem(p1);
  FreeMem(p2);
