...
interface
uses SysUtils, math, windows, ETblExcept
//,DateUtils
;
...
const
    ...
    DateFormatReservedWords: array[0..DateFormatReservedWordsCount-1] of AnsiString = (
                         'YYYY', 'YY', 'YEAR',
                         'Q',
                         'MONTH', 'MON', 'MM', 'M', 'RM',
                         'DDD', 'DD', 'DAY', 'DY', 'DW', 'D',
...
Type
  ...
  private
   ...
   procedure ParseDateFormat(const DateFormatStr: AnsiString);
   function GetReservedWord(const Text: AnsiString): TDateFormatType;
  public
   constructor Create(DateFormat: AnsiString);
   destructor Destroy; override;

   function DebugGetTokenList: AnsiString;
   ...
   function ToDate(str: AnsiString): TDateTime;
   function ToString(dt: TDateTime): AnsiString;
...
constructor TDateFormater.Create(DateFormat: AnsiString);
begin
...
procedure TDateFormater.ParseDateFormat(const DateFormatStr: AnsiString);
var
  i,j,L: integer;
  start: integer;
  s: AnsiString;
...
function TDateFormater.GetReservedWord(const Text: AnsiString): TDateFormatType; 
var i,n: integer;
    s: AnsiString; 
begin
...
function TDateFormater.DebugGetTokenList: AnsiString;
var
 i: Integer;
begin
 Result := '';
...
begin
{
 ...
 while (Start <= l) and (n < 4) do
  begin
   if (not (Str[Start] in ['I','V','X'])) then
    break;
   Inc(Start);
   Inc(n);
  end;
...
function TDateFormater.ToDate(str: AnsiString): TDateTime;
var
 y,m,d,h,mi,s,z: word;
 i, l, start: Integer;
begin
 ...
 start := 1;
  ...
  begin
   case FDateFormat[i].TokenType of
     ...
     dfAMPM:   begin
                if AnsiUpperCase(copy(str,start,2))='PM' then
                 h := h + 12
                else
                 if AnsiUpperCase(copy(str,start,2))<>'AM' then
                  raise ETblException.Create(02146,[copy(str,start,2)],nil);
                Inc(Start,2);
               end;
...
function TDateFormater.ToString(dt: TDateTime): AnsiString;
var
 i: Integer;
 s: AnsiString;
begin
