...
select * from memory test1 FULL JOIN memory test2 ON (test1.state = test2.state) order by id;
#Paradox
#exec
drop table test1;
#exec
...
select * from test1 FULL JOIN test2 ON (test1.state = test2.state) order by id;
