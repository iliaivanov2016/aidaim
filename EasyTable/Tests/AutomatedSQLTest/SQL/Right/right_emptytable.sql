...
select * from memory test2 RIGHT JOIN memory test1 ON (test1.state = test2.state) order by id;
#DBISAM
#exec
...
select * from "\memory\test2" RIGHT JOIN "\memory\test1" ON (test1.state = test2.state) order by id;
