...
select * from memory test1 RIGHT JOIN memory test2 ON (test1.state = test2.state) order by id;
#DBISAM
#exec
...
select * from "\memory\test1" RIGHT JOIN "\memory\test2" ON (test1.state = test2.state) order by id;
