        ...
        if (Users[i].Status == msgOnLine)
         {
		  ...
		  sgConnectedUsers->Cells[2][nc] = (AnsiString)Users[i].Host;
          sgConnectedUsers->Cells[3][nc] = IntToStr((int)Users[i].Port);
         }
        na++;
       ...
       if (Clients[i].UserID == MSG_INVALID_USER_ID)
        {
		  ...
		  sgConnectedUsers->Cells[2][nc] = (AnsiString)Clients[i].Host;
		  sgConnectedUsers->Cells[3][nc] = IntToStr((int)Clients[i].Port);
		  na++;
          if (na >= sgAllUsers->RowCount)
           sgAllUsers->RowCount = na+1;
          sgAllUsers->Cells[0][na] = "+";
