   ...
   if (sgUsers->Cells[11][i] != "")
    {
     try
     {
       ...
       if (Code != MSG_COMMAND_OK)
        {
         ShowMessage("Cannot add selected user(s) to contact list-> Error code = "+IntToStr((int)Code));
         FClose = false;
         return;
        }
     }
...
*/
  if (Code != MSG_COMMAND_OK)
    ShowMessage("Cannot find users-> Error code = "+IntToStr((int)Code));
  else
  {
   gbUsers->Caption = " Users found: "+IntToStr((int)Users.Length)+" ";
   ...
   for (i = 0; i < numUsers; i++)
    {
     switch (Users[i].Status)
     {
      ...
      default:         sgUsers->Cells[1][i+1] = "";
		 ...
		 sgUsers->Cells[2][i+1] = IntToStr((int)Integer(Users[i].UserID));
		 sgUsers->Cells[3][i+1] = (AnsiString)Users[i].UserName;
		 sgUsers->Cells[4][i+1] = (AnsiString)Users[i].FirstName;
		 sgUsers->Cells[5][i+1] = (AnsiString)Users[i].LastName;
		 sgUsers->Cells[6][i+1] = (AnsiString)Users[i].Organization;
		 sgUsers->Cells[7][i+1] = (AnsiString)Users[i].Department;
		 sgUsers->Cells[8][i+1] = (AnsiString)Users[i].Application;
		 sgUsers->Cells[9][i+1] = (AnsiString)Users[i].Host;
     sgUsers->Cells[10][i+1] = IntToStr((int)Users[i].Port);
...
if ((ARow >= 1) && (ARow < sgUsers->RowCount)) 
  if (ACol == 0)
   {
		if (sgUsers->Cells[2][ARow] == IntToStr((int)fmMain->MsgClient1->UserID))
     CanSelect = false;
    else
     {
      if (sgUsers->Cells[11][ARow] == "")
