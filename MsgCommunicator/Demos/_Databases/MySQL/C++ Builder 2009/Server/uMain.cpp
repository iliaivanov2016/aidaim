...
#pragma link "DBTables"
#pragma link "MsgComBase"
#pragma link "MsgDatabase"
#pragma link "MsgDatabaseMySQL"
...
{
	LocalPort->Text = IntToStr((int)MsgServer1->ConnectionParams->LocalPort);
  // grids headers
      ...
      for (i = 0; i < numUsers; i++)
       {
        if (Users[i].Status == msgOnLine) 
          ...
          if (nc >= sgConnectedUsers->RowCount) 
           sgConnectedUsers->RowCount = nc+1;
					sgConnectedUsers->Cells[0][nc] = IntToStr((int)Users[i].UserID);
					sgConnectedUsers->Cells[1][nc] = (AnsiString)Users[i].UserName;
					sgConnectedUsers->Cells[3][nc] = (AnsiString)Users[i].Host;
          sgConnectedUsers->Cells[4][nc] = IntToStr((int)Users[i].Port);
         }
        na++;
        ...
        if (Users[i].Status == msgOnLine)
         ...
         sgAllUsers->Cells[0][na] = "-";
				sgAllUsers->Cells[1][na] = IntToStr((int)Users[i].UserID);
				sgAllUsers->Cells[2][na] = (AnsiString)Users[i].UserName;
				sgAllUsers->Cells[3][na] = (AnsiString)Users[i].Department;
				sgAllUsers->Cells[4][na] = (AnsiString)Users[i].Host;
        sgAllUsers->Cells[5][na] = IntToStr((int)Users[i].Port);
       }
       ...
       if (Clients[i].UserID == MSG_INVALID_USER_ID)
        {
          ...
          if (nc >= sgConnectedUsers->RowCount)
           sgConnectedUsers->RowCount = nc+1;
					sgConnectedUsers->Cells[0][nc] = IntToStr((int)MSG_INVALID_USER_ID);
					sgConnectedUsers->Cells[1][nc] = Guest;
					sgConnectedUsers->Cells[3][nc] = (AnsiString)Clients[i].Host;
					sgConnectedUsers->Cells[4][nc] = IntToStr((int)Clients[i].Port);
					na++;
					if (na >= sgAllUsers->RowCount)
					 sgAllUsers->RowCount = na+1;
					sgAllUsers->Cells[0][na] = "+";
					sgAllUsers->Cells[1][na] = IntToStr((int)MSG_INVALID_USER_ID);
					sgAllUsers->Cells[2][na] = Guest;
					sgAllUsers->Cells[3][na] = "";
					sgAllUsers->Cells[4][na] = (AnsiString)Clients[i].Host;
					sgAllUsers->Cells[5][na] = IntToStr((int)Clients[i].Port);
        }
...
void __fastcall TfmMain::ServerSendClick(TObject *Sender)
{
	...
	if (ServerToID->Text == "")
		return;
	MsgServer1->SendMessage(Cardinal(StrToInt(ServerToID->Text)), (WideString)ServerMsg->Text);
	...
	if ((AnsiString)UserInfo.UserName != "")
		str = (WideString)(AnsiString)UserInfo.UserName + " " + str;
  ...
  if (sgAllUsers->Cells[1][ARow] != "")
   {
    ...
    if (sgAllUsers->Cells[1][ARow] != IntToStr((int)MSG_INVALID_USER_ID))
      ...
      int ACol, int ARow, bool &CanSelect)
{
  ...
  if (sgConnectedUsers->Cells[0][ARow] != "") 
   {
    if (sgConnectedUsers->Cells[0][ARow] != IntToStr((int)MSG_INVALID_USER_ID))
     {
...
//---------------------------------------------------------------------------
void __fastcall TfmMain::MsgServer1ReceiveUnicodeTextMessage(const DWORD FromUserID,
