...
uses
  ...
  MsgClient, MsgComBase, MsgConst, MsgTypes, ACRMain, MsgDatabase,
  MsgCriticalSection,
  {$IFDEF DEBUG_LOG}
  MsgDebug,
  {$ENDIF}
  MsgDatabaseAccuracer, ComCtrls, uStart, uRegister, uLogin, uFind, uHistory;

type
  TfmMain = class(TForm)
    LocalDatabaseAccuracer: TACRDatabase;
    MsgClient1: TMsgClient;
    MsgDatabaseAccuracer1: TMsgDatabaseAccuracer;
    MsgTempTableAccuracer1: TMsgTempTableAccuracer;
    ...
    procedure MsgClient1ReceiveTextMessage(const FromUserID: Cardinal;
      const SendingDate, DeliveryDate: TDateTime; const Text: AnsiString);
    ...
    procedure MsgClient1ReceiveUnicodeTextMessage(const FromUserID: Cardinal;
...
begin
  ...
  if (FStarting) then
   ...
   begin
    dt := TClientDisplayThread.Create(Capt);
    dt.Resume;
   end;
end;
...
begin
  ...
  try
   ...
   for i := 0 to lbContacts.Items.Count-1 do
    ...
    cdt := TClientDisplayThread.Create(Capt);
    cdt.Resume;
...
begin
 if (FTemp.Count <> 0) then
  begin
   ...
   dt := TClientDisplayThread.Create(FTemp.Text);
   dt.Resume;
   FTemp.Clear;
