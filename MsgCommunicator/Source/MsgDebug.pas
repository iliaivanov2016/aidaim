//==============================================================================
// Unit name: MsgDebug
// Copyright 2000-2009 AidAim Software.
...
var
    ...
    DefaultCounter: TMsgCounter;
    Counter1:       TMsgCounter;
    Counter2:       TMsgCounter;
    Counter3:       TMsgCounter;
    Counter4:       TMsgCounter;
    Counter5:       TMsgCounter;
    Counter6:       TMsgCounter;
    Counter7:       TMsgCounter;
    Counter8:       TMsgCounter;
    Counter9:       TMsgCounter;
    Counter10:       TMsgCounter;

    TimeNo: Integer;
    time1,time2,time3,time4,time5: TMsgTestTime;
    time6,time7,time8,time9,time10: TMsgTestTime;
    time11,time12,time13,time14,time15: TMsgTestTime;
    time16,time17,time18,time19,time20: TMsgTestTime;
...
function aaGetTime(var TimeRec: TMsgTestTime): Integer; overload;
// inits time counting
procedure aaInitTime; overload;
procedure aaInitTime(var TimeRec: TMsgTestTime); overload;
// starts time counting from current time
procedure aaStartTime; overload;
procedure aaStartTime(var TimeRec: TMsgTestTime); overload;
// stops time counting
procedure aaStopTime; overload;
procedure aaStopTime(var TimeRec: TMsgTestTime); overload;
// shows time
procedure aaShowTime; overload;
procedure aaShowTime(var TimeRec: TMsgTestTime); overload;
// write time to log
procedure aaWriteTime; overload;
procedure aaWriteTime(var TimeRec: TMsgTestTime); overload;
...
procedure aaInitCounter(var Counter: TMsgCounter; Name: AnsiString); overload;
procedure aaIncCounter(var Counter: TMsgCounter; Increment: Int64 = 1);  overload;
procedure aaDecCounter(var Counter: TMsgCounter; Decrement: Int64 = 1);  overload;
function aaGetCounter(var Counter: TMsgCounter): Int64;  overload;
procedure aaSetCounter(var Counter: TMsgCounter; Value: Int64);  overload;
procedure aaShowCounter(var Counter: TMsgCounter);  overload;
...
//------------------------------------------------------------------------------
function MsgGetTickCountDiff(NewTime, OldTime: Cardinal): Cardinal;
const MsgMaxCardinal: Cardinal = $FFFFFFFF;
begin
  if (NewTime >= OldTime) then
   Result := NewTime - OldTime
  else
   Result := Cardinal((MsgMaxCardinal-OldTime+1)+NewTime);
end; // MsgGetTickCountDiff
...
begin
 if (timeStarted) then
  Result := MsgGetTickCountDiff(aaGetTickCount,startTime)
 else
  Result := MsgGetTickCountDiff(stopTime,startTime);
end; // aaGetTime

function aaGetTime(var TimeRec: TMsgTestTime): Integer;
begin
 if (TimeRec.timeStarted) then
  Result := MsgGetTickCountDiff(aaGetTickCount,TimeRec.startTime)
 else
  Result := MsgGetTickCountDiff(TimeRec.stopTime,TimeRec.startTime);
end; // aaGetTime
...
// inits time counting
procedure aaInitTime(var TimeRec: TMsgTestTime);
begin
...
begin
 if (timeRestarted) then
  ...
  if (not timeStarted) then
   begin
    startTime := startTime + MsgGetTickCountDiff(aaGetTickCount,stopTime);
   end;
end;


// starts time counting from current time
procedure aaStartTime(var TimeRec: TMsgTestTime);
begin
 if (TimeRec.timeRestarted) then
  ...
  if (not TimeRec.timeStarted) then
   begin
    TimeRec.startTime := TimeRec.startTime + MsgGetTickCountDiff(aaGetTickCount,TimeRec.stopTime);
   end;
end;
...
// stops time counting
procedure aaStopTime(var TimeRec: TMsgTestTime);
begin
 TimeRec.timeStarted := false;
...
// shows time
procedure aaShowTime(var TimeRec: TMsgTestTime);
begin
...
procedure aaWriteTime(var TimeRec: TMsgTestTime);
begin
...
// Result := LogPath + 'log_'+ExtractFileName(ParamStr(0))+
 ...
 Result := ExtractFilePath(ParamStr(0)) + 'msg_log_'+ExtractFileName(ParamStr(0))+
 '.txt';
...
procedure aaInitCounter(var Counter: TMsgCounter; Name: AnsiString);
begin
 Counter.Name := Name;
 Counter.Value := 0;
end;

procedure aaIncCounter(var Counter: TMsgCounter; Increment: Int64 = 1);
begin
...
procedure aaDecCounter(var Counter: TMsgCounter; Decrement: Int64 = 1);
begin
 Dec(Counter.Value, Decrement);
end;

function aaGetCounter(var Counter: TMsgCounter): Int64;
begin
 Result := Counter.Value;
end; //


procedure aaSetCounter(var Counter: TMsgCounter; Value: Int64);
begin
 Counter.Value := Value;
end;


procedure aaShowCounter(var Counter: TMsgCounter);
begin
...
function aaGetCurrentTimeAsString: AnsiString;
begin
 Result := IntToStr(aaGetTickCount)+' - '+'PrID = '+IntToStr(GetCurrentProcessId)+', ThID = '+IntToStr(GetCurrentThreadId)+': ';
end;
