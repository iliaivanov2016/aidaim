...
type
  ...
  TSFSHugeFilePartitionHeader = record
...
const
   HFPHeaderSize = sizeof(TSFSHugeFilePartitionHeader);
...
type
 ...
 public
  ...
  function CopyFrom(Source: TSFSAbstractFile; Count: Int64): Int64;
  // close file
...
// Disk file
//------------------------------------------------------------------------------
 TSFSDiskFile=class(TSFSAbstractFile)
 private
...
// Memory file
//------------------------------------------------------------------------------
 TSFSMemoryFile=class(TSFSAbstractFile)
 private
...
//------------------------------------------------------------------------------
 ...
 private
  ...
  PartFiles:      array of TSFSAbstractFile; // list of partition files
  ...
  // init part header
  procedure InitPartHeader(var HFPartHeader: TSFSHugeFilePartitionHeader);
  // append part file
  ...
  procedure CalculateStubSize(FFile: TSFSAbstractFile);
 ...
 public
  ...
  destructor Destroy; override;
  // copy form source file
  function CopyFrom(Source: TSFSHugeFile; Count: Int64): Int64;
  // save to stream
...
// set exclusive mode
//------------------------------------------------------------------------------
procedure TSFSAbstractFile.SetExclusive(value: boolean);
begin
 FExclusive := value;
end;// TSFSAbstractFile.SetExclusive


//------------------------------------------------------------------------------
// get file position
//------------------------------------------------------------------------------
function TSFSAbstractFile.GetPosition: Int64;
begin
  Result := Seek(0, 1);
end;// GetPosition
...
// set file position
//------------------------------------------------------------------------------
procedure TSFSAbstractFile.SetPosition(Pos: Int64);
begin
  Seek(Pos, 0);
end;// SetPosition
...
// set read-only mode
//------------------------------------------------------------------------------
procedure TSFSAbstractFile.SetReadOnly(value: boolean);
begin
 FReadOnly := value;
end;// TSFSAbstractFile.SetReadOnly


//------------------------------------------------------------------------------
// get file size
//------------------------------------------------------------------------------
function TSFSAbstractFile.GetSize: Int64;
var
  Pos: Longint;
begin
  Pos := Seek(0, 1);
  Result := Seek(0, 2);
  Seek(Pos, 0);
end;// TSFSAbstractFile.GetSize


//------------------------------------------------------------------------------
// constructor
//------------------------------------------------------------------------------
constructor TSFSAbstractFile.Create(FileName: AnsiString; bReadOnly, bExclusive: boolean);
begin
 FFileName := FileName;
 FReadOnly := bReadOnly;
 FExclusive := bExclusive;
end;// TSFSAbstractFile.Create


//------------------------------------------------------------------------------
// copy form source file
//------------------------------------------------------------------------------
function TSFSAbstractFile.CopyFrom(Source: TSFSAbstractFile; Count: Int64): Int64;
const
  MaxBufSize = $F000;
var
  BufSize, N: Integer;
...
end;// TSFSAbstractFile.CopyFrom


//------------------------------------------------------------------------------
// read known bytes from file
//------------------------------------------------------------------------------
procedure TSFSAbstractFile.ReadBuffer(var Buffer; Count: Longint);
begin
  if (Count <> 0) and (Read(Buffer, Count) <> Count) then
    raise Exception.Create('TSFSAbstractFile.ReadBuffer - Stream read error');
end;// TSFSAbstractFile.ReadBuffer


//------------------------------------------------------------------------------
// write known bytes into the file
//------------------------------------------------------------------------------
procedure TSFSAbstractFile.WriteBuffer(const Buffer; Count: Longint);
begin
  if (Count <> 0) and (Write(Buffer, Count) <> Count) then
    raise Exception.Create('TSFSAbstractFile.WriteBuffer - Stream write error');
end;// TSFSAbstractFile.WriteBuffer
...
begin
 ...
 Open(false);
 Position := pos;
end;// TSFSDiskFile.Reopen


//------------------------------------------------------------------------------
// constructor
//------------------------------------------------------------------------------
constructor TSFSDiskFile.Create(FileName: AnsiString; bReadOnly, bExclusive: boolean);

begin
 inherited Create(FileName, bReadOnly, bExclusive);
 FDirectory := ExtractFilePath(FileName);
 FHandle := -1;
end;// TSFSDiskFile.Create


//------------------------------------------------------------------------------
// destructor
//------------------------------------------------------------------------------
destructor TSFSDiskFile.Destroy;
begin
 Close;
 inherited Destroy;
end;// TSFSDiskFile.Destroy


//------------------------------------------------------------------------------
// internal open
//------------------------------------------------------------------------------
function TSFSDiskFile.InternalOpen(const FileName: AnsiString; bCreate: boolean): boolean;
const
...
//  FullFileName := FDirectory+FileName;
  ...
  if FHandle < 0 then
   Result := false;
end;// TSFSDiskFile.InternalOpen


//------------------------------------------------------------------------------
// flush file buffers
//------------------------------------------------------------------------------
procedure TSFSDiskFile.FlushBuffers;
begin
  FlushFileBuffers(FHandle);
end;// TSFSDiskFile.FlushBuffers;


//------------------------------------------------------------------------------
// set read-only mode
//------------------------------------------------------------------------------
procedure TSFSDiskFile.SetReadOnly(value: boolean);
begin
...
end;// TSFSDiskFile.SetReadOnly


//------------------------------------------------------------------------------
// set file size
//------------------------------------------------------------------------------
procedure TSFSDiskFile.SetSize(value: Int64);
begin
  Seek(value, 0);
  Win32Check(SetEndOfFile(FHandle));
end;// TSFSDiskFile.SetSize


//------------------------------------------------------------------------------
// set exclusive mode
//------------------------------------------------------------------------------
procedure TSFSDiskFile.SetExclusive(value: boolean);
begin
...
end;// TSFSDiskFile.SetExclusive


//------------------------------------------------------------------------------
// open file
//------------------------------------------------------------------------------
function TSFSDiskFile.Open(bCreate: boolean): boolean;
begin
 Result := InternalOpen(FFileName, bCreate);
end;// TSFSDiskFile.Open


//------------------------------------------------------------------------------
// close file
//------------------------------------------------------------------------------
procedure TSFSDiskFile.Close;
begin
  if FHandle >= 0 then
   begin
    FlushBuffers;
  ...
  FHandle := -1;
end;// TSFSDiskFile.Close;


//------------------------------------------------------------------------------
// read from file
//------------------------------------------------------------------------------
function TSFSDiskFile.Read(var Buffer; Count: Longint): Longint;
begin
 Result:=FileRead(FHandle,Buffer,Count);
end;// TSFSDiskFile.Read


//------------------------------------------------------------------------------
// seek in file
//------------------------------------------------------------------------------
function TSFSDiskFile.Seek(Offset: Int64; Origin: Word): Int64;
begin
{
  ...
  {$R+}
end;// TSFSDiskFile.Seek


//------------------------------------------------------------------------------
// write into the file
//------------------------------------------------------------------------------
function TSFSDiskFile.Write(const Buffer; Count: Longint): Longint;
begin
 Result := 0;
 if (Count = 0) then
...
// constructor
//------------------------------------------------------------------------------
constructor TSFSMemoryFile.Create(FileName: AnsiString; bReadOnly, bExclusive: boolean);
begin
 inherited Create(FileName, bReadOnly, bExclusive);
 FStream := nil;
end;// TSFSMemoryFile.Create


//------------------------------------------------------------------------------
// destructor
//------------------------------------------------------------------------------
destructor TSFSMemoryFile.Destroy;
begin
 Close;
 inherited Destroy;
end;// TSFSMemoryFile.Destroy


//------------------------------------------------------------------------------
// flush file buffers
//------------------------------------------------------------------------------
procedure TSFSMemoryFile.FlushBuffers;
begin
; // do nothing
end;// TSFSMemoryFile.FlushBuffers;


//------------------------------------------------------------------------------
// set file size
//------------------------------------------------------------------------------
procedure TSFSMemoryFile.SetSize(value: Int64);
begin
  FStream.Size := value;
end;// TSFSMemoryFile.SetSize


//------------------------------------------------------------------------------
// open file
//------------------------------------------------------------------------------
function TSFSMemoryFile.Open(bCreate: boolean): boolean;
begin
 if (not bCreate) then
  raise Exception.Create('TSFSMemoryFile.Open - File cannot be open, only creation is available.');
 try
...
end;// TSFSMemoryFile.Open


//------------------------------------------------------------------------------
// close file
//------------------------------------------------------------------------------
procedure TSFSMemoryFile.Close;
begin
  if FStream <> nil then
   FStream.Free;
  FStream := nil;
end;// TSFSMemoryFile.Close;


//------------------------------------------------------------------------------
// read from file
//------------------------------------------------------------------------------
function TSFSMemoryFile.Read(var Buffer; Count: Longint): Longint;
begin
 Result := FStream.Read(Buffer, Count);
end;// TSFSMemoryFile.Read


//------------------------------------------------------------------------------
// seek in file
//------------------------------------------------------------------------------
function TSFSMemoryFile.Seek(Offset: Int64; Origin: Word): Int64;
begin
  Result := FStream.Seek(Offset, Origin);
end;// TSFSMemoryFile.Seek


//------------------------------------------------------------------------------
// write into the file
//------------------------------------------------------------------------------
function TSFSMemoryFile.Write(const Buffer; Count: Longint): Longint;
begin
 Result := FStream.Write(Buffer, Count);
end;// TSFSMemoryFile.Write
...
// constructor
//------------------------------------------------------------------------------
constructor TSFSHugeFile.Create(FileName: AnsiString; bReadOnly, bExclusive, bInMemory: boolean;
                             PartitionSize: Int64=-1);
begin
 FFileName := FileName;
 FReadOnly := bReadOnly;
...
// destructor
//------------------------------------------------------------------------------
destructor TSFSHugeFile.Destroy;
begin
  FInMemory := false;
  Close;
  inherited Destroy;
...
// get partition No and offset there by global file position
//------------------------------------------------------------------------------
procedure TSFSHugeFile.GetFileNoAndOffset(Pos: Int64; var FileNo: integer; var Offset: Int64);
begin
...
// init part header
//------------------------------------------------------------------------------
procedure TSFSHugeFile.InitPartHeader(var HFPartHeader: TSFSHugeFilePartitionHeader);
begin
  // clear record
...
// append part file
//------------------------------------------------------------------------------
function TSFSHugeFile.AppendPartFile: boolean;
var
  FName, s: AnsiString;
  i: integer;
  Pos: Int64;
  HFPHeader: TSFSHugeFilePartitionHeader;
begin
  ...
  if (FInMemory) then
   PartFiles[i] := TSFSMemoryFile.Create(FName,FReadOnly,FExclusive)
  else
   PartFiles[i] := TSFSDiskFile.Create(FName,FReadOnly,FExclusive);
...
// delete last part file
//------------------------------------------------------------------------------
procedure TSFSHugeFile.DeleteLastPartFile;
var
  FName: AnsiString;
  i: integer;
  Pos: Int64;
...
begin
  if (Length(PartFiles) <= 1) then
   raise Exception.Create('TSFSHugeFile.DeleteLastPartFile - No parts to delete.');
...
// open files chain
//------------------------------------------------------------------------------
function TSFSHugeFile.OpenFilesChain(const FileName: AnsiString): boolean;
var
...
begin
  ...
  repeat
    ...
    SetLength(PartFiles, i+1);
    if (FInMemory) then
     PartFiles[i] := TSFSMemoryFile.Create(CurFolder+CurFileName, FReadOnly, FExclusive)
    else
     PartFiles[i] := TSFSDiskFile.Create(CurFolder+CurFileName, FReadOnly, FExclusive);
    // open disk file and copy it to in-memory file?
    if (FInMemory) then
     begin
       df := TSFSDiskFile.Create(CurFolder+CurFileName, FReadOnly, FExclusive);
       res := df.Open(false);
       if (res) then
        begin
...
// internal open
//------------------------------------------------------------------------------
function TSFSHugeFile.InternalOpen(const FileName: AnsiString; bCreate: boolean;
                                bIgnoreErrors: boolean=false): boolean;
var
  i: integer;
begin
  if (IsFileOpen) then
    raise Exception.Create('TSFSHugeFile.InternalOpen - File is already open.');
  ...
  if (Length(PartFiles) <> 0) then
    raise Exception.Create('TSFSHugeFile.InternalOpen - File was not closed.')
  else
  // create file?
  if (bCreate) then
   begin
...
// set exclusive mode
//------------------------------------------------------------------------------
procedure TSFSHugeFile.SetExclusive(value: boolean);
begin
 FExclusive := value;
end;// SetExclusive
...
// set in-memory mode
//------------------------------------------------------------------------------
procedure TSFSHugeFile.SetInMemory(value: boolean);
var
  PartFile: TSFSAbstractFile;
  s: AnsiString;
  i: integer;
begin
   ...
   begin
    ...
    for i := 0 to Length(PartFiles)-1 do
     begin
      ...
      // create new file
      if (value) then
       PartFile := TSFSMemoryFile.Create(s, FReadOnly, FExclusive)
      else
       PartFile := TSFSDiskFile.Create(s, FReadOnly, FExclusive);
      PartFile.Open(true);
...
// get file position
//------------------------------------------------------------------------------
function TSFSHugeFile.GetPosition: Int64;
begin
  Result := Seek(0, 1);
end;// GetPosition
...
// set file position
//------------------------------------------------------------------------------
procedure TSFSHugeFile.SetPosition(Pos: Int64);
begin
...
// set read-only mode
//------------------------------------------------------------------------------
procedure TSFSHugeFile.SetReadOnly(value: boolean);
begin
 FReadOnly := value;
end;// SetReadOnly
...
// set file size
//------------------------------------------------------------------------------
procedure TSFSHugeFile.SetSize(Value: Int64);
var
  FileNo: integer;
  Offset: Int64;
begin
...
// get file size
//------------------------------------------------------------------------------
function TSFSHugeFile.GetSize: Int64;
begin
  Result := FSize;
end;// GetSize
...
// detects stub and+ gets its size
//------------------------------------------------------------------------------
procedure TSFSHugeFile.CalculateStubSize(FFile: TSFSAbstractFile);
var
    size,offset,pos,
    i,j,k:	      Integer;
    buf:		      PAnsiChar;
   ...
   begin
    ...
    if (HFPHeader.signature = HFSignature) then
     begin
       ...
       if (name = SingleFileSystemSignature) then
        result := true;
     end;
   end;
...
// copy form source file
//------------------------------------------------------------------------------
function TSFSHugeFile.CopyFrom(Source: TSFSHugeFile; Count: Int64): Int64;
...
// save to stream
//------------------------------------------------------------------------------
procedure TSFSHugeFile.SaveToStream(Stream: TStream);
var outBytes,oldPos,oldPos1,inSize:	Int64;
    outSize:					Integer;
    buf:            	PAnsiChar;
    HFPartHeader: TSFSHugeFilePartitionHeader;
begin
...
// load from stream
//------------------------------------------------------------------------------
procedure TSFSHugeFile.LoadFromStream(Stream: TStream);
...
begin
 ...
 if (StrLComp(HFPartHeader.Signature, HFSignature, HFSignatureSize) <> 0) then
  raise Exception.Create('TSFSHugeFile.LoadFromStream - header corrupted or this is not SFS file.');
 Size := 0;
 Position := 0;
...
// flush file buffers
//------------------------------------------------------------------------------
procedure TSFSHugeFile.FlushBuffers;
var
  i: integer;
begin
  for i := 0 to Length(PartFiles)-1 do
...
// open file (with params specified in constructor)
//------------------------------------------------------------------------------
function TSFSHugeFile.Open(bCreate: boolean; bIgnoreErrors: boolean=false): boolean;
begin
...
// read from file
//------------------------------------------------------------------------------
function TSFSHugeFile.Read(var Buffer; Count: Longint): Longint;
var
  ReadSize: LongInt;
  FileNo: integer;
  Offset: Int64;
...
begin
  ReadSize := 0;
{$IFDEF DEBUG_TRACE_TSFSHugeFile_READ}
aaWriteToLog('0 TSFSHugeFile.FileRead. FPosition = '+IntToStr(FPosition)+#9+'Count = '+IntToStr(count));
{$ENDIF}
  // increase file size if required
  if (FPosition > FSize) then
   raise Exception.Create('TSFSHugeFile.Read - Stream read error.');
  // read
...
{$IFDEF DEBUG_TRACE_TSFSHugeFile_READ}
aaWriteToLog('1 TSFSHugeFile.FileRead. FPosition = '+IntToStr(FPosition)+#9+'Offset = '+IntToStr(Offset)+#9+'FileNo = '+IntToStr(FileNo));
{$ENDIF}
  ...
  Result := ReadSize;
{$IFDEF DEBUG_TRACE_TSFSHugeFile_READ}
aaWriteToLog('2 TSFSHugeFile.FileRead. Result = '+IntToStr(Result)+#9+'sz = '+IntToStr(sz)+#9+'FPosition = '+IntToStr(FPosition)+#9+'PortionSize = '+IntToStr(PortionSize));
{$ENDIF}
end;// Read
...
// read known bytes from file
//------------------------------------------------------------------------------
procedure TSFSHugeFile.ReadBuffer(var Buffer; Count: Longint);
begin
  if (Count <> 0) and (Read(Buffer, Count) <> Count) then
    raise Exception.Create('TSFSHugeFile.ReadBuffer - Stream read error');
end;// ReadBuffer
...
// seek in file
//------------------------------------------------------------------------------
function TSFSHugeFile.Seek(Offset: Int64; Origin: Word): Int64;
var
  FileNo: integer;
  FileOffset: Int64;
begin
  ...
  case Origin of
    soFromBeginning:
        if (Offset < 0) then
         raise Exception.Create('TSFSHugeFile.Seek - Offset < 0 from file beginning.')
        else
         FPosition := Offset;
    soFromCurrent:
...
// write into the file
//------------------------------------------------------------------------------
function TSFSHugeFile.Write(const Buffer; Count: Longint): Longint;
var
  WrittenSize: LongInt;
  FileNo: integer;
  Offset: Int64;
...
//    SetSize(FPosition-1);
 ...
 if (Count <= 0) then
  Exit;
{$IFDEF DEBUG_TRACE_TSFSHugeFile_WRITE}
aaWriteToLog('0 TSFSHugeFile.FileWrite. FPosition = '+IntToStr(FPosition)+#9+'Count = '+IntToStr(count));
{$ENDIF}
 ...
 if (FPosition > FSize) then
  ...
  while (WrittenSize < Count) do
   begin
     ...
     GetFileNoAndOffset(FPosition,FileNo,Offset);
{$IFDEF DEBUG_TRACE_TSFSHugeFile_WRITE}
 aaWriteToLog('1 TSFSHugeFile.FileWrite. FPosition = '+IntToStr(FPosition)+#9+'Offset = '+IntToStr(Offset)+#9+'FileNo = '+IntToStr(FileNo));
{$ENDIF}
...
// write known bytes into the file
//------------------------------------------------------------------------------
procedure TSFSHugeFile.WriteBuffer(const Buffer; Count: Longint);
begin
  if (Count <> 0) and (Write(Buffer, Count) <> Count) then
    raise Exception.Create('TSFSHugeFile.WriteBuffer - Stream write error');
end;// WriteBuffer
...
// copy file
//------------------------------------------------------------------------------
function TSFSHugeFile.CopyFile(const NewName: AnsiString): Boolean;
var
...
begin
  ...
  if (FInMemory) then
   begin
     if (Length(PartFiles) <> 1) then
       raise Exception.Create('TSFSHugeFile.CopyFile - Cannot copy memory SFS file.');
     HF := TSFSHugeFile.Create(NewName, false, true, false);
     HF.Open(true);
     i := Position;
     Position := 0;
  ...
  i := 0;
  repeat
    PartFile := TSFSDiskFile.Create(CurFileName, FReadOnly, FExclusive);
...
// rename file
//------------------------------------------------------------------------------
function TSFSHugeFile.RenameFile(const NewName: AnsiString): Boolean;
var
...
begin
  ...
  if (FInMemory) then
   begin
     if (Length(PartFiles) = 0) then
       raise Exception.Create('TSFSHugeFile.RenameFile - No memory files exist.');
...
// delete file
//------------------------------------------------------------------------------
function TSFSHugeFile.DeleteFile: Boolean;
var
  i: integer;
