...
var
    ...
    DefaultCounter: TSFSCounter;
    Counter1:       TSFSCounter;
    Counter2:       TSFSCounter;
    Counter3:       TSFSCounter;
    Counter4:       TSFSCounter;
    Counter5:       TSFSCounter;
    Counter6:       TSFSCounter;
    Counter7:       TSFSCounter;
    Counter8:       TSFSCounter;
    Counter9:       TSFSCounter;
    Counter10:       TSFSCounter;

    TimeNo: Integer;
    time1,time2,time3,time4,time5: TSFSTestTime;
    time6,time7,time8,time9,time10: TSFSTestTime;
    time11,time12,time13,time14,time15: TSFSTestTime;
    time16,time17,time18,time19,time20: TSFSTestTime;
...
function aaGetTime(var TimeRec: TSFSTestTime): Integer; overload;
// inits time counting
procedure aaInitTime; overload;
procedure aaInitTime(var TimeRec: TSFSTestTime); overload;
// starts time counting from current time
procedure aaStartTime; overload;
procedure aaStartTime(var TimeRec: TSFSTestTime); overload;
// stops time counting
procedure aaStopTime; overload;
procedure aaStopTime(var TimeRec: TSFSTestTime); overload;
// shows time
procedure aaShowTime; overload;
procedure aaShowTime(var TimeRec: TSFSTestTime); overload;
// write time to log
procedure aaWriteTime; overload;
procedure aaWriteTime(var TimeRec: TSFSTestTime); overload;
...
procedure aaInitCounter(var Counter: TSFSCounter; Name: AnsiString); overload;
procedure aaIncCounter(var Counter: TSFSCounter; Increment: Int64 = 1);  overload;
procedure aaDecCounter(var Counter: TSFSCounter; Decrement: Int64 = 1);  overload;
function aaGetCounter(var Counter: TSFSCounter): Int64;  overload;
procedure aaSetCounter(var Counter: TSFSCounter; Value: Int64);  overload;
procedure aaShowCounter(var Counter: TSFSCounter);  overload;
...
//------------------------------------------------------------------------------
function SFSGetTickCountDiff(NewTime, OldTime: Cardinal): Cardinal;
const SFSMaxCardinal: Cardinal = $FFFFFFFF;
begin
  if (NewTime >= OldTime) then
   Result := NewTime - OldTime
  else
   Result := Cardinal((SFSMaxCardinal-OldTime+1)+NewTime);
end; // SFSGetTickCountDiff
...
begin
 if (timeStarted) then
  Result := SFSGetTickCountDiff(aaGetTickCount,startTime)
 else
  Result := SFSGetTickCountDiff(stopTime,startTime);
end; // aaGetTime

function aaGetTime(var TimeRec: TSFSTestTime): Integer;
begin
 if (TimeRec.timeStarted) then
  Result := SFSGetTickCountDiff(aaGetTickCount,TimeRec.startTime)
 else
  Result := SFSGetTickCountDiff(TimeRec.stopTime,TimeRec.startTime);
end; // aaGetTime
...
// inits time counting
procedure aaInitTime(var TimeRec: TSFSTestTime);
begin
...
begin
 if (timeRestarted) then
  ...
  if (not timeStarted) then
   begin
    startTime := startTime + SFSGetTickCountDiff(aaGetTickCount,stopTime);
   end;
end;


// starts time counting from current time
procedure aaStartTime(var TimeRec: TSFSTestTime);
begin
 if (TimeRec.timeRestarted) then
  ...
  if (not TimeRec.timeStarted) then
   begin
    TimeRec.startTime := TimeRec.startTime + SFSGetTickCountDiff(aaGetTickCount,TimeRec.stopTime);
   end;
end;
...
// stops time counting
procedure aaStopTime(var TimeRec: TSFSTestTime);
begin
 TimeRec.timeStarted := false;
...
// shows time
procedure aaShowTime(var TimeRec: TSFSTestTime);
begin
...
procedure aaWriteTime(var TimeRec: TSFSTestTime);
begin
...
// Result := LogPath + 'log_'+ExtractFileName(ParamStr(0))+
 ...
 Result := ExtractFilePath(ParamStr(0)) + 'sfs_log_'+ExtractFileName(ParamStr(0))+
 '.txt';
...
procedure aaInitCounter(var Counter: TSFSCounter; Name: AnsiString);
begin
 Counter.Name := Name;
 Counter.Value := 0;
end;

procedure aaIncCounter(var Counter: TSFSCounter; Increment: Int64 = 1);
begin
...
procedure aaDecCounter(var Counter: TSFSCounter; Decrement: Int64 = 1);
begin
 Dec(Counter.Value, Decrement);
end;

function aaGetCounter(var Counter: TSFSCounter): Int64;
begin
 Result := Counter.Value;
end; //


procedure aaSetCounter(var Counter: TSFSCounter; Value: Int64);
begin
 Counter.Value := Value;
end;


procedure aaShowCounter(var Counter: TSFSCounter);
begin
