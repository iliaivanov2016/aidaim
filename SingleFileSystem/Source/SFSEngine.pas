...
{$IFNDEF X64_ON}
     SFSStrFunc,
{$ENDIF}
     SFSPassword,SFSDECUtil,SFSCipher,SFSCipher1,
     SFSFileCtrl
{$IFDEF D12H}
     ,SFS_d12h
{$ENDIF}
...
const
      crlf = #13#10;
      SingleFileSystemSignature = 'AASFSSGN';
			SFSStreamSignature = 'AACS';
      SignatureSize = 8;
      SFSCurrentVersion = 20.00;
      FormatVersion = 1.00;
...
const
 ...
 SFSCompressCurrentVersion = 2.0;
 SFSDirectoryElementSize = sizeof(TDirectoryElement);
 SFSPageHeaderSize = sizeof(TPageHeader);
 // commented by Leo Martin, Ezt 5.40
// MIN_PAGE_SIZE = SFSDirectoryElementSize + SFSPageHeaderSize; // minimum size of one page in Kbytes
//var MIN_PAGE_SIZE: Integer = 2 * 1024;
var MIN_PAGE_SIZE: Integer = SFSDirectoryElementSize + SFSPageHeaderSize;

type
 TSFSFileStreamHeader = packed record
...
const
 SFSCompressedHeaderSize = sizeof(TSFSHeader); // size in bytes (16)
 SFSFileStreamHeaderSize = sizeof(TSFSFileStreamHeader); // size in bytes (16)

type

 TSFSHeadersArray = class
    private
     AllocBy:             integer;
     DeAllocBy:           integer;
     MaxAllocBy:          integer;
    ...
    public
     ...
     procedure AppendItem(value: TSFSHeader; pos: Int64);
     function FindPosition(pos: Int64) : Integer;
   end; // TSFSHeadersArray

 // progress event
 TSFSProgressEvent = procedure 	(
                              Sender      : TObject;
                              PercentDone : Real;
                              var Cancel: Boolean
                             		)  of object;
 ...
 // progress event
 TSFSNoCancelProgressEvent = procedure 	(
                              Sender      : TObject;
                              PercentDone : Real
                             		)  of object;

 // progress event
 TSFSFileProgressEvent = procedure 	(
                              Sender:       TObject;
                              PercentDone:  Real;
 ...
 // progress event
 TSFSOverwritePromptEvent = procedure 	(
                              Sender:   TObject;
 ...
 // disk full event
 TSFSDiskFullEvent = procedure 	(
                              Sender: TObject
                             		)  of object;

 // occurs when password needed
 TSFSOnPasswordEvent = procedure(
                           Sender: TObject;
                           FileName: AnsiString;
 ...
 TPageFileManager = class (TObject)
	public
   SFSFile:    TSFSHugeFile;   // Single file
	private
...
//------------------------------------------------------------------------------
constructor TSFSHeadersArray.Create;
begin
...
//------------------------------------------------------------------------------
destructor TSFSHeadersArray.Destroy;
begin
 SetSize(0);
 inherited Destroy;
end;//Destroy;
...
//------------------------------------------------------------------------------
procedure TSFSHeadersArray.SetSize(newSize: integer);
begin
 if (newSize = 0) then
  begin
   ItemCount := 0;
...
//------------------------------------------------------------------------------
function TSFSHeadersArray.FindPosition(pos: Int64) : Integer;
var i,dx,f,
    oldRes,res : Integer;
...
//------------------------------------------------------------------------------
procedure TSFSHeadersArray.AppendItem(value: TSFSHeader;  pos: Int64);
begin
 inc(ItemCount);
 SetSize(ItemCount);
  ...
  begin
    ...
    if (Hi > iLo) then
     begin
      ...
      if (iHi = Hi) then
       raise Exception.Create('SFSEngine - Sorting error in TIntegerArray');
      QuickSort(iLo, Hi);
     end;
    if (Lo < iHi) then
      QuickSort(Lo, iHi);
...
begin
 ...
 if (Mode = fmCreate) then
  begin
   ...
   FExclusive := True;
   // create Single file
   SFSFile := TSFSHugeFile.Create(FileName, FReadOnly, FExclusive, FInMemory,
                                  PartFileSize);
   SFSFile.Open(true);
   FillChar(FHeader,sizeof(FHeader),$00);
   FHeader.Signature := SingleFileSystemSignature;
   FHeader.Version := SFSCurrentVersion;
...
//   FHeader.ExtentPageCount := 2;
  ...
  begin
   ...
   if (not SFSFile.Open(false, true)) then
    begin
      SFSFile.Free;
      SFSFile := nil;
...
begin
  bOK := true;
//  SFSFile.Seek(FileOffset,soFromBeginning);
  SFSFile.Position := FileOffset;
  SFSFile.ReadBuffer(FHeader,SingleFileHeaderSize);
  // check signature
  if (FHeader.Signature <> SingleFileSystemSignature) then
    bOK := false
  else
   begin
    // check crc
...
begin
 ...
 if (not InternalLoadHeader(0)) then
  raise Exception.Create('TSingleFileSystem.LoadHeader - Invalid SFS file, header is corrupted.');
end; //LoadSFHeader
...
begin
 ...
 SFSFile.Seek(0,soFromBeginning);
 // write primary Single file header
 SFSFile.WriteBuffer(FHeader,SingleFileHeaderSize);
// SFSFile.FlushBuffers;
end; //SaveSFHeader
...
{$ENDIF}
  ...
  // seek
//  SFSFile.Seek(Offset, soFromBeginning);
  SFSFile.Position := Offset;
  // read page header
  Res := SFSFile.Read(buffer.PageHeader, PageHeaderSize);
{$IFDEF DEBUG_TRACE_TPageFileManager_READPAGE}
...
{$ENDIF}
  ...
  Res := SFSFile.Read(buffer.pData^, Size);
  Result := (Res = Size);
{$IFDEF DEBUG_TRACE_TPageFileManager_READPAGE}
...
//  SFSFile.Seek(Offset, soFromBeginning);
  SFSFile.Position := Offset;
  // write page header
  Res := SFSFile.Write(buffer.PageHeader, PageHeaderSize);
{$IFDEF DEBUG_TRACE_TPageFileManager_WRITEPAGE}
...
// aaStartTime;
  if (buffer.pData <> nil) and (size > 0) then
   begin
    Res := SFSFile.Write(buffer.pData^, Size);
{$IFDEF DEBUG_TRACE_TPageFileManager_WRITEPAGE}
...
//  Exit;
 // add pages to total pages
  // resize file
 try
  SFSFile.Size := Int64(FHeader.TOTALPageCount+qty) * Int64(FHeader.PageSize);
  if (SFSFile.Size = Int64(FHeader.TOTALPageCount+qty) * Int64(FHeader.PageSize)) then
   result := True;
 except
   result := False;
 end;
...
begin
  Result := SFSFile.RenameFile(NewName);
end;// RenameFile
...
begin
 if (SFSFile = nil) then
  raise Exception.Create('TPageFileManager.LoadFromStream - SFSFile = nil');
 SFSFile.LoadFromStream(Stream);
 LoadSFHeader;
 // decode default key for user files
 if (FHeader.EncMethod <> encNone) then
...
// LastPageNo := PFMHandle.FHeader.TOTALPageCount-1;
 ...
 if (LastPageNo < OldLastPageNo) then
  begin
   ...
   PFMHandle.SFSFile.Size := NewSize;
  end;
...
begin
 ...
 if (not ExtractPathAndPattern(pAnsiChar(FileName),path,dirName)) then
  begin
   dirID := currentDir;
   SFSEngine.SetFileName(FileName,el);
  end
 else
  begin
   if (path^ = #0) then
   ...
   SFSEngine.SetFileName(dirName,el);
   FreeMem(path);
   FreeMem(dirName);
  end;
...
begin
 ...
 if (not PFMHandle.FReadOnly) then
  begin
   SFSEngine.SetCurrentTime(el.LastAccessTime);
   WriteItem(result,el);
  end;
end; // FileOpen
