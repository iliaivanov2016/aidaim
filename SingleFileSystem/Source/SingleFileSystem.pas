//==============================================================================
// Product name: SingleFileSystem
// Copyright 2001-2021 AidAim Software.
// Description:
//  Single file system.
// Version: 1.13
// Date: 06/20/2002
...
type
 TSingleFileSystem = class;

 TSFSCompressionLevel = (sfsNone, zlibFastest, zlibNormal, ppmNormal, ppmMax,
                         zlibMax, ppmFastest, bzipFastest, bzipNormal, bzipMax);

 TSFSOverwriteMode = (omAlways,omNever,omPrompt);

{$IFDEF FULL_VERSION}
  // user file stream
  TSFSUserFileStream = class(TStream)
  private
    FSFSHandle: TSingleFileSystem;
    FFileName:  AnsiString;
    FOnProgress:    TSFSNoCancelProgressEvent; // progress for bulk operations
  ...
  public
    constructor Create(
           SingleFileSystem: TSingleFileSystem;
   ...
   public
    // Progress Event
    property OnProgress : TSFSNoCancelProgressEvent read FOnProgress write FOnProgress;
    // file system
    property SFSHandle: TSingleFileSystem read FSFSHandle;
    // file name
 ...
 // SFS file stream supporing compression
 TSFSAdvancedFileStream = class(TStream)
 private
    FBlockSize:			Integer;
    FProgress:      Extended;
    FProgressMax:   Extended;
    headers:        TSFSHeadersArray;
    FTrueSize:      Int64;
    FPackedSize:    Int64;
    FCurrentHeader: Integer;
    FCurrentPos:    Int64;
    FCompressionLevel: TSFSCompressionLevel;
    FOnProgress:    TSFSNoCancelProgressEvent; // progress for bulk operations
    FCompressionRate: Real;
    FNoProgress:    Boolean;
    FFileName:      AnsiString;
    FSFSHandle:     TObject; // TSingleFileSystem
    // true for SFS 2.60 and lower
    FIsSFSRelativeOffsets: Boolean;
 protected
    FFile:          TSFSUserFileStream;
    FHeader:        TSFSFileStreamHeader;
    // returns true if stream is encrypted
    function GetEncrypted: Boolean;
    // returns SFSHandle
    function GetSFSHandle: TSingleFileSystem;
 ...
 private
   ...
   // prepares buffer for writing (compresses, fills header structure, calculates crc)
    procedure PrepareBufferForWriting(inBuf: PAnsiChar; inSize: Integer;
                         var outBuf: PAnsiChar; var hdr: TSFSHeader);
    // load block from file, decompress it and checks crc
    procedure LoadBlock(curHeader: Integer;
                        var outBuf: PAnsiChar);
 public
    // if compressionLevel not specified, SFS.CompressionLevel will be used
    constructor Create(
						           SingleFileSystem: TSingleFileSystem;
					  	  			 const FileName: AnsiString; Mode: Word;
						           Password: AnsiString = '';
						           Question: AnsiString = '';
						           Answer: AnsiString = '';
                       compressLevel: TSFSCompressionLevel = sfsNone
                       );
    // destructor
    destructor Destroy; override;
    function Read(var Buffer; Count: Longint): Longint; override;
   ...
   public
    ...
    // compression level
    property CompressionLevel: TSFSCompressionLevel read FCompressionLevel;
    // Packed size
    property PackedSize: Integer read GetPackedSize;
    // handle
    property Handle: Integer read GetHandle;
    // SFSHandle
    property SFSHandle: TSingleFileSystem read GetSFSHandle;
    // file name
    property FileName: AnsiString read FFileName;
    // encrypted
    property Encrypted: Boolean read GetEncrypted;
end; //TSFSAdvancedFileStream

 TSFSFileStream = class(TStream)
 private
  	cStream:        TSFSAdvancedFileStream;
    FOnProgress:    TSFSNoCancelProgressEvent; // progress for bulk operations
    FMode:          Word;
    FFileName:      AnsiString;
    ...
    function GetCompressionLevel: TSFSCompressionLevel;
    // changes compression level, counts progress
    procedure SetCompressionLevel(newCompressLevel: TSFSCompressionLevel);
   public
 ...
 public
    ...
    property CompressionRate: Real read GetCompressionRate;
    // compression level
    property CompressionLevel: TSFSCompressionLevel read GetCompressionLevel
...
{$ENDIF}
 ...
 // Single file system
 TSingleFileSystem = class(TObject)
 protected
 ...
 private
  FProgress:          Extended;
  FProgressMax:       Extended;
  FOnProgress:        TSFSProgressEvent; // progress for bulk operations
  FOnFileProgress:    TSFSFileProgressEvent; // progress for bulk operations for current file
  FOnOverwritePrompt: TSFSOverwritePromptEvent; // overwrite prompt
  FOnDiskFull:        TSFSDiskFullEvent; // not enough space
  FOnPassword:        TSFSOnPasswordEvent; // occurs when password needed
...
{$IFDEF FULL_VERSION}
  ...
  procedure SetCompressionLevel(newLevel: TSFSCompressionLevel);
{$ENDIF}
  // sets in-memory or disk mode
  procedure SetInMemory(value: boolean);
 public
  FLastError:						Integer;
  property IsSFSRelativeOffsets: Boolean read FIsSFSRelativeOffsets;
  ...
  constructor Create(FileName: AnsiString; Mode: Word;
  						...
  						Question: AnsiString = '';
   						Answer: AnsiString = '';
              DefaultCompressionLevel: TSFSCompressionLevel = sfsNone
             ); overload;
...
{$IFDEF FULL_VERSION}
   ...
   function ImportFiles(
                         ...
                         OverwriteMode: TSFSOverwriteMode = omPrompt;
   ...
   // runs application stored inside the SFS file
...
{$ENDIF}
 ...
 public
   ...
   property OnFileProgress : TSFSFileProgressEvent read FOnFileProgress write FOnFileProgress;
   // Progress Event
   property OnProgress : TSFSProgressEvent read FOnProgress write FOnProgress;
   // overwrite prompt
   property OnOverwritePrompt : TSFSOverwritePromptEvent read FOnOverwritePrompt write FOnOverwritePrompt;
   // disk full
   property OnDiskFull: TSFSDiskFullEvent read FOnDiskFull write FOnDiskFull;
   // On password needed Event
   property OnPassword: TSFSOnPasswordEvent read FOnPassword write FOnPassword;
   // Ecnrypted;
   property Encrypted: Boolean read GetEncrypted;
   // password;
   property Password: AnsiString read FPassword;
...
{$IFDEF FULL_VERSION}
   ...
   property DefaultCompressionLevel: TSFSCompressionLevel read GetCompressionLevel
   					write SetCompressionLevel;
{$ENDIF}
 end;// TSingleFileSystem



 //----------------------- SFS control routines -----------------------
 // rename sfs files
 function RenameSFS(const OldName, NewName: AnsiString): Boolean;
 // copy sfs files
 function CopySFS(const OldName, NewName: AnsiString): Boolean;
 // delete sfs files
 function DeleteSFS(const FileName: AnsiString): Boolean;
 // returns true if this file is a SFS file
 function IsSFSFile(const FileName: AnsiString): Boolean;
 // makes SFX from SFS file
 procedure MakeSFX(SFSFileName, SFXStubFileName, SFXFileName: AnsiString);
...
// returns true if stream is encrypted
//------------------------------------------------------------------------------
function TSFSUserFileStream.GetEncrypted: Boolean;
begin
 result := FSFSHandle.IsFileEncrypted(FFileName);
end;
...
// on progress
//------------------------------------------------------------------------------
procedure TSFSUserFileStream.DoOnProgress(Progress : Real);
begin
...
// set size
//------------------------------------------------------------------------------
procedure TSFSUserFileStream.SetSize(NewSize: LongInt);
begin
 SFSHandle.FileSetSize(FHandle,NewSize);
end; //


{$IFDEF D6H}
procedure TSFSUserFileStream.SetSize(const NewSize: Int64);
begin
 SFSHandle.FileSetSize(FHandle,NewSize);
end; //
{$ENDIF}
...
// create
//------------------------------------------------------------------------------
constructor TSFSUserFileStream.Create(SingleFileSystem: TSingleFileSystem;
    			 const FileName: AnsiString; Mode: Word;
           Password: AnsiString = '';
           Question: AnsiString = '';
           Answer: AnsiString = ''
...
begin
 FFileName := FileName;
 if (SingleFileSystem = nil) then
  raise Exception.Create('TSFSUserFileStream.Create - Single file system = nil!');
 FSFSHandle := SingleFileSystem;
{$IFDEF ENCRYPTION_ON}
 FHandle := SFSHandle.FileOpen(FileName,Mode,Password,Question,Answer);
{$ELSE}
 FHandle := SFSHandle.FileOpen(FileName,Mode,'','','');
{$ENDIF}
 if (FHandle <= None) then
  raise Exception.Create('TSFSUserFileStream.Create - unable to open file '+
...
// destroy
//------------------------------------------------------------------------------
destructor TSFSUserFileStream.Destroy;
begin
 SFSHandle.FileClose(FHandle);
 inherited Destroy;
end; //
...
// read
//------------------------------------------------------------------------------
function TSFSUserFileStream.Read(var Buffer; Count: Longint): Longint;
begin
 result := SFSHandle.FileRead(FHandle,Buffer,Count);
end; //
...
// write
//------------------------------------------------------------------------------
function TSFSUserFileStream.Write(const Buffer; Count: Longint): Longint;
begin
 result := SFSHandle.FileWrite(FHandle,Buffer,Count);
end; //
...
// seek
//------------------------------------------------------------------------------
function TSFSUserFileStream.Seek(Offset: LongInt; Origin: Word): Integer;
begin
 result := SFSHandle.FileSeek(FHandle,Offset,Origin);
end; //

{$IFDEF D6H}
function TSFSUserFileStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
 result := SFSHandle.FileSeek(FHandle,Offset,Integer(Origin));
end; //
{$ENDIF}

procedure TSFSUserFileStream.SaveToStream(Stream: TStream);
...
procedure TSFSUserFileStream.LoadFromStream(Stream: TStream);
...
procedure TSFSUserFileStream.SaveToFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
...
procedure TSFSUserFileStream.LoadFromFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
...
// returns true if stream is encrypted
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.GetEncrypted: Boolean;
begin
{$IFDEF ENCRYPTION_ON}
...
// returns SFSHandle
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.GetSFSHandle: TSingleFileSystem;
begin
 result := FFile.SFSHandle;
end;
...
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.SetSize(NewSize: Longint);
var buf,outBuf:           PAnsiChar;
    extSize:              Int64;
    curHdr,numBlocks:     Integer;
begin
 if (FCompressionLevel = sfsNone) then
  begin
   FFile.Size := NewSize;
   Exit;
  end;
   ...
   begin
    ...
    if (Write(buf^,extSize) <> extSize) then
     begin
      FNoProgress := false;
      FreeMem(buf);
      raise Exception.Create('TSFSAdvancedFileStream.SetSize - error extending file. '+
   ...
   begin
    ...
    if (extSize > 0) then
     begin
      ...
      if (FIsSFSRelativeOffsets) then
       begin
        // >= 2.70
        headers.Items[curHdr].nextHeaderNo :=
          Integer(headers.Items[curHdr].packedSize) + SFSCompressedHeaderSize;
       end
      else
       begin
        // <= 2.60
      ...
      FFile.WriteBuffer(headers.Items[curHdr], SFSCompressedHeaderSize);
...
//------------------------------------------------------------------------------
{$IFDEF D6H}
procedure TSFSAdvancedFileStream.SetSize(const NewSize: Int64);
var buf,outBuf:           PAnsiChar;
    extSize:              Int64;
    curHdr:               Integer;
begin
...
// gets FFile.size
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.GetPackedSize: Integer;
begin
  result := FFile.Size;
end; // GetPackedSize
...
// returns compression rate (100.0 if there is no compression)
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.GetCompressionRate: Real;
begin
 CalculateRate;
...
// on progress
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.DoOnProgress(Progress : Real);
begin
...
// set size of compressed stream
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.CalculateRate;
var i: 		Integer;
    f,f1:	Extended;
begin
 FPackedSize := 0;
...
// create
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.InternalCreate(bCreate: Boolean);
begin
 ...
 headers := TSFSHeadersArray.Create;
 if (bCreate) and (FCompressionLevel = sfsNone) then
  Exit;
 if (bCreate) then
  begin
   FHeader.signature := SFSStreamSignature;
   FHeader.BlockSize := SFSInternalGetBlockSize(TSFSCompressionLevel1(FCompressionLevel));
   FHeader.NumBlocks := 0;
   FHeader.version := SFSCompressCurrentVersion;
   ...
   if (FFile.Write(FHeader,sizeof(FHeader)) <> sizeof(FHeader)) then
    raise Exception.Create('TSFSAdvancedFileStream.Create - error writing to data stream. May be data stream in read only mode.');
   FFile.Size := sizeof(FHeader);
  end;
 FFile.Position := 0;
 FTrueSize := 0;
// FCompressionLevel := sfsNone;
 LoadHeaders; // loading headers
// FCompressionLevel := TSFSCompressionLevel(FHeader.CompressionLevel);
 FCurrentHeader := 0;
 FCurrentPos := 0;
 FNoProgress := false;
 FFile.Position := 0;
...
// returns handle
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.GetHandle: Integer;
begin
 result := FFile.Handle;
end;
...
// load block headers
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.LoadHeaders;
var cHeader:  TSFSHeader;
    pos:      Int64;
    oldPos:		Int64;
    i:        Integer;
begin
 ...
 if (FHeader.signature <> SFSStreamSignature) then
  begin
   FTrueSize := FFile.Size;
   FFile.Position := oldPos;
   FCompressionLevel := sfsNone;
...
{
 if (FHeader.version - 0.001 > SFSCompressCurrentVersion) then
  raise Exception.Create('TSFSAdvancedFileStream.LoadHeaders - invalid stream version, currentVersion = '+
        FloatToStr(SFSCompressCurrentVersion)+
        ', FHeader.version = '+
 ...
 for i:= 0 to FHeader.NumBlocks-1 do
  begin
   pos := FFile.Position;
   if (FFile.Size - FFile.Position < SFSCompressedHeaderSize) then
    begin
   ...
   FFile.ReadBuffer(cHeader,SFSCompressedHeaderSize);
...
// save stream header
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.SaveHeader;
begin
 FFile.Seek(0,soFromBeginning);
...
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.PrepareBufferForWriting(inBuf: PAnsiChar; inSize: Integer;
                         var outBuf: PAnsiChar; var hdr: TSFSHeader);
begin
  outBuf := nil;
  hdr.trueSize := inSize;
  hdr.crc32 := CountCRC(inBuf,LongWord(inSize),crcFull);
  if (not SFSInternalCompressBuffer(inBuf,inSize,outBuf,
          Integer(hdr.packedSize),TSFSCompressionLevel1(FCompressionLevel))) then
   begin
    if (outBuf <> nil) then
     FreeMem(outBuf);
    raise Exception.Create('TSFSAdvancedFileStream.PrepareBufferForWriting - compression error in PrepareBuffer.');
   end;
end; //PrepareBuffer;
...
// load block from file, decompress it and checks crc
//------------------------------------------------------------------------------
procedure TSFSAdvancedFileStream.LoadBlock(curHeader: Integer;
                                      var outBuf: PAnsiChar);
...
//      +SFSCompressedHeaderSize,soFromBeginning);
   FFile.Position := headers.Positions[curHeader] + Int64(SFSCompressedHeaderSize);
   rSize := Cardinal(FFile.Read(inBuf^,pSize));
   if (rSize <> Integer(pSize)) then
    begin
     FreeMem(inBuf);
     raise Exception.Create('TSFSAdvancedFileStream.LoadBlock - block read error. rSize = '+IntToStr(rSize)+', pSize = '+IntToStr(pSize));
     Exit;
    end;

   size := headers.Items[curHeader].trueSize;
   if (not SFSInternalDecompressBuffer(inBuf,pSize,outBuf,Integer(size),
            TSFSCompressionLevel1(FCompressionLevel))) then
    begin
     // decompression error
     FreeMem(inBuf);
     raise Exception.Create('TSFSAdvancedFileStream.LoadBlock - decompression error.');
    end;
   if (headers.Items[curHeader].trueSize <> size) then
    begin
     FreeMem(inBuf);
     FreeMem(outBuf);
     raise Exception.Create('TSFSAdvancedFileStream.LoadBlock - decompression error, invalid size.');
    end;
   // check crc
   if (headers.Items[curHeader].crc32 <>
    ...
    // decompression crc error
     FreeMem(inBuf);
     FreeMem(outBuf);
     raise Exception.Create('TSFSAdvancedFileStream.LoadBlock - decompression crc error.');
    end;
 FreeMem(inBuf);
end;
...
//------------------------------------------------------------------------------
constructor TSFSAdvancedFileStream.Create(
						           SingleFileSystem: TSingleFileSystem;
					  	  			 const FileName: AnsiString; Mode: Word;
						           Password: AnsiString = '';
						           Question: AnsiString = '';
...
begin
 FSFSHandle := SingleFileSystem;
 // true for SFS 2.60 and lower
 FIsSFSRelativeOffsets := SingleFileSystem.IsSFSRelativeOffsets;
 FFileName := FileName;
 if (SingleFileSystem = nil) then
  raise Exception.Create('TSFSAdvancedFileStream.Create - SingleFileSystem = nil.');
{$IFDEF ENCRYPTION_ON}
 FFile := TSFSUserFileStream.Create(SingleFileSystem,FileName,Mode,
...
{$ENDIF}
 if (FFile = nil) then
    raise Exception.Create('TSFSAdvancedFileStream.Create - error creating user file stream, fileName = '+
    	AnsiQuotedStr(FileName,'"')+'.');
 FCompressionLevel := compressLevel;
 if (FCompressionLevel = sfsNone) then
    FCompressionLevel := SingleFileSystem.DefaultCompressionLevel;
...
// destructor
//------------------------------------------------------------------------------
destructor TSFSAdvancedFileStream.Destroy;
begin
 FFile.Free;
 headers.Free;
 FSFSHandle := nil;
...
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.Read(var Buffer; Count: Longint): Longint;
var size:   Int64;
    outBuf: PAnsiChar;
begin
 DoOnProgress(0);
...
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.Write(const Buffer; Count: Longint): Longint;
...
begin
 ...
 if (startHeaderNo = 0) then
  headers.Positions[0] := SFSFileStreamHeaderSize + FHeader.CustomHeaderSize;
 FProgress := 0;
 FProgressMax := (headers.ItemCount - FCurrentHeader) ;
 curPos := FCurrentPos;
 ...
 while (i <= headers.ItemCount-1)  do
  begin
   ...
   if (not FIsSFSRelativeOffsets) then
    if (i = headers.itemCount-1) then
     begin
      // <= 2.60
      if (i = 0) then
        ...
        headers.Items[i].nextHeaderNo := headers.Positions[0] +
          headers.Items[i].packedSize + SFSCompressedHeaderSize
      else
...
//   FFile.Seek(headers.Positions[j],soFromBeginning);
   ...
   FFile.WriteBuffer(headers.Items[j], SFSCompressedHeaderSize);
...
//------------------------------------------------------------------------------
function TSFSAdvancedFileStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
 if (FCompressionLevel = sfsNone) then
  begin
...
{$IFDEF D6H}
function TSFSAdvancedFileStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
 if (FCompressionLevel = sfsNone) then
  begin
   result := FFile.Seek(Offset,Origin);
...
procedure TSFSAdvancedFileStream.SaveToStream(Stream: TStream);
var outBytes,oldPos,oldPos1,inSize:	Int64;
    outSize:					Integer;
    buf:            	PAnsiChar;
    FProgress:      	Extended;
...
procedure TSFSAdvancedFileStream.LoadFromStream(Stream: TStream);
var oldPos,oldPos1:	Int64;
    outSize:					Integer;
    buf:            	PAnsiChar;
    FProgress:      	Extended;
...
procedure TSFSAdvancedFileStream.SaveToFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
  Stream := TFileStream.Create(FileName, fmCreate);
...
procedure TSFSAdvancedFileStream.LoadFromFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
  Stream := TFileStream.Create(FileName, sysUtils.fmOpenRead or fmShareDenyWrite);
...
// returns Handle
//------------------------------------------------------------------------------
function TSFSFileStream.GetHandle: Integer;
begin
...
// returns SFSHandle
//------------------------------------------------------------------------------
function TSFSFileStream.GetSFSHandle: TSingleFileSystem;
begin
 result := cStream.SFSHandle;
end;
...
// returns compression level
//------------------------------------------------------------------------------
function TSFSFileStream.GetCompressionLevel: TSFSCompressionLevel;
begin
...
// changes compression level, counts progress
//------------------------------------------------------------------------------
procedure TSFSFileStream.SetCompressionLevel(newCompressLevel: TSFSCompressionLevel);
var
    cs:         TSFSAdvancedFileStream;
    sfs:        TSingleFileSystem;
    attr:       word;
    passHeader: TPasswordHeader;
begin
  sfs := cStream.SFSHandle;
  ...
  attr := sfs.FileGetAttr(FFileName);
  // save file password header
  if (not sfs.GetPasswordHeader(FFileName,passHeader)) then
   raise Exception.Create('TSFSFileStream.SetCompressionLevel - can not retreive password header. Probably file does not exists.'+
   'File name = "'+FFileName+'"');
  sfs.RenameFile(FFileName,FFileName+'.bak');
  cs := TSFSAdvancedFileStream.Create(sfs,FFileName+'.bak',SysUtils.fmOpenRead,
        FPassword);
  cStream := TSFSAdvancedFileStream.Create(sfs,FFileName,fmCreate,
              FPassword,'','',newCompressLevel);
  cStream.Free;
  // restore old password header
  sfs.SetPasswordHeader(FFileName,passHeader);
  // restore file attributes
  sfs.FileSetAttr(FFileName,attr);
  cStream := TSFSAdvancedFileStream.Create(sfs,FFileName,fmOpenWrite,FPassword);
  cStream.OnProgress := FOnProgress;
  cStream.LoadFromStream(cs);
  cs.Free;
  cStream.Free;
  cStream := TSFSAdvancedFileStream.Create(sfs,FFileName,FMode,FPassword);
	sfs.DeleteFile(FFileName+'.bak');
...
// if newPassword = '' then encryption will be removed
//------------------------------------------------------------------------------
procedure TSFSFileStream.ChangeEncryption(
                              newPassword: AnsiString = '';
                              newQuestion: AnsiString = '';
                              newAnswer: AnsiString = ''
                              );
var
    ...
    sfs:        TSingleFileSystem;
    attr:       word;
    CompressLevel:  TSFSCompressionLevel;
begin
...
{$ENDIF}
  ...
  sfs.FileSetAttr(FFileName,attr);
  // reopening new file
  cStream := TSFSAdvancedFileStream.Create(sfs,FFileName,fmOpenWrite,newPassword);
  cStream.OnProgress := FOnProgress;
  cStream.LoadFromStream(cs);
  cs.Free;
  cStream.Free;
...
// returns packed size
//------------------------------------------------------------------------------
function TSFSFileStream.GetPackedSize: Integer;
begin
...
// returns compression rate (100.0 if there is no compression)
//------------------------------------------------------------------------------
function TSFSFileStream.GetCompressionRate: Real;
begin
...
// returns true if stream is encrypted
//------------------------------------------------------------------------------
function TSFSFileStream.GetEncrypted: Boolean;
begin
{$IFDEF ENCRYPTION_ON}
...
// sets new size of the stream
//------------------------------------------------------------------------------
procedure TSFSFileStream.SetSize(NewSize: Longint);
begin
 if (bReadOnly) then
  raise Exception.Create('TSFSFileStream.SetSize - file is in read only mode.');
...
//------------------------------------------------------------------------------
{$IFDEF D6H}
procedure TSFSFileStream.SetSize(const NewSize: Int64);
begin
 if (bReadOnly) then
  raise Exception.Create('TSFSFileStream.SetSize - file is in read only mode.');
 cStream.Size := NewSize;
...
// if compressionLevel not specified, SFS.CompressionLevel will be used
//------------------------------------------------------------------------------
constructor TSFSFileStream.Create(
						           SingleFileSystem: TSingleFileSystem;
					  	  			 const FileName: AnsiString; Mode: Word;
						           Password: AnsiString = '';
						           Question: AnsiString = '';
...
{$IFDEF ENCRYPTION_ON}
 ...
 if (Mode = fmCreate) then
  cStream := TSFSAdvancedFileStream.Create(SingleFileSystem,
...
// destructor
//------------------------------------------------------------------------------
destructor TSFSFileStream.Destroy;
begin
 if (cStream <> nil) then
  begin
   cStream.Free;
...
// read buffer
//------------------------------------------------------------------------------
function TSFSFileStream.Read(var Buffer; Count: Longint): Longint;
begin
 cStream.OnProgress := FOnProgress;
...
// write buffer
//------------------------------------------------------------------------------
function TSFSFileStream.Write(const Buffer; Count: Longint): Longint;
begin
 if (bReadOnly) then
  raise Exception.Create('TSFSFileStream.Write - file is in read only mode.');
 cStream.OnProgress := FOnProgress;
...
// seek
//------------------------------------------------------------------------------
function TSFSFileStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
...
{$IFDEF D6H}
function TSFSFileStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
 result := cStream.Seek(Offset,Origin);
end; // Seek
{$ENDIF}
...
// save data to stream
//------------------------------------------------------------------------------
procedure TSFSFileStream.SaveToStream(Stream: TStream);
begin
 cStream.OnProgress := FOnProgress;
...
//------------------------------------------------------------------------------
procedure TSFSFileStream.LoadFromStream(Stream: TStream);
begin
 if (bReadOnly) then
  raise Exception.Create('TSFSFileStream.LoadFromStream - file is in read only mode.');
 cStream.OnProgress := FOnProgress;
...
// save data to file
//------------------------------------------------------------------------------
procedure TSFSFileStream.SaveToFile(const FileName: AnsiString);
begin
 cStream.OnProgress := FOnProgress;
...
// load data from file
//------------------------------------------------------------------------------
procedure TSFSFileStream.LoadFromFile(const FileName: AnsiString);
begin
 cStream.OnProgress := FOnProgress;
...
// on progress
//------------------------------------------------------------------------------
procedure TSingleFileSystem.DoOnProgress(Progress : Real);
begin
  if Assigned(FOnProgress) then
...
// closes all opened files
//------------------------------------------------------------------------------
procedure TSingleFileSystem.CloseAllFiles;
var i: Integer;
begin
...
// create all internal objects
//------------------------------------------------------------------------------
procedure TSingleFileSystem.InternalCreate;
begin
...
// not enough space
//------------------------------------------------------------------------------
procedure TSingleFileSystem.DoOnDiskFull(
                         Sender: TObject
                        );
begin
...
// occurs when password needed
//------------------------------------------------------------------------------
procedure TSingleFileSystem.DoOnPassword(
                           FileName: AnsiString;
                           var NewPassword: AnsiString;
                           var SkipFile: Boolean
                           );
...
// on overwrite prompt
//------------------------------------------------------------------------------
procedure TSingleFileSystem.DoOnOverwritePrompt(
            ExistingFileName,
            NewFileName: AnsiString;
            var bOverwrite: Boolean);
begin
...
// file progress
//------------------------------------------------------------------------------
procedure TSingleFileSystem.DoOnFileProgress(
                              Sender: TObject;
                              PercentDone:  Real
                             		);
begin
...
// reloads all data from disk
//------------------------------------------------------------------------------
procedure TSingleFileSystem.InternalReopen(Stream: TStream);
begin
//free handles
 UFMHandle.Free;
 DMHandle.Free;
...
// returns default compression level
//------------------------------------------------------------------------------
function TSingleFileSystem.GetCompressionLevel: TSFSCompressionLevel;
begin
 result := TSFSCompressionLevel(PFMHandle.FHeader.CompressionLevel);
end;
...
// sets default compression level
//------------------------------------------------------------------------------
procedure TSingleFileSystem.SetCompressionLevel(newLevel: TSFSCompressionLevel);
begin
...
// sets in-memory or disk mode
//------------------------------------------------------------------------------
procedure TSingleFileSystem.SetInMemory(value: boolean);
begin
...
// constructor
//------------------------------------------------------------------------------
constructor TSingleFileSystem.Create(FileName: AnsiString; Mode: Word;
  						Password: AnsiString = '';
  						Question: AnsiString = '';
  						Answer: AnsiString = '';
              DefaultCompressionLevel: TSFSCompressionLevel = sfsNone
...
begin
 if (Mode <> fmCreate) then
  begin
   if (not SingleFileSystem.IsPasswordValid(FileName,Password)) then
    raise Exception.Create('TSingleFileSystem.Create - invalid password specified, file name = '+
			    AnsiQuotedStr(FileName,'"'));
  end;
 FOpenMode := Mode;
 FFileName := FileName;
...
{$ELSE}
 ...
 FPassword := '';
{$ENDIF}
 FIsSFSRelativeOffsets := (PFMHandle.FHeader.Version > 2.601);
{$IFDEF FULL_VERSION}
 if (mode = fmCreate) then
...
// destructor
//------------------------------------------------------------------------------
destructor TSingleFileSystem.Destroy;
begin
...
// repairs file
//------------------------------------------------------------------------------
function TSingleFileSystem.InternalRepair(
                          var log: AnsiString; DeleteCorruptedFiles: Boolean = false;
                          ChangeEncryption: Boolean = false;
                          newPassword: AnsiString = '';
                          newQuestion: AnsiString = '';
...
//	 SFSEngine.SetCurrentTime(el.LastModifiedTime);
...
begin
 ...
 try
   ...
   if (ChangeEncryption) then
    begin
     NewFile := TSingleFileSystem.Create(NewName,fmCreate,FInMemory,
                newPassword,newQuestion,newAnswer,
                TSFSCompressionLevel(DefaultCompressionLevel),
                PageSize,
  ...
  //NewFile.Free;
  //NewFile := TSingleFileSystem.Create(NewName,fmOpenReadWrite,FInMemory,FPassword);
     if (NewFile = nil) then
      Exit;
...
try
 ...
 for i := 0 to DMHandle.FDIR.ItemCount-1 do
  begin
   ...
   if (FCancel) then
    begin
     NewFile.PFMHandle.SFSFile.Close;
     NewFile.PFMHandle.SFSFile.DeleteFile;
     NewFile.Free;
...
// NewFile.Free;
 NewFile.PFMHandle.SFSFile.Close;
 PFMHandle.SFSFile.Close;
{ PFMHandle.SFSFile.Free;
 PFMHandle.SFSFile := nil;}
 ...
 if (not NewFile.PFMHandle.RenameFile(FullPath)) then
  begin
   ...
   PFMHandle.SFSFile.Open(false, true);
   NewFile.Free;
...
function TSingleFileSystem.Repair(var log: AnsiString; DeleteCorruptedFiles: Boolean = false): Boolean;
begin
 result := false;
 if (FReadOnly) then
  Exit;
...
// returns true and restores password if control answer is valid
//------------------------------------------------------------------------------
function TSingleFileSystem.RestorePasswordByControlAnswer(FileName: AnsiString; Answer: AnsiString; var Password: AnsiString): Boolean;
begin
...
// returns true if Single file password is valid
//------------------------------------------------------------------------------
function TSingleFileSystem.IsPasswordValid(FileName: AnsiString; Password: AnsiString): Boolean;
begin
...
// returns control question
//------------------------------------------------------------------------------
function TSingleFileSystem.GetControlQuestion(FileName: AnsiString): AnsiString;
begin
...
// get password header
//------------------------------------------------------------------------------
function TSingleFileSystem.GetPasswordHeader(FileName: AnsiString; var passHeader: TPasswordHeader): Boolean;
begin
...
// set password header
//------------------------------------------------------------------------------
procedure TSingleFileSystem.SetPasswordHeader(FileName: AnsiString; passHeader: TPasswordHeader);
begin
...
// returns encrypted
//------------------------------------------------------------------------------
function TSingleFileSystem.GetEncrypted: Boolean;
begin
...
// returns control question
//------------------------------------------------------------------------------
function TSingleFileSystem.PGetControlQuestion: AnsiString;
begin
...
// returns true if file is encrypted by its own password
//------------------------------------------------------------------------------
function TSingleFileSystem.IsFileEncrypted(FileName: AnsiString): boolean;
begin
...
// creates file
//------------------------------------------------------------------------------
function TSingleFileSystem.FileCreate(const FileName: AnsiString;
						 Password: AnsiString = '';
						 Question: AnsiString = '';
						 Answer: AnsiString = ''
             ): Integer;
...
// open file
//------------------------------------------------------------------------------
function TSingleFileSystem.FileOpen(const FileName: AnsiString; Mode: LongWord;
						 Password: AnsiString = '';
						 Question: AnsiString = '';
						 Answer: AnsiString = ''
             ): Integer;
...
// file close
//------------------------------------------------------------------------------
procedure TSingleFileSystem.FileClose(Handle: Integer);
var UFHandle: pUserFileHandle;
begin
 UFHandle := FFileHandles.Find(Handle);
 if (UFHandle <> nil) then
...
// read from file
//------------------------------------------------------------------------------
function TSingleFileSystem.FileRead(Handle: Integer; var Buffer; Count: Integer): Integer;
var UFHandle: pUserFileHandle;
    pages:		TIntegerArray;
...
aaWriteToLog('0 TSingleFileSystem.FileRead. Position = '+IntToStr(UFHandle^.Position)+#9+'Size = '+IntToStr(el.FileSize)+#9+'Count = '+IntToStr(Count));
{$ENDIF}
 ...
 UFMHandle.GetPages(el,UFHandle^.Position,size,false,pages);
{$IFDEF DEBUG_TRACE_READ}
aaWriteToLog('1 TSingleFileSystem.FileRead. pages.ItemCount = '+IntToStr(Pages.ItemCount));
aaWriteToLog('First map PageNo = '+IntToStr(el.FirstMapPageNo));
{$ENDIF}
 if (pages.ItemCount <= 0) then
  raise Exception.Create('TSingleFileSystem.FileRead - no pages were found by UFM.');
 buf.pData := AllocMem(PFMHandle.PageDataSize);
 buf.PageHeader.PageType := UFPage;
...
//size bugs
   ...
   if (Count - result > PFMHandle.PageDataSize-offset) then
    ...
    size := Count - result;
{$IFDEF DEBUG_TRACE_READ}
aaWriteToLog('1 TSingleFileSystem.FileRead reading page, pageNo = '+IntToStr(pages.Items[i])+#9+'i = '+IntToStr(i)+#9+'Pos = '+IntToStr(UFHandle^.Position));
{$ENDIF}
   if (not PFMHandle.ReadPage(buf,pages.Items[i],PFMHandle.PageDataSize,Key)) then
    begin
     FLastError := PFMHandle.FLastError;
{$IFDEF DEBUG_TRACE_READ}
aaWriteToLog('2 TSingleFileSystem.FileRead reading page failed! pageNo = '+IntToStr(pages.Items[i])+#9+'i = '+IntToStr(i)+#9+'LastError = '+IntToStr(FLastError));
{$ENDIF}
     break;
    end;
...
// write to file
//------------------------------------------------------------------------------
function TSingleFileSystem.FileWrite(Handle: Integer; const Buffer; Count: Integer): Integer;
var UFHandle: pUserFileHandle;
    pages:		TIntegerArray;
    el:				TDirectoryElement;
    size,i:		Integer;
...
aaWriteToLog('0 TSingleFileSystem.FileWrite. Position = '+IntToStr(UFHandle^.Position)+#9+'Size = '+IntToStr(el.FileSize)+#9+'Count = '+IntToStr(Count));
aaWriteToLog('First map PageNo = '+IntToStr(el.FirstMapPageNo));
{$ENDIF}
 pages := TIntegerArray.Create(0,1,100);
//aaStartTime;
...
{$IFDEF DEBUG_TRACE_WRITE}
aaWriteToLog('0.1 TSingleFileSystem.FileWrite. pages.ItemCount = '+IntToStr(Pages.ItemCount));
{$ENDIF}
...
//debugFlag := true;
{$IFDEF DEBUG_TRACE_WRITE}
aaWriteToLog('1 TSingleFileSystem.FileWrite writing page, pageNo = '+IntToStr(pages.Items[i])+#9+'i = '+IntToStr(i)+#9+'Pos = '+IntToStr(UFHandle^.Position));
{$ENDIF}
   if (not PFMHandle.WritePage(buf,pages.Items[i],PFMHandle.PageDataSize,Key)) then
    begin
     FLastError := PFMHandle.FLastError;
{$IFDEF DEBUG_TRACE_READ}
aaWriteToLog('2 TSingleFileSystem.FileWrite writing page failed! pageNo = '+IntToStr(pages.Items[i])+#9+'i = '+IntToStr(i)+#9+'LastError = '+IntToStr(FLastError));
{$ENDIF}
     break;
    end;
//debugFlag := false;
...
// seek in file
//------------------------------------------------------------------------------
function TSingleFileSystem.FileSeek(Handle: Integer; const Offset: Int64; Origin: Integer): Int64;
var UFHandle: pUserFileHandle;
    el:				TDirectoryElement;
begin
 result := 0;
...
// flsuh file buffers
//------------------------------------------------------------------------------
procedure TSingleFileSystem.FlushFileBuffers(Handle: Integer);
begin
 PFMHandle.FlushFileBuffers;
...
// sets file size
//------------------------------------------------------------------------------
function TSingleFileSystem.FileSetSize(Handle: Integer; Size: Int64): Int64;
var UFHandle: pUserFileHandle;
    el:				TDirectoryElement;
begin
...
// delete file
//------------------------------------------------------------------------------
function TSingleFileSystem.DeleteFile(const FileName: AnsiString): Boolean;
...
// rename file
//------------------------------------------------------------------------------
function TSingleFileSystem.RenameFile(const OldName, NewName: AnsiString): Boolean;
begin
...
// copy file
//------------------------------------------------------------------------------
function TSingleFileSystem.CopyFile(const OldName, NewName: AnsiString; Password: AnsiString = ''): Boolean;
var Size1,
    FBlockSize: Integer;
    Count:      Int64;
    Size:       Int64;
...
// move file
//------------------------------------------------------------------------------
function TSingleFileSystem.MoveFile(const OldName, NewName: AnsiString; Password: AnsiString = ''): Boolean;
begin
...
// is file exists
//------------------------------------------------------------------------------
function TSingleFileSystem.FileExists(const FileName: AnsiString): Boolean;
begin
...
// returns file attributes
//------------------------------------------------------------------------------
function TSingleFileSystem.FileGetAttr(const FileName: AnsiString): Integer;
var  FileID:  Integer;
     el:			TDirectoryElement;
begin
 result := 0;
...
// set attributes, FileSetAttr returns zero if the function was successful.
//------------------------------------------------------------------------------
function TSingleFileSystem.FileSetAttr(const FileName: AnsiString; Attr: Integer): Integer;
var  FileID:  Integer;
     el:			TDirectoryElement;
begin
 result := None;
...
// Returns the date-and-time stamp of a specified file.
//------------------------------------------------------------------------------
function TSingleFileSystem.FileAge(const FileName: AnsiString): Integer;
var  FileID:  Integer;
     el:			TDirectoryElement;
...
// Returns a DOS date-time stamp for a specified file.
//------------------------------------------------------------------------------
function TSingleFileSystem.FileGetDate(Handle: Integer): Integer;
var UFHandle: pUserFileHandle;
    el:				TDirectoryElement;
    LocalFileTime: TFileTime;
begin
...
// Sets the DOS time stamp for a specified file.
//------------------------------------------------------------------------------
function TSingleFileSystem.FileSetDate(Handle: Integer; Age: Integer): Integer;
var UFHandle: pUserFileHandle;
    el:				TDirectoryElement;
...
// find file by pattern using '*', '?'
//------------------------------------------------------------------------------
function TSingleFileSystem.FindFirst(const Path: AnsiString; Attr: Integer; var F: TSearchRec): Integer;
begin
...
// find next file
//------------------------------------------------------------------------------
function TSingleFileSystem.FindNext(var F: TSearchRec): Integer;
begin
...
//------------------------------------------------------------------------------
procedure TSingleFileSystem.FindClose(var F: TSearchRec);
begin
...
// if newPassword = '' then encryption will be removed
//------------------------------------------------------------------------------
function TSingleFileSystem.ChangeEncryption(
                              newPassword: AnsiString = '';
                              newQuestion: AnsiString = '';
                              newAnswer: AnsiString = ''
                              ): Boolean;
...
//------------------------------------------------------------------------------
function TSingleFileSystem.ChangeFilesEncryption(FileMask, oldPassword,
  newPassword, newQuestion, newAnswer: AnsiString): Boolean;
var
    sr: TSearchRec;
    SFStream: TSFSFileStream;
    pwd: AnsiString;
    SkipFile: boolean;
begin
 result := false;
...
{$ENDIF}
 ...
 repeat
   ...
   if not SkipFile then
     begin
       SFStream := TSFSFileStream.Create(self, sr.Name, fmOpenReadWrite, pwd);
       try
         SFStream.ChangeEncryption(newPassword, newQuestion, newAnswer);
         result := true;
       finally
         SFStream.Free;
       end;
...
// returns current directory name ('\' if root directory )
//------------------------------------------------------------------------------
function TSingleFileSystem.GetCurrentDir: AnsiString;
begin
...
//------------------------------------------------------------------------------
procedure TSingleFileSystem.PSetCurrentDir(const Dir: AnsiString);
begin
 SetCurrentDir(Dir);
end;
...
// return value set to True if directory successfully changed
//------------------------------------------------------------------------------
function TSingleFileSystem.SetCurrentDir(const Dir: AnsiString): Boolean;
begin
...
// removes directory
//------------------------------------------------------------------------------
function TSingleFileSystem.RemoveDir(const Dir: AnsiString): Boolean;
begin
...
// creates directory
//------------------------------------------------------------------------------
function TSingleFileSystem.CreateDir(const Dir: AnsiString): Boolean;
begin
...
// Creates all the directories along a directory path if they do not already exist
//------------------------------------------------------------------------------
function TSingleFileSystem.ForceDirectories(Dir: AnsiString): Boolean;
begin
...
// determines whether a specified directory exists.
//------------------------------------------------------------------------------
function TSingleFileSystem.DirectoryExists(Name: AnsiString): Boolean;
begin
...
// returns size of Single file
//------------------------------------------------------------------------------
function TSingleFileSystem.DiskSize: Int64;
begin
 result := PFMHandle.SFSFile.Size;
end; // DiskSize
...
// returns free space in Single file
//------------------------------------------------------------------------------
function TSingleFileSystem.DiskFree: Int64;
begin
...
//------------------------------------------------------------------------------
function TSingleFileSystem.ImportFiles(
                         SourcePath:    AnsiString;
                         DestPath:      AnsiString = ''; // current dir
                         Attr:          Integer = faAnyFile;
                         bRecursive:    Boolean = true;
...
const
 ...
 begin
  ...
  fs := TSFSFileStream.Create(self, name, fmCreate, password);
...
// returns number of files imported
//------------------------------------------------------------------------------
function TSingleFileSystem.ImportFolder(
                         SourcePath:    AnsiString;
                         DestPath:      AnsiString = ''; // current dir
                         bRecursive:    Boolean = true;
                         OverwriteMode: TSFSOverwriteMode = omPrompt;
...
//------------------------------------------------------------------------------
function TSingleFileSystem.ExportFiles(
                         SourcePath:    AnsiString;
                         DestPath:      AnsiString = ''; // current dir
                         Attr:          Integer = faAnyFile;
                         bRecursive:    Boolean = true;
...
const
 ...
 begin
  ...
  FCurrentFileName := name;
  fs := TSFSFileStream.Create(self,StartPath+name,fmOpenRead, password);
  try
   fs.OnProgress := DoOnFileProgress;
   if (SysUtils.FileExists(name)) then
   ...
   if (SysUtils.FileExists(name)) then
    raise Exception.Create('Error in TSingleFileSystem.ExportFiles - file '+
...
// returns number of files Exported
//------------------------------------------------------------------------------
function TSingleFileSystem.ExportFolder(
                         SourcePath:    AnsiString;
                         DestPath:      AnsiString = ''; // current dir
                         bRecursive:    Boolean = true;
                         OverwriteMode: TSFSOverwriteMode = omPrompt
...
// returns true if directory is empty
//------------------------------------------------------------------------------
function TSingleFileSystem.IsFolderEmpty(Dir: AnsiString): Boolean;
...
// returns number of files deleted
//------------------------------------------------------------------------------
function TSingleFileSystem.DeleteFiles(
                         Path:    AnsiString;
                         Attr:    Integer = faAnyFile;
                         bRecursive: Boolean = true
                        ): Integer;
...
{$ENDIF}
       ...
       begin
        ...
        if (not RemoveDir(Path+sr.Name)) then
         raise Exception.Create('TSingleFileSystem.DeleteFiles - could not delete directory '''+
         Path+sr.Name+'''');
       end;
    until (FindNext(sr) <> 0);
   end;
...
// returns number of files exported
//------------------------------------------------------------------------------
function TSingleFileSystem.DeleteFolder(
                         Dir:    AnsiString
                         ): Integer;
var oldPath: AnsiString;
begin
...
// runs application stored inside the SFS file
//------------------------------------------------------------------------------
function TSingleFileSystem.RunApplication(FileName: AnsiString; Parameters: String='';
                                          Directory: String='';
                                          ShowCmd: Integer=SW_SHOWNORMAL): Boolean;
var s1,s: AnsiString;
begin
...
// runs application stored inside the SFS file
//------------------------------------------------------------------------------
function TSingleFileSystem.LoadLibrary(FileName: AnsiString): Integer;
var s1,s: AnsiString;
begin
 Result := 0;
 if (not self.FileExists(FileName)) then
...
//------------------------------------------------------------------------------
// loads SFS file from stream
//------------------------------------------------------------------------------
procedure TSingleFileSystem.LoadFromStream(Stream: TStream);
begin
 if (FReadOnly) then
  raise Exception.Create('TSingleFileSystem.LoadFromStream - file is in read only mode!');
 InternalReopen(Stream);
end;


procedure TSingleFileSystem.LoadFromFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
  Stream := TFileStream.Create(FileName, sysUtils.fmOpenRead or fmShareDenyWrite);
...
procedure TSingleFileSystem.SaveToStream(Stream: TStream);
begin
 PFMHandle.SFSFile.SaveToStream(Stream);
end;


procedure TSingleFileSystem.SaveToFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
  Stream := TFileStream.Create(FileName, fmCreate);
...
var
  hf: TSFSHugeFile;
begin
 hf := TSFSHugeFile.Create(OldName, false, true, false);
 hf.Close;
...
var
  hf: TSFSHugeFile;
begin
 hf := TSFSHugeFile.Create(FileName, false, true, false);
 hf.Close;
...
begin
// hf := TSFSHugeFile.Create(FileName, false, true, false);
 hf := TSFSHugeFile.Create(FileName, true, false, false);
...
begin
  ...
  if (not FileExists(SFSFileName)) then
   raise Exception.Create('MakeSFX - Cannot open file '+SFSFileName);
  ...
  try
   // open SFS
   hSFS := TSFSHugeFile.Create(SFSFileName, true, false, false);
   if (not hSFS.Open(false,false)) then
    raise Exception.Create('MakeSFX - Cannot open file '+SFSFileName);
...
//------------------------------------------------------------------------------
Function SFSWindowCallback(WHandle : HWnd; Var Parm : Pointer) : Boolean;
          stdcall;
...
initialization
  ...
  EnumWindows(@SFSWindowCallback,Longint(@WindowLst));
  // IDE detection
  IsIDERunning := false;
...
capt := 'Single File System Trial - v.'+FormatFloat('0.00',SFSEngine.SFSCurrentVersion);
msg :=             'This is the full functional trial version of Single File System  by'#13+
