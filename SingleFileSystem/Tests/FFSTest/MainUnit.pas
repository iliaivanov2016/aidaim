...
//const TestCount = 1000; // TestCount mod 2 shold be always 0
const TestCount = 100; // TestCount mod 2 shold be always 0
const TestPath = 's:\temp';

// BIG SFS TEST constants
const aSFSInMemory = false;
const aSFSCompressionLevel = sfsNone;
const aSFSPassword = '';
const aSFSQuestion = '';
const aSFSAnswer = '';
//const aSFSPageSize = DEFAULT_PAGE_SIZE;
const aSFSPageSize = 2048;
const aSFSExtentPageCount = 8;
...
type
  TForm1 = class(TForm)
    ...
    procedure BigSFSTestClick(Sender: TObject);
  private
...
{
if (FindFirst('s:\temp\file.*.txt',faAnyFile+32,sr) <> 0) then
...
// SetCurrentTime(time);
 CreateDir('s:\temp\dir');
 FindFirst('s:\temp\d*.*',faDirectory,sr);
 FindClose(sr);
 fh := FileCreate('s:\temp\test1234.txt');
 FileClose(fh);
 FindFirst('s:\temp\test1234.txt',faAnyFile,sr);
 FindClose(sr);

 fh := FileCreate('s:\temp\test12345.txt');
 FileClose(fh);
 FindFirst('s:\temp\test12345.txt',faAnyFile,sr);
 FindClose(sr);

 fh := FileCreate('s:\temp\test1234.txt3');
 FileClose(fh);
 FindFirst('s:\temp\test1234.txt3',faAnyFile,sr);
 FindClose(sr);

 fh := FileCreate('s:\temp\test12345.txt5');
 FileClose(fh);
 FindFirst('s:\temp\test12345.txt5',faAnyFile,sr);
 FindClose(sr);

 fh := FileCreate('s:\temp\test12345.txt123');
 FileClose(fh);
 FindFirst('s:\temp\test12345.txt123',faAnyFile,sr);
 FindClose(sr);

 fh := FileCreate('s:\temp\file12345.abcdef');
 FileClose(fh);
 FindFirst('s:\temp\file12345.abcdef',faAnyFile,sr);
 FindClose(sr);
{
...
//Password := '';
 srcFileName := 's:\temp\test.txt';
 FileName := 's:\temp\test.edb';
 FileName1 := 'file1.dat';
 FileName2 := 'file2.dat';
...
procedure TForm1.BigSFSTestClick(Sender: TObject);
label m1;
var
  bOK: boolean;
  FileName: 	string;
...
begin
 mode := crcFull;
 bOk := true;
// srcFileName := 's:\temp\test.zip';
// FileName := 's:\temp\test.edb';
 srcFileName := TestPath + 'test.zip';
 FileName := TestPath + 'test.sfs';
...
//  FFSHandle := TSingleFileSystem.Create(FileName, fmCreate);
  FFSHandle := TSingleFileSystem.Create(FileName, fmCreate, aSFSInMemory,
                                        aSFSPassword,aSFSQuestion,aSFSAnswer,
                                        aSFSCompressionLevel,
                                        aSFSPageSize,
                                        aSFSExtentPageCount);
 ...
 Info.Text := Info.Text + 'Writing files, FileCount = '+inttostr(TestCount)+
              ...
              '... '+#13#10;
 OverallProgress.Progress := 0;
 OverallProgress.MaxValue := 4; // number of test
 CurrentProgress.MaxValue := TestCount;
 CurrentProgress.Progress := 0;
 aaInitTime;
 Application.ProcessMessages;
...
aaStopTime;
 ...
 try
  for i := 0 to TestCount - 1 do
   begin
	  FileName1 := 'file'+IntToStr(i)+'.zip';
	  FileName2 := 'the_file'+IntToStr(i)+'.zip';
aaStartTime;
    if (not FFSHandle.RenameFile(FileName1,FileName2)) then
     begin
      bOK := false;
      Info.Text := Info.Text + 'Error on user file renaming. FileName = '+
        FileName1+', NewFileName = '+FileName2+#13#10;
      break;
     end;
aaStopTime;
    CurrentProgress.Progress := i+1;
  ...
  bOK := false;
  Info.Text := Info.Text + 'Error on user file renaming.'#13#10;
 end;
 OverallProgress.Progress := 3;
 Info.Text := Info.Text + 'Rename time = '+IntToStr(aaGetTime)+#13#10;
 Application.ProcessMessages;
 ...
 try
  for i := 0 to TestCount - 1 do
   begin
	  ...
	  FileName1 := 'the_file'+IntToStr(i)+'.zip';
	  FHandle := FFSHandle.FileOpen(FileName1, fmOpenRead);
	  if (FHandle < 0) then raise Exception.Create('');
	  if (FFSHandle.FindFirst(FileName1,faAnyFile,sr) <> 0) then
  		raise Exception.Create('');
...
aaStopTime;
 ...
 Info.Text := Info.Text + 'Read time2 = '+IntToStr(aaGetTime)+#13#10;

 m1:
 OverallProgress.Progress := 4;
