...
const TestCount = 4000; // TestCount mod 2 shold be always 0
...
// FileName := 'd:\temp\test.edb';
 srcFileName := 'e:\temp\test.txt';
 FileName := 'e:\temp\test.sfs';
...
fs.Size := 0;
 ...
 // create Single file
 try
  FFSHandle := TSingleFileSystem.Create(FileName, fmCreate, false, '','','',
                                        sfsNone, 1);
  if (FFSHandle = nil) then raise Exception.Create('');
 except
  bOK := false;
