...
begin
 ...
 t := GetTickCount;
 Str := CPSManager1.CompressAnsiString(reText.Text);
...
begin
 CPSManager1.CryptoParams.Password := edPassword.Text;
 t := GetTickCount;
 s := CPSManager1.DecompressAnsiString(Str);
 edDecompTime.Text := IntToStr(GetTickCount-t);
 if (s <> reText.Text) then
  MessageDlg('Error - decompressed AnsiString "'+s+'" does not match source string',mtError,[mbOk],0);
end;
