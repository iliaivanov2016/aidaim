...
{Decides what foldertypes to accept and whether to show path.}
	TBrowseInfoAFlags=(OnlyComputers, OnlyPrinters, OnlyDomains, OnlyAncestors,
		OnlyFileSystem, ShowPath);
{Decides what foldertypes to accept and whether to show path.}
	TBrowseInfoAFlagSet=set of TBrowseInfoAFlags;
...
{The event that is triggered whenever a folder is selected.}
	TSelectionChangedEvent=procedure(Sender: TPBFolderDialog; DialogHandle: HWND; const ItemIDList: PItemIDList; const Folder: AnsiString) of object;
...
{The 'New folder'-button caption and a 'Label'-caption (shown above the path) are automatic localized (national language) detected every time the application runs.}
	...
	private
		...
		FLabelCaption : AnsiString;
		FParentHandle : HWnd;
		FDisplayName : AnsiString;
		FImageIndex : Integer;
		FFolder : AnsiString;
		FSelectedFolder : AnsiString;
		FFlags : TBrowseInfoAFlagSet;
		FRootFolder : TSHFolders;
		FNewFolderVisible : Boolean;
		FNewFolderEnabled : Boolean;
		FNewFolderCaption : AnsiString;
		FNewFolderWidth : Integer;
		FRestart : Boolean;
		FValidPath : Boolean;
		FVersion, FLocale : AnsiString;
		...
		function LocaleText(List : TStringList) : AnsiString;
		function MakeDisplayPath(Path : AnsiString; MaxL : integer) : AnsiString;
		procedure Dummy(Value: AnsiString);
		procedure SetNewFolderCaption(Value : AnsiString);
		...
		procedure SetSelectedFolder(Value : AnsiString);
		procedure SetFlags(Value : TBrowseInfoAFlagSet);
...
{Use it from an event.}
		procedure SetSelectionPath(const Hwnd : HWND; const Path : AnsiString);
...
{Use this procedure to get an ItemIDList, when you know the path.}
		procedure GetIDListFromPath(Path: AnsiString; var ItemIDList: PItemIDList);
...
{The currently selected folder. You can access and set this path in one of the events.}
		property SelectedFolder : AnsiString read FSelectedFolder write SetSelectedFolder;
	published
...
{Decides what foldertypes to accept and whether to show path.}
		property Flags : TBrowseInfoAFlagSet read FFlags write SetFlags;
...
{At runtime the text that fits the Windows-language is used.}
{If the Windows-localeversion is not found in the list the 'Default'-value is used.}
		property NewFolderCaptions : TStringList read FNewFolderCaptions write SetNewFolderCaptions;
//ReadOnly property.
		property Version : AnsiString read FVersion write Dummy stored False;
	end;
...
const
	...
	BROWSE_FLAG_ARRAY: array[TBrowseInfoAFlags] of Integer = (BIF_BROWSEFORCOMPUTER,
...
begin
	...
	case Screen.PixelsPerInch of
		72, 96: NewFolderWindowHandle:=Windows.CreateWindowA('Button', PAnsiChar(Instance.FNewFolderCaption),
			ControlCreateStyles, 12, Height0 - 36,
			Instance.FNewFolderWidth, 23, HWindow, _BUTTON_ID, HInstance, nil);
		120: NewFolderWindowHandle:=Windows.CreateWindowA('Button', PAnsiChar(Instance.FNewFolderCaption),
			ControlCreateStyles, 15, Height0 - 45,
			Instance.FNewFolderWidth * 5 div 4, 28, HWindow, _BUTTON_ID, HInstance, nil);
		144: NewFolderWindowHandle:=Windows.CreateWindowA('Button', PAnsiChar(Instance.FNewFolderCaption),
			ControlCreateStyles, 17, Height0 - 56,
			Instance.FNewFolderWidth * 3 div 2,
			35, HWindow, _BUTTON_ID, HInstance, nil);
		192: NewFolderWindowHandle:=Windows.CreateWindowA('Button', PAnsiChar(Instance.FNewFolderCaption),
			ControlCreateStyles, 20,
			Height0 - 66,
			Instance.FNewFolderWidth * Screen.PixelsPerInch div 96,
			41, HWindow, _BUTTON_ID, HInstance, nil);
		else NewFolderWindowHandle:=Windows.CreateWindowA('Button', PAnsiChar(Instance.FNewFolderCaption),
...
begin
	...
	case uMsg of
		...
		BFFM_SELCHANGED:
		begin
			ShlObj.SHGetPathFromIDListA(PItemIDList(lParameter),TempPath);
			...
			begin
				...
				SendMessage(HWindow,BFFM_SETSTATUSTEXT,0,Longint(PAnsiChar(Path)));
			end;
...
begin
	...
	FNewFolderCaptions.CommaText := '"Default=New folder", "0009=New folder",' +
		...
		' "1C09=New folder", "2009=New folder", "2809=New folder",' +
		' "2C09=New folder"';
...
procedure TPBFolderDialog.GetIDListFromPath(Path: AnsiString; var ItemIDList: PItemIDList);
var
...
procedure TPBFolderDialog.SetSelectionPath(const Hwnd: HWND; const Path: AnsiString);
var
...
procedure TPBFolderDialog.SetNewFolderCaption(Value: AnsiString);
begin
	FNewFolderCaption:=Value;
	if (IsWindow(FNewFolderHandle)) then SetWindowText(FNewFolderHandle,PAnsiChar(Value));
end;
...
procedure TPBFolderDialog.SetSelectedFolder(Value: AnsiString);
begin
	SetSelectionPath(DialogHandle,Value);
end;

procedure TPBFolderDialog.SetFlags(Value : TBrowseInfoAFlagSet);
begin
	FFlags := Value;
end;
...
procedure TPBFolderDialog.Dummy(Value: AnsiString);
begin
//	Read only !
end;

function TPBFolderDialog.LocaleText(List : TStringList) : AnsiString;
begin
...
function TPBFolderDialog.MakeDisplayPath(Path : AnsiString; MaxL : integer) : AnsiString;
var
...
begin
	...
	try
		...
		BrowseInfo.pszDisplayName:=PAnsiChar(FDisplayName);
		BrowseInfo.lpszTitle:=PAnsiChar(FLabelCaption);
		BrowseInfo.ulFlags:=0;
		for i:=0 to NUMBER_OF_BROWSE_INFO_FLAGS-1 do
		begin
			if( TBrowseInfoAFlags(i) in FFlags)
				then	BrowseInfo.ulFlags:=BrowseInfo.ulFlags
				or LongWord(BROWSE_FLAG_ARRAY[TBrowseInfoAFlags(i)]);
		end;
		...
		repeat
			...
			ItemIDList:=ShlObj.SHBrowseForFolderA(BrowseInfo);
		until not FRestart;

		Result:=(ItemIDList <> nil);
		if Result then
		begin
			ShlObj.SHGetPathFromIDListA(ItemIDList,TempPath);
